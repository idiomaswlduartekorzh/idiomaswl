...
const CATEGORIES = Object.keys(CATEGORY_LABELS).filter(c => !['educacion', 'salud', 'corrupcion'].includes(c));
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {CATEGORIES.map((cat, i) => (
            <motion.div key={cat} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}>
              <Link href={`/indicadores/${cat}`} className="block glass-card rounded-xl p-6 hover:border-slate-600 transition-all group" style={{ borderLeft: `3px solid ${CATEGORY_COLORS[cat] || '#3b82f6'}` }}>
                <div className="text-2xl mb-3">{CATEGORY_ICONS[cat]}</div>
                <h2 className="text-white font-semibold mb-1 group-hover:text-blue-400 transition-colors">{CATEGORY_LABELS[cat]}</h2>
