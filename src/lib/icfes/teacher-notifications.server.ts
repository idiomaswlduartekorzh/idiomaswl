...
function publicOrigin(): string {
  ...
  try {
    if (configured) {
      ...
      if (value.protocol === 'https:' || process.env.VERCEL_ENV !== 'production') return value.origin;
    }
  } catch {}
...
async function loadNotificationContext(row: NotificationRow) {
  ...
  const { data: review, error: reviewError } = await db.from('xpress_teacher_reviews')
    .select('id,membership_id,icfes_attempt_id,status,requested_at,due_at,completed_at')
...
async function deliverNotification(row: NotificationRow): Promise<void> {
  ...
  if (isOwnerNotice) {
    const ownerEmail = process.env.COURSE_OWNER_NOTIFICATION_EMAIL || DEFAULT_OWNER_EMAIL;
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(ownerEmail)) throw new Error('icfes_teacher_owner_email_not_configured');
    ...
    await sendCourseEmail({
      to: ownerEmail,
      subject: subjects[row.kind as Exclude<NotificationKind, 'student_completed'>],
  ...
  await sendCourseEmail({
    ...
    idempotencyKey: `icfes-teacher/${row.id}`,
    html: `<h1>Tu retroalimentación ICFES está lista</h1><p>Terminamos la revisión humana de tu intento.</p><p><a href="${escapeEmailHtml(resultUrl)}">Ver mi retroalimentación</a></p><p>Debes iniciar sesión con la misma cuenta que solicitó la revisión.</p>`,
  });
}
