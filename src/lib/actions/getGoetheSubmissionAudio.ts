...
export async function getGoetheSubmissionAudio(submissionId: string): Promise<
  { ok: true; files: { questionId: string; signedUrl: string }[] } | { ok: false; error: string }
> {
  try {
    await requireAdmin()
    if (!GOETHE_SUBMISSION_ID_PATTERN.test(submissionId)) return { ok: false, error: 'La entrega no es válida.' }
    const admin = createAdminClient()
    const { data: submission, error } = await admin.from('exam_submissions').select('speaking_audio_paths')
    ...
    const signed = await Promise.all(paths.map(async ([questionId, path]) => {
      const { data } = await admin.storage.from(GOETHE_SPEAKING_BUCKET).createSignedUrl(path, 5 * 60)
