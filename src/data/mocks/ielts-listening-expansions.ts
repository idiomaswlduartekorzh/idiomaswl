...
function densityCoda(setNumber: number, part: number, context: string, labels: { A: string; B: string }) {
  if (part === 1) return `${labels.A}: Before I close this ${context}, I will separate the details we have confirmed from the points that remain optional. Please listen to the complete summary before responding, because correcting one item can sometimes affect a later choice as well. I will keep the original order so that it is easy to compare with your notes.

${labels.B}: That is helpful. I will check each detail as you read it and ask about anything that does not match what I intended. Once the summary is accurate, I will also note the reference and the method for making a later amendment.`;
  if (part === 2) return `Before the ${context} moves to its final practical details, remember that similar locations or services may have different conditions. Listen for the full name, the direction attached to it and any time restriction. A descriptive example may help you recognise a place, but only a direct instruction tells you to change route or take action. If a temporary arrangement replaces the normal one, the replacement will be stated after the original is identified. This final distinction connects the earlier overview with the choices that follow and helps prevent a familiar name from being confused with the service currently available.`;
  if (part === 3) return `${labels.A}: Before we finish the ${context}, let us read the argument from the perspective of somebody who did not collect the evidence. Each claim should identify what was observed, how it was measured and which limitation affects the interpretation.

${labels.B}: I will also check that our recommendation follows from the result rather than from what we expected at the beginning. If a sentence sounds stronger than the method allows, we can narrow it without losing the main point.

${labels.A}: Good. Then the final version will show a clear line from the research question through the evidence to the conclusion.`;
  return `One final point about the ${context} concerns the difference between a useful model and a complete explanation. A model selects the relationships that matter most for a particular question, so it can be informative even when it leaves out local variation. Problems arise when that simplification is forgotten and a result from one scale is treated as a rule for every case. The evidence discussed here should therefore be read with its conditions attached: the period observed, the population or material studied, the method used and the uncertainty that remains. Those conditions do not weaken the account; they show exactly where the conclusion is supported and where further observation would be needed.`;
}
...
export function expandIeltsListeningTranscript(setNumber: number, part: number, transcript: string) {
  ...
  if (projectedWords < targetPartWords) {
    const context = CONTEXTS[setNumber]?.[part - 1] ?? 'listening section';
    const coda = densityCoda(setNumber, part, context, labels);
    additions.push(coda);
    projectedWords += countWords(coda);
  }
  if (projectedWords >= 650 && projectedWords < targetPartWords) {
    const tail = part === 1 || part === 3
      ? `${labels.A}: We will make that final check slowly, preserve the order of the information and correct any difference before treating the record as complete.`
      : `This final check preserves the order of the explanation and makes clear which conclusion follows from the evidence already described.`;
    additions.push(tail);
    projectedWords += countWords(tail);
  }
