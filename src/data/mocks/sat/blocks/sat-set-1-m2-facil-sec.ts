...
export const meta: SatItemMeta[] = [
  ...
  },
  {
    ...
    razones: {
      ...
      D:
        "Correcta: el núcleo del sujeto es «the row», singular, aunque entre él y el verbo se interpongan «of gauges» y la relativa «that the port authority has kept at intervals along the estuary»; y el presente simple es el tiempo que exige el verbo coordinado «and stores».",
    },
    fuenteHecho:
      "Oceanografía de manual: el mareógrafo de flotador en pozo tranquilizador, con plumilla sobre tambor de relojería, deja un trazo **continuo** sobre el papel —de ahí «without a break»— y no una lectura cada diez minutos, que es lo que hace un registrador digital. El nivel del mar es la serie larga de esos rollos. El puerto y el estuario son invención propia.\n\n**Corte de complejidad y de localización (22 ago 2026).** T: se parte la oración de apertura —el tambor de relojería pasa a oración propia— y la cola de la oración del hueco. De **20,6 a 15,3** palabras por oración, de cinco oraciones a siete. «Its value» pasa a «The value of the gauge» porque el posesivo se quedaba sin antecedente claro al cortar, y «nothing grander» a «nothing more» por frecuencia; ninguna de las dos está citada en ninguna razón. L: la oración del hueco pasa de **43 a 34** palabras, el tramo del hueco a «and stores» de **13 a 11** y lo que queda detrás de «stores» de **11 a 5**, porque la relativa final —«a cupboard that is opened only after a storm»— sale a oración propia y ya no obliga a volver atrás. Lo que **no** puede bajar es el tramo de «row» al hueco: esa frase interpuesta es la regla que el ítem mide y `razones.D` la cita entera. El núcleo, la interposición y el verbo coordinado «and stores» siguen los tres dentro de la oración del hueco. Ni una opción ni una razón se tocaron.\n\n**Verosimilitud del sujeto (23 ago 2026).** La oración decía que la hilera de mareógrafos «stores the rolls in a cupboard», y una hilera de instrumentos no guarda rollos en un armario: eso lo hace alguien. Importa más de lo que parece porque «and stores» es el verbo coordinado que fija el tiempo de la clave y lo citan tres de las cuatro razones; si su sujeto no es creíble, el estudiante duda de la coordinación por donde no debe. → «…and stores the record on rolls of paper. The rolls go into a cupboard that is opened only after a storm.» El verbo se queda donde estaba, el núcleo «row» y la frase interpuesta también, y la oración del hueco pasa de 34 a 35 palabras. El pasaje sube de 15,3 a 16,1 palabras por oración: por debajo de 18, así que T sigue en 1. Ni una opción ni una razón se tocaron.\n\n**Fuera «since 1892» (23 ago 2026): pedía una forma que no está en la lista.** La frase interpuesta terminaba en «since 1892», y un complemento de duración abierta empuja al presente perfecto: quien lee bien busca «has traced» —singular, concordando con «row»— y no lo encuentra, porque las cuatro opciones son «trace», «traced», «have traced» y «traces». El estudiante fuerte quedaba castigado por leer bien, que es justo lo contrario de lo que el ítem mide. La interposición pasa a «that the port authority has kept at intervals along the estuary»: **sigue teniendo trece palabras** de «row» al hueco, que es el tramo que la CALIBRACIÓN del 22 ago dejó escrito que no puede bajar porque es la regla misma. El núcleo «row» no se toca, «has kept» no se toca —`razones.C` lo cita como la forma que el estudiante copia y pluraliza— y el verbo coordinado «and stores», que es lo que fija el tiempo de la clave y lo citan tres razones, tampoco. Con la duración fuera, «traces» es la única forma que concuerda en número con «row» y en tiempo con «stores», y los tres distractores conservan su camino: «trace» concuerda con lo más cercano, «traced» acierta el número y falla el tiempo, «have traced» falla los dos. `razones.D` citaba la relativa entera y literal, así que se actualiza la cita y solo la cita; el resto de las cuatro razones no se tocó, ni una opción cambió y la fecha no reaparece en ninguna otra parte del pasaje.",
  },
  {
    id: 'q22',
    domain: 'SEC',
