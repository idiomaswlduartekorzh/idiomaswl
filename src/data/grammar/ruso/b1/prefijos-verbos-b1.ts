...
const topic: GrammarTopic = {
  ...
  seo: [
    ...
    {heading: '¿Qué aporta el prefijo за- (comienzo y resultado)?', paragraphs: ['"работать" → "заработать" (earn money — resultado del trabajo). "писать" → "записать" (write down — anotar, registrar).', 'За- suele crear perfectivos y denota logro o inicio.']},
    ...
    {heading: '¿Qué aporta el prefijo пере- (repetición y cambio)?', paragraphs: ['"писать" → "переписать" (rewrite — hacer de nuevo). "идти" → "перейти" (cross — cambio de lado/dirección).', 'Muy productivo en ruso para dentar repetición o cambio de dirección.']},
