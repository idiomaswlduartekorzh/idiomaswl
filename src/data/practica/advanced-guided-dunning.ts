...
export const DUNNING_KRUGER_GUIDED_LESSON: GuidedAdvancedLesson = {
  ...
  title: 'Calibration, competence and the graph that escaped the lab',
  subtitle: 'A C1 seminar on what the evidence measures, what the meme invents and how better feedback changes the question.',
  objective: 'You will distinguish confidence from calibration, evaluate metacognitive and statistical explanations, and formulate a claim that stays proportional to the evidence.',
  level: 'B2–C1',
  evidenceClass: 'empirical',
  guidedMinutes: 138,
  selfStudyMinutes: 82,
  centralQuestion: 'When does inaccurate self-assessment reflect missing knowledge, and when can the measurement itself create the pattern?',
  openingStatements: {
    title: 'Which claims currently seem most defensible?',
    instruction: 'Choose at least two. You are recording a starting position, not taking a test. Your selections will remain visible when you return at the end.',
    minimum: 2,
    statements: [
      { id: 'confidence-evidence', text: 'Confidence alone is useful evidence of competence.' },
      { id: 'novice-rule', text: 'Novices are usually more confident than experts.' },
      { id: 'relative-error', text: 'Some low performers misjudge their relative performance.' },
      { id: 'measurement-shape', text: 'Measurement choices can create part of the famous pattern.' },
      { id: 'expert-humility', text: 'Greater expertise consistently produces greater humility.' },
      { id: 'feedback-calibration', text: 'Calibration improves when predictions meet diagnostic feedback.' },
    ],
  },
  discussion: {
    targetMinutes: 20,
    questions: [
      ...
      { id: 'coupling', kind: 'pattern', prompt: 'Should competence and confidence rise together at the same rate?', teacherIntent: 'Elicit possible relationships before introducing calibration.', followUps: ['Could accurate experts remain highly confident?', 'Could an uncertain novice still overestimate a very low score?'] },
      ...
      { id: 'alternative', kind: 'exception', prompt: 'How could a bottom group appear overconfident without a unique psychological deficit?', teacherIntent: 'Invite statistical alternatives such as noise, boundaries and regression.', followUps: ['How was the group selected?', 'What happens near the edge of a scale?'] },
      { id: 'test', kind: 'evidence', prompt: 'What evidence would distinguish missing metacognitive skill from a statistical artifact?', teacherIntent: 'End with falsifiability and a better design.', followUps: ['Would repeated measures help?', 'What should training change if the mechanism is real?'] },
    ],
  },
  recordings: {
