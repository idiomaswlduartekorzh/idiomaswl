...
const NARRATOR_QS: Question[] = [
  ...
  },
  {
    ...
    opts: [
      'ミサキ は スマホ の 中身 に 興味 が なく、 ただ の 冗談 だった',
      'ミサキ は リョウ を 傷つける ため に、 わざと 冗談 で からかった',
  ...
  },
  {
    ...
    opts: [
      '断る 人 は たいてい 疲れて いる か 忙しい だけ で、 深い 意味 は ない',
      '無実 なら 見せた はず だ — だから 断る こと 自体 が 証拠 に なる',
      'リョウ が スマホ を 共有 する と 約束 して いた のに、 それ を 破った',
      '日本語 で は はっきり 断る の が いつも 失礼 に なる',
    ],
    correct: 1,
    explanation:
...
const A_QS: Question[] = [
  {
    ...
    opts: [
      '二年分 の メッセージ を 読んで、 おかしな 相手 が いない か 確かめる こと',
      '夕食中 に 届いた 通知 を 送った の が 誰 な の か、 名前 を 知る こと',
      'リョウ が 自分 から 渡す 姿 を 見る こと — 頼み は テスト だった',
      '位置情報 の 履歴 を 開いて、 金曜日 の 夜 の 行き先 を 確認 する こと',
    ],
    correct: 2,
    explanation:
  ...
  },
  {
    ...
    opts: [
      '文 の 形 が 間違って いて、 質問 と して 成り立って いない',
      '断る こと を 罪 と 同じ に して いる ので、 リョウ が どう 答えて も 晴らせ ない',
      'ミサキ が スマホ の 中身 を すでに 知って いて、 知らない ふり を して いる こと を 示す',
      'ミサキ が 二人 の 関係 に 必要 な 信用 を もう 大事に して いない こと を 示す',
    ],
    correct: 1,
    explanation:
  ...
  },
  {
    ...
    opts: [
      '落ち着いて いて 分析的 で、 感情 を 出さ ず に 話して いる',
      '格式 ばって いて 事務的 で、 仕事 の 報告 の よう な 話し方',
      '怒って いて 皮肉 っぽい が、 その 下 に 不安 と 痛み が ある',
      '最初 から 最後 まで 軽く て 楽しく、 笑い話 に なって いる',
    ],
    correct: 2,
    explanation:
  ...
  },
  {
    ...
    opts: [
      '関係 に は 線 が 大事 だ と いう リョウ の 考え を 丁寧 に 認めて いる',
      'リョウ の 言葉 を そのまま 返して 皮肉 に し、 言い訳 と して 退けて いる',
      '「線」 と いう 言葉 の 意味 が 分からず、 もう 一度 説明 して ほしい と 頼んで いる',
      'カウンセラー が よく 使う 言い方 を そのまま 引用 して いる',
    ],
    correct: 1,
    explanation:
  ...
  },
  {
    ...
    opts: [
      '法律 の 文書 に 出て くる 決まった 言い方 を そのまま 引用 して いる',
      'リョウ の 言葉 を 引用 して 距離 を 取り、 その 非難 を 受け入れない と 示す',
      '日本語 で は この 表現 に 必ず かぎ括弧 を つける 決まり が あり、 書き方 の 話 だ',
      'その 言葉 を どう 訳せば いい か 分からず、 かぎ括弧 で 囲んだ だけ だ',
    ],
    correct: 1,
    explanation:
...
const B_QS: Question[] = [
  {
    ...
    opts: [
      '見せたく ない メッセージ が 残って いて、 読まれる と 説明 が 面倒 に なる から',
      'スマホ が 会社 の もの で、 仕事 の 情報 を 人 に 見せて は いけない から',
      '隠して いる から で は なく、 一度 見せる と 確認 が 当たり前 に なる から',
      'パスワード を 忘れて しまい、 その 場 で 開く こと が でき なかった から',
    ],
    correct: 2,
    explanation:
  ...
  },
  {
    ...
    opts: [
      ...
      'ミサキ が 実際 に 言った 要求 を そのまま 並べた 引用',
      '夕食 の とき の 自分 の 態度 に ついて の 遠回し な 謝罪',
      'すでに 二人 の 間 で 起きた 事実 を 時間 の 順 に 並べた 話',
    ],
    correct: 0,
    explanation:
  ...
  },
  {
    ...
    opts: [
      '警察 が 本当に リョウ を 調べて いて、 その こと を ミサキ に 言えず に いる',
      'リョウ は ミサキ の 元彼 の 行い の 代償 を 自分 が 払って いる と 感じて いる',
      'リョウ が 元彼 と 同じ こと を して しまい、 その 証拠 を 見られ たく ない と 思って いる',
      'リョウ は ミサキ に、 元彼 と もう 一度 連絡 を 取って ほしい と 思って いる',
    ],
    correct: 1,
    explanation:
  ...
  },
  {
    ...
    opts: [
      'ミサキ が 質問 を せず に、 いきなり スマホ を 出せ と 言った こと',
      'その 頼み は 正直 に 合格 する 方法 が ない こと — どんな 反応 でも 疑い を 裏づける',
      'もう この 関係 を 続ける の は 無理 だ と 感じて いて、 別れたい と 思い 始めて いる こと',
      'ミサキ の 質問 の 意味 が 理解 でき ず、 何 を 答えれば いい か 分から なかった こと',
    ],
    correct: 1,
    explanation:
  ...
  },
  {
    ...
    opts: [
      'あの 場 の 言い方 に ついて 完全 に 責任 を 引き受けて いる',
      '何 が 起きた の か 自分 でも 全く 分から ない 混乱',
      '小さな 譲歩 を して すぐ 引っ込める — 謝罪 の 形 だけ の 文',
      '本当に 後悔 して いて、 これ から 変わる と 決めた 言い方',
    ],
    correct: 2,
    explanation:
...
const FINAL_QS: Question[] = [
  ...
  },
  {
    ...
    opts: [
      'どちら か が わざと 嘘 を ついて いて、 話 が 合わない の は その せい だ',
      '同じ 行為 でも、 それ が 何 の しるし だ と 思う か で 正反対 の 意味 に なる',
      '二人 と も その 夜 の こと を よく 覚えて いなく て、 記憶 が ずれて いる',
      'リョウ の 方 が 難しい 語彙 を 使う ので、 同じ 出来事 が 大げさ に 聞こえる',
    ],
    correct: 1,
    explanation:
  ...
  },
  {
    ...
    opts: [
      'リョウ が スマホ の 中 に 見せられ ない もの を 隠して いて、 見つかる の を こわがって いる',
      'ミサキ が もともと 嫉妬 深い 性格 で、 相手 が 誰 でも 同じ こと を 言った はず だ',
      '二人 の 間 で プライバシー が 何 を 意味 する か、 試される まで 一度 も 決めて いなかった',
      'ミサキ の 元彼 が 昔 した こと が、 今 の 二人 の 間 に そのまま 残って いる こと',
    ],
    correct: 2,
    explanation:
  ...
  },
  {
    ...
    opts: [
      '二人 が 同じ 映画 の 台詞 を 引用 して いて、 いつも 一緒 に 見て いた こと が 分かる',
      '同じ 「線」 の イメージ で 正反対 の 立場 を 守って いる — どちら も 自分 が 線 を 守る 側 だ と 思って いる',
      '二つ の 場合 で 「線」 と いう 言葉 の 意味 が 全く 違って いて、 同じ 言葉 に 聞こえる の は 偶然 に すぎない',
      'リョウ が ミサキ の 語彙 を そのまま まねて いる 証拠 で、 話 を 合わせよう と して いる こと が 分かる',
    ],
    correct: 1,
    explanation:
  ...
  },
  {
    ...
    opts: [
      'ミサキ は 本当 は 最初 から 中身 を 読む つもり で、 見る 気 は なかった と いう 言い方 は 断られた 後 で 作った 言い訳 だ',
      'ミサキ が ほしかった の は 情報 で は なく 安心 だった — そして それ を 与えられる の は、 リョウ が 主義 と して 断った まさに その 行為 だけ だった',
      'ミサキ は 前 に も リョウ の スマホ を 黙って 開いた こと が あり、 今回 は その とき 見た 内容 を リョウ 本人 の 口 から 言わせよう と した だけ だった',
      'ミサキ は パスワード を 知らない ので、 リョウ 本人 に 開いて もらう しか なかった',
    ],
    correct: 1,
    explanation:
