...
const NARRATOR_QS: Question[] = [
  ...
  },
  {
    ...
    opts: [
      '만호 씨는 다른 뜻 없이 확실히 관대했다',
      '그 관대함 뒤에 그때는 보이지 않던 무언가가 더 있었을 수 있다',
      '가족은 처음부터 만호 씨가 조건을 단다는 걸 알았다',
      '만호 씨는 대놓고 가족을 통제하려고 했다',
    ],
    correct: 1,
    explanation:
...
const A_QS: Question[] = [
  ...
  },
  {
    ...
    opts: [
      '시아버지의 부동산 사업 이야기를 진지하게 하고 있다',
      '존댓말은 그대로 두면서, 가족을 투자처럼 다루는 태도를 비꼬고 있다',
      '시아버지의 관점에 정중하게 동의하고 있다',
      '목록을 그대로 인용하고 있다',
    ],
    correct: 1,
    explanation:
...
const FINAL_QS: Question[] = [
  ...
  },
  {
    ...
    opts: [
      '이야기하는 사람이 만호 씨를 싫어한다',
      '만호 씨가 생각한 의도와 그 말이 실제로 준 느낌 사이에 큰 차이가 있다',
      '수진이 대부분을 지어냈다',
      '이야기하는 사람이 사실을 잘못 알았다',
    ],
    correct: 1,
    explanation:
