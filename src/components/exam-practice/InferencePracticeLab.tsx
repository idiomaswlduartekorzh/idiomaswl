...
import { CheckCircle2, Clock3, LockKeyhole, RotateCcw, Scale, XCircle } from 'lucide-react';
import {
  ...
  type InferenceTrainingPassage,
} from '@/data/practica-exams/ielts-reading-inference-progress';
...
type StoredProgress = {
  ...
  errorCounts: Partial<Record<InferenceErrorCode, number>>;
  reviewQueue: string[];
...
const ERROR_LABELS: Record<InferenceErrorCode, string> = {
  'outside-knowledge': 'used a plausible fact that the passage never supplied',
  'certainty-inflation': 'turned a possibility or tendency into a guarantee',
  'causation-invented': 'invented or reversed a causal relationship',
  'scope-overreach': 'made the conclusion wider than the evidence',
  contradiction: 'selected a conclusion contradicted by an explicit clue',
  'detail-not-conclusion': 'repeated one detail without connecting the evidence',
};
...
function normalizeProgress(value: unknown): StoredProgress {
  ...
  const knownLevels = new Set(INFERENCE_LEVELS.map((level) => level.id));
  ...
  const unlockedLevel = Math.min(INFERENCE_LEVELS.length - 1, nonNegativeInteger(parsed.unlockedLevel));
...
function readProgress() {
  try { return normalizeProgress(JSON.parse(window.localStorage.getItem(INFERENCE_STORAGE_KEY) ?? window.localStorage.getItem(INFERENCE_LEGACY_STORAGE_KEY) ?? 'null')); }
  catch { return EMPTY_PROGRESS; }
}
function writeProgress(progress: StoredProgress) { try { window.localStorage.setItem(INFERENCE_STORAGE_KEY, JSON.stringify(progress)); } catch { /* The practice remains usable without storage. */ } }
...
function ChoiceCards({ options, selected, disabled, label, onSelect }: { options: string[]; selected: string; disabled: boolean; label: string; onSelect: (value: string) => void }) {
  const group = `infer-${useId().replace(/:/g, '')}`;
...
function Passage({ passage }: { passage: InferenceTrainingPassage }) {
...
function SourceBoundary({ passage }: { passage: InferenceTrainingPassage }) {
...
function DecisionCard({ decision, selected, disabled, attemptSeed, showInvariant = false, onSelect }: { decision: InferenceDecision; selected: string; disabled: boolean; attemptSeed: number; showInvariant?: boolean; onSelect: (value: string) => void }) {
  return <><div className={styles.quoteBlock}><p className={styles.panelLabel}>Evidence bridge</p><blockquote>1 · {decision.evidence[0]}</blockquote><blockquote>2 · {decision.evidence[1]}</blockquote>{showInvariant && <p><strong>Reasoning control:</strong> {decision.reasoningControl}</p>}</div><ChoiceCards options={getInferenceOptions(decision, attemptSeed)} selected={selected} disabled={disabled} label={`Safest conclusion for ${decision.question}`} onSelect={onSelect} /></>;
}

export function InferenceGuidedPractice({ passage }: { passage: InferenceTrainingPassage }) {
  ...
  return <section className={styles.lab} data-active-practice="true" aria-label="Guided inference practice"><div className={styles.labTopline}><div><span className={styles.modeTag}>Guided practice · three evidence bridges</span><h3>{passage.title}</h3><p>Combine only the explicit clues, then choose the smallest conclusion the passage allows.</p></div><button type="button" className="btn btn-ghost" onClick={reset}><RotateCcw size={16} aria-hidden="true" />{confirmReset ? 'Press again to reset' : 'Reset guided set'}</button></div><Passage passage={passage} /><div className={styles.engineQuestions}>{decisions.map((decision, index) => { const done = Boolean(checked[decision.id]); const correct = answers[decision.id] === decision.answer; return <article className={styles.engineQuestion} key={decision.id}><div className={styles.questionMeta}><span>{String(index + 1).padStart(2, '0')}</span><small>Locate → connect → restrain</small></div><h3>{decision.question}</h3><DecisionCard decision={decision} selected={answers[decision.id] ?? ''} disabled={done} attemptSeed={attemptSeed} showInvariant onSelect={(value) => { setAnswers((current) => ({ ...current, [decision.id]: value })); setConfirmReset(false); }} />{!done && <button type="button" className="btn btn-primary" disabled={!answers[decision.id]} onClick={() => setChecked((current) => ({ ...current, [decision.id]: true }))}>Check inference</button>}{done && <div className={`${styles.feedback} ${correct ? styles.feedbackCorrect : styles.feedbackIncorrect}`} role="status">{correct ? <CheckCircle2 size={18} aria-hidden="true" /> : <XCircle size={18} aria-hidden="true" />}<div><strong>{correct ? 'Inference supported.' : `Safest conclusion: ${decision.answer}`}</strong><p>{correct ? decision.explanation : decision.trap}</p></div></div>}</article>; })}</div><SourceBoundary passage={passage} /></section>;
}

export function InferenceIndependentPractice({ passage }: { passage: InferenceTrainingPassage }) {
  const [answers, setAnswers] = useState<AnswerMap>({});
  ...
  return <section className={styles.lab} data-active-practice="true" aria-label="Independent inference set"><div className={styles.labTopline}><div><span className={styles.modeTag}>Full passage · feedback closed</span><h3>{passage.title}</h3><p>Complete all five conclusions before feedback opens. Answer positions change on a clean retry.</p></div><span className={styles.counter} aria-live="polite">{answered}/5 inferred</span></div><Passage passage={passage} /><div className={styles.independentGrid}>{passage.decisions.map((decision) => { const correct = answers[decision.id] === decision.answer; return <article className={styles.independentCard} key={decision.id}><p className={styles.panelLabel}>Inference decision</p><h3>{decision.question}</h3><DecisionCard decision={decision} selected={answers[decision.id] ?? ''} disabled={submitted} attemptSeed={attemptSeed} onSelect={(value) => { setAnswers((current) => ({ ...current, [decision.id]: value })); setConfirmReset(false); }} />{submitted && <div className={`${styles.feedback} ${correct ? styles.feedbackCorrect : styles.feedbackIncorrect}`} role="status">{correct ? <CheckCircle2 size={18} aria-hidden="true" /> : <XCircle size={18} aria-hidden="true" />}<div><strong>{correct ? 'Supported inference.' : `Safest conclusion: ${decision.answer}`}</strong><p>{correct ? decision.explanation : decision.trap}</p></div></div>}</article>; })}</div><div className={styles.submitBar} aria-live="polite"><div>{submitted ? <><strong>{score}/5 correct</strong><span>{score >= 4 ? 'Independent transfer target reached.' : 'Review each unsupported reasoning step, then retry.'}</span></> : <><strong>One complete submission</strong><span>Feedback stays closed so every conclusion remains independent.</span></>}</div><div className={styles.actions}><button type="button" className="btn btn-ghost" onClick={reset}><RotateCcw size={16} aria-hidden="true" />{confirmReset ? 'Press again to reset' : 'Reset set'}</button>{!submitted && <button type="button" className="btn btn-primary" disabled={answered !== 5} onClick={() => setSubmitted(true)}>Submit inference set</button>}</div></div><SourceBoundary passage={passage} /></section>;
}

type EngineQuestion = { passage: InferenceTrainingPassage; decision: InferenceDecision };
function buildQuestions(levelIndex: number): EngineQuestion[] {
  const level = INFERENCE_LEVELS[levelIndex];
  const passages = level.passageIds.map(getInferencePassage).filter((item): item is InferenceTrainingPassage => Boolean(item));
...
export function InferenceProgressEngine() {
  ...
  const completed = INFERENCE_LEVELS.filter((item) => progress.levels[item.id]?.mastered).length;
  const topErrors = Object.entries(progress.errorCounts).sort(([, a], [, b]) => (b ?? 0) - (a ?? 0)).slice(0, 3) as Array<[InferenceErrorCode, number]>;
  ...
  return <section className={styles.engine} data-active-practice="true" aria-labelledby="inference-progress-title"><div className={styles.engineHeading}><div><span className={styles.modeTag}>WeLearn Progress Engine</span><h2 id="inference-progress-title">Build evidence-constrained conclusions across six levels</h2><p>Two reasoning drills lead into four full passage sets. Drafts, time, scores and inference patterns stay on this device.</p></div><div className={styles.engineScore}><strong>{completed}/6</strong><span>levels mastered</span></div></div><div className={styles.dashboard}><div><Scale size={19} aria-hidden="true" /><span><strong>{progress.reviewQueue.length}</strong> inferences in review</span></div><div><Clock3 size={19} aria-hidden="true" /><span><strong>{formatTime(elapsed)}</strong> current attempt</span></div><div><CheckCircle2 size={19} aria-hidden="true" /><span><strong>{hydrated ? 'Saved locally' : 'Loading…'}</strong> draft and progress</span></div></div><nav className={styles.levelRail} aria-label="Inference progress levels">{INFERENCE_LEVELS.map((item, index) => { const locked = index > progress.unlockedLevel; const record = progress.levels[item.id]; return <button key={item.id} type="button" disabled={locked} aria-current={index === levelIndex ? 'step' : undefined} className={index === levelIndex ? styles.levelCurrent : record?.mastered ? styles.levelMastered : ''} onClick={() => switchLevel(index)}><span>{locked ? <LockKeyhole size={14} aria-hidden="true" /> : index + 1}</span><b>{item.title}</b><small>{record ? `Best ${record.bestScore}/${index < 2 ? 4 : 5}` : locked ? 'Locked' : 'Ready'}</small></button>; })}</nav><div className={styles.enginePanel}><header className={styles.levelHeader}><div><span>Level {levelIndex + 1} of 6 · {level.focus}</span><h3>{level.title}</h3><p>{level.instruction}</p></div><div className={styles.levelTarget}><strong>{level.masteryScore}/{questions.length}</strong><span>to master</span></div></header><div className={styles.engineQuestions}>{questions.map(({ passage, decision }, index) => { const selected = answers[decision.id] ?? ''; const correct = selected === decision.answer; return <article className={styles.engineQuestion} key={decision.id}><div className={styles.questionMeta}><span>{String(index + 1).padStart(2, '0')}</span><small>{passage.title} · {decision.reasoningControl}</small></div><h3>{decision.question}</h3><DecisionCard decision={decision} selected={selected} disabled={submitted} attemptSeed={attemptSeed} onSelect={(value) => { setAnswers((current) => ({ ...current, [decision.id]: value })); setConfirmAttemptReset(false); }} />{submitted && <div className={`${styles.feedback} ${correct ? styles.feedbackCorrect : styles.feedbackIncorrect}`} role="status">{correct ? <CheckCircle2 size={18} aria-hidden="true" /> : <XCircle size={18} aria-hidden="true" />}<div><strong>{correct ? 'Supported inference.' : `Safest conclusion: ${decision.answer}`}</strong><p>{correct ? decision.explanation : decision.trap}</p></div></div>}</article>; })}</div><footer className={styles.engineFooter} aria-live="polite"><div>{submitted ? <><strong>{score}/{questions.length} correct · {mastered ? 'Skill level mastered' : 'Target not reached yet'}</strong><span>{mastered ? 'The next level is available. This is a WeLearn micro-skill result, not an IELTS band.' : `Review ${questions.length - score} inference${questions.length - score === 1 ? '' : 's'}, then retry.`}</span></> : <><strong>{answered}/{questions.length} decisions complete</strong><span>Answers remain editable and saved locally until submission.</span></>}</div><div className={styles.actions}><button type="button" className="btn btn-ghost" onClick={resetAttempt}><RotateCcw size={16} aria-hidden="true" />{confirmAttemptReset ? 'Press again to reset' : 'Reset attempt'}</button>{!submitted && <button type="button" className="btn btn-primary" disabled={answered !== questions.length} onClick={submitLevel}>Submit level</button>}{submitted && mastered && levelIndex < 5 && <button type="button" className="btn btn-primary" onClick={() => switchLevel(levelIndex + 1)}>Open next level</button>}</div></footer></div><aside className={styles.reviewPanel} aria-label="Inference review plan"><div><span className={styles.modeTag}>Review plan</span><h3>Repair the unsupported reasoning step</h3><p>{topErrors.length ? 'Your most frequent inference errors appear below.' : 'Complete a level to build a personalised review queue.'}</p></div><ul>{topErrors.length ? topErrors.map(([code, count]) => <li key={code}><strong>{count}</strong><span>{ERROR_LABELS[code]}</span></li>) : <li><strong>0</strong><span>No recorded errors yet</span></li>}</ul><button type="button" className={styles.textButton} onClick={resetAll}><RotateCcw size={16} aria-hidden="true" />{confirmAllReset ? 'Press again to erase progress' : 'Reset all progress'}</button></aside>{questions[0] && <SourceBoundary passage={questions[0].passage} />}<p className={styles.securityNote}>Guided Learn mode delivers answers and explanations to this browser. It is not a secure Practice, Exam or proctored mode.</p></section>;
}
