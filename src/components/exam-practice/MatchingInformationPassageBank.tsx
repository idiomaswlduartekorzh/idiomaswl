import MatchingInformationEngine from '@/components/exam-practice/MatchingInformationEngine';
import type { MatchingInformationPassage } from '@/data/practica-exams/seo-catalog';

export default function MatchingInformationPassageBank({
  passages,
  accent = '#0369a1',
  eyebrow = 'Banco Matching Information',
  title = 'Pasajes para ubicar información específica',
  intro,
}: {
  passages: MatchingInformationPassage[];
  accent?: string;
  eyebrow?: string;
  title?: string;
  intro?: string;
          ...
          <section key={passage.id} style={{ display: 'grid', gap: '0.85rem' }}>
            <article className="wl-card" style={{ padding: '1rem', borderRadius: 8, background: 'var(--bg)', borderLeft: `4px solid ${accent}` }}>
              <p style={{ margin: '0 0 0.35rem', color: accent, fontFamily: 'var(--mono)', fontWeight: 900, fontSize: '0.72rem', textTransform: 'uppercase' }}>
                Set {index + 1} · {passage.paragraphs.length} párrafos · {passage.questions.length} statements
              </p>
            ...
            <MatchingInformationEngine passage={passage} accent={accent} />
          </section>
        ))}
      </div>
    </section>
