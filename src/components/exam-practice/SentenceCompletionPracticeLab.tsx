...
import {
  ...
  type SentenceCompletionDecision,
  type SentenceCompletionErrorCode,
  type SentenceCompletionTrainingPassage,
...
type StoredProgress = {
  ...
  errorCounts: Partial<Record<SentenceCompletionErrorCode, number>>;
  reviewQueue: string[];
...
const ERROR_LABELS: Record<SentenceCompletionErrorCode, string> = {
  ...
  'wrong-evidence': 'nearby true detail answers a different relationship',
  'instruction-misread': 'source or word-limit instruction was not followed',
};
...
function readProgress(): StoredProgress {
  try {
    const raw = window.localStorage.getItem(SENTENCE_COMPLETION_STORAGE_KEY)
      ?? window.localStorage.getItem(SENTENCE_COMPLETION_LEGACY_STORAGE_KEY);
    ...
    const knownLevels = new Set(SENTENCE_COMPLETION_LEVELS.map((level) => level.id));
    ...
    const drafts = parsed.version === 2 && parsed.drafts && typeof parsed.drafts === 'object' && !Array.isArray(parsed.drafts)
      ...
      : {};
    const unlockedLevel = Math.min(SENTENCE_COMPLETION_LEVELS.length - 1, nonNegativeInteger(parsed.unlockedLevel));
    return {
      version: 2,
      unlockedLevel,
...
function writeProgress(progress: StoredProgress) {
  try { window.localStorage.setItem(SENTENCE_COMPLETION_STORAGE_KEY, JSON.stringify(progress)); } catch { /* Local persistence is optional. */ }
}
...
function Passage({ passage }: { passage: SentenceCompletionTrainingPassage }) {
  return (
...
function SourceBoundary({ passage }: { passage: SentenceCompletionTrainingPassage }) {
  return (
...
function AnswerField({ decision, value, disabled, onChange }: {
  decision: SentenceCompletionDecision;
  value: string;
  disabled: boolean;
  onChange: (value: string) => void;
}) {
  const fieldId = useId();
  const count = countSentenceCompletionWords(value);
...
function Feedback({ decision, value }: { decision: SentenceCompletionDecision; value: string }) {
  const correct = isSentenceCompletionCorrect(decision, value);
  return (
...
export function SentenceCompletionGuidedPractice({ passage }: { passage: SentenceCompletionTrainingPassage }) {
  ...
  const correct = isSentenceCompletionCorrect(decision, answer);
  ...
  return (
    <section className={styles.lab} data-active-practice="true" aria-label="Guided Sentence Completion practice">
      ...
      <aside className={styles.workedDecision} aria-labelledby="sentence-completion-worked-example">
        <div><span className={styles.modeTag}>Worked decision</span><h4 id="sentence-completion-worked-example">Predict before you scan</h4><p>Frame: “The archive remains open after ___.” · ONE WORD ONLY</p></div>
        <ol>
          <li><strong>Predict grammar</strong><span>The gap needs a time noun after “after”.</span></li>
          <li><strong>Locate equivalent meaning</strong><span>The passage says the archive is available after sunset.</span></li>
          <li><strong>Copy only the missing span</strong><span>Submit “sunset”, not “after sunset”.</span></li>
        </ol>
      </aside>
      <div className={styles.mapRail} role="progressbar" aria-label="Guided Sentence Completion progress" aria-valuemin={0} aria-valuemax={passage.decisions.length} aria-valuenow={completed.length}>
      ...
      {finished ? (
        ...
        <div className={styles.guidedLayout}>
          ...
          <article className={styles.decisionPanel}>
            <p className={styles.panelLabel}>Sentence {activeIndex + 1} of {passage.decisions.length}</p>
            <h3>Complete the sentence from passage evidence</h3>
            ...
            {checked && <><Feedback decision={decision} value={answer} /><button type="button" className="btn btn-primary" onClick={continuePractice}>{correct ? 'Open next sentence' : 'Retry this sentence'}</button></>}
          </article>
        </div>
      )}
...
export function SentenceCompletionIndependentPractice({ passage }: { passage: SentenceCompletionTrainingPassage }) {
  const [answers, setAnswers] = useState<AnswerMap>({});
  const [submitted, setSubmitted] = useState(false);
  const [confirmReset, setConfirmReset] = useState(false);
  const answered = passage.decisions.filter((decision) => answers[decision.id]?.trim()).length;
  const score = passage.decisions.filter((decision) => isSentenceCompletionCorrect(decision, answers[decision.id] ?? '')).length;
  function reset() {
    if ((answered || submitted) && !confirmReset) { setConfirmReset(true); return; }
    setAnswers({}); setSubmitted(false); setConfirmReset(false);
  }
  return (
    <section className={styles.lab} data-active-practice="true" aria-label="Independent Sentence Completion set">
      <div className={styles.labTopline}><div><span className={styles.modeTag}>Independent full set · feedback closed</span><h3>{passage.title}</h3><p>Complete all six sentences before any key or explanation opens.</p></div><span className={styles.counter} aria-live="polite">{answered}/{passage.decisions.length} complete</span></div>
...
type EngineQuestion = { passage: SentenceCompletionTrainingPassage; decision: SentenceCompletionDecision };
function buildQuestions(levelIndex: number): EngineQuestion[] {
  const level = SENTENCE_COMPLETION_LEVELS[levelIndex];
  const passages = level.passageIds.map(getSentenceCompletionPassage).filter((item): item is SentenceCompletionTrainingPassage => Boolean(item));
...
export function SentenceCompletionProgressEngine() {
  ...
  useEffect(() => {
    const task = window.setTimeout(() => {
      ...
      const level = SENTENCE_COMPLETION_LEVELS[index];
  ...
  const score = questions.filter(({ decision }) => isSentenceCompletionCorrect(decision, answers[decision.id] ?? '')).length;
  const mastered = score >= level.masteryScore;
  const completed = SENTENCE_COMPLETION_LEVELS.filter((item) => progress.levels[item.id]?.mastered).length;
  const singlePassage = new Set(questions.map(({ passage }) => passage.id)).size === 1;
  const topErrors = Object.entries(progress.errorCounts).sort(([, a], [, b]) => (b ?? 0) - (a ?? 0)).slice(0, 3) as Array<[SentenceCompletionErrorCode, number]>;
  ...
  function switchLevel(nextIndex: number) {
    ...
    const target = SENTENCE_COMPLETION_LEVELS[nextIndex];
  ...
  function submitLevel() {
    ...
    const wrong = questions.filter(({ decision }) => !isSentenceCompletionCorrect(decision, answers[decision.id] ?? ''));
    ...
    const next: StoredProgress = {
      version: 2,
      unlockedLevel: mastered ? Math.max(progress.unlockedLevel, Math.min(SENTENCE_COMPLETION_LEVELS.length - 1, levelIndex + 1)) : progress.unlockedLevel,
      ...
      errorCounts: errors,
      reviewQueue: Array.from(new Set([...progress.reviewQueue, ...wrong.map(({ decision }) => decision.id)])).filter((id) => !questions.some(({ decision }) => decision.id === id && isSentenceCompletionCorrect(decision, answers[id] ?? ''))),
      drafts,
    };
  ...
  return (
    <section className={styles.engine} data-active-practice="true" aria-labelledby="sentence-completion-progress-title">
      <div className={styles.engineHeading}><div><span className={styles.modeTag}>WeLearn Progress Engine</span><h2 id="sentence-completion-progress-title">Build exact answers across six levels</h2><p>Two focused drills lead into four full-passage sets. Drafts, time, scores and error patterns stay on this device.</p></div><div className={styles.engineScore}><strong>{completed}/6</strong><span>levels mastered</span></div></div>
      ...
      <nav className={styles.levelRail} aria-label="Sentence Completion progress levels">{SENTENCE_COMPLETION_LEVELS.map((item, index) => { const locked = index > progress.unlockedLevel; const record = progress.levels[item.id]; const total = buildQuestions(index).length; return <button key={item.id} type="button" disabled={locked} aria-current={index === levelIndex ? 'step' : undefined} className={index === levelIndex ? styles.levelCurrent : record?.mastered ? styles.levelMastered : ''} onClick={() => switchLevel(index)}><span>{locked ? <LockKeyhole size={14} aria-hidden="true" /> : index + 1}</span><b>{item.title}</b><small>{record ? `Best ${record.bestScore}/${total}` : locked ? 'Locked' : 'Ready'}</small></button>; })}</nav>
