...
import type { WritingPrompt } from '@/data/practica-exams/seo-catalog';

function countWords(text: string) {
  return text.trim().split(/\s+/).filter(Boolean).length;
}

export default function AcademicDiscussionWorkbench({
  prompts,
  accent = '#1a4fcc',
}: {
  ...
  const wordStatus = words >= 100 && words <= 170 ? 'Listo' : words < 100 ? 'Desarrolla más' : 'Recorta';
      ...
      <div style={{ display: 'grid', gap: '1rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '0.8rem', flexWrap: 'wrap', alignItems: 'start' }}>
          <div>
            ...
            <h2 style={{ margin: 0, fontSize: '1.35rem', letterSpacing: 0 }}>Escribe una respuesta de discusión académica</h2>
          </div>
        ...
        <div style={{ display: 'grid', gap: '0.75rem' }}>
          <article style={{ padding: '1rem', borderRadius: 14, background: `${accent}0d`, border: `1px solid ${accent}25` }}>
            <p style={{ margin: '0 0 0.35rem', color: accent, fontFamily: 'var(--mono)', fontWeight: 900, fontSize: '0.72rem', textTransform: 'uppercase' }}>
              Professor
            </p>
            <p style={{ margin: 0, color: 'var(--ink-2)', lineHeight: 1.65 }}>{prompt.professor}</p>
          </article>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 240px), 1fr))', gap: '0.75rem' }}>
            <article className="wl-card" style={{ padding: '0.9rem', borderRadius: 14 }}>
              ...
              <p style={{ margin: 0, color: 'var(--ink-2)', lineHeight: 1.6, fontSize: '0.92rem' }}>{prompt.studentA}</p>
            </article>
            <article className="wl-card" style={{ padding: '0.9rem', borderRadius: 14 }}>
              <p style={{ margin: '0 0 0.35rem', color: 'var(--muted)', fontFamily: 'var(--mono)', fontWeight: 900, fontSize: '0.72rem', textTransform: 'uppercase' }}>
                Student B
              </p>
              <p style={{ margin: 0, color: 'var(--ink-2)', lineHeight: 1.6, fontSize: '0.92rem' }}>{prompt.studentB}</p>
            </article>
          </div>

          <article style={{ padding: '0.9rem', borderRadius: 14, background: 'var(--bg-2)', border: '1px solid var(--line-soft)' }}>
            <p style={{ margin: '0 0 0.3rem', fontWeight: 800, color: 'var(--ink)' }}>Question</p>
            <p style={{ margin: 0, color: 'var(--muted)', lineHeight: 1.6 }}>{prompt.question}</p>
          </article>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 300px), 1fr))', gap: '1rem', alignItems: 'start' }}>
          <label style={{ display: 'grid', gap: '0.45rem' }}>
            <span style={{ display: 'flex', justifyContent: 'space-between', gap: '0.75rem', flexWrap: 'wrap' }}>
              ...
              <span style={{ fontFamily: 'var(--mono)', fontSize: '0.78rem', color: words >= 100 && words <= 170 ? '#059669' : 'var(--muted)' }}>
            ...
            <textarea
              ...
              placeholder="I find Student A's argument more convincing because..."
              style={{
                width: '100%',
                minHeight: 260,
                resize: 'vertical',
