import DiagramLabelingEngine from '@/components/exam-practice/DiagramLabelingEngine';
import type { DiagramLabelingPassage } from '@/data/practica-exams/seo-catalog';

export default function DiagramLabelingPassageBank({
  passages,
  accent = '#0369a1',
  eyebrow = 'Banco Diagram Labeling',
  title = 'Diagramas para etiquetar con palabras exactas',
  intro,
}: {
  passages: DiagramLabelingPassage[];
  accent?: string;
  eyebrow?: string;
  title?: string;
  intro?: string;
}) {
  const labelCount = passages.reduce((sum, passage) => sum + passage.questions.length, 0);
          ...
          <section key={passage.id} style={{ display: 'grid', gap: '0.85rem' }}>
            <article className="wl-card" style={{ padding: '1rem', borderRadius: 8, background: 'var(--bg)', borderLeft: `4px solid ${accent}` }}>
              <p style={{ margin: '0 0 0.35rem', color: accent, fontFamily: 'var(--mono)', fontWeight: 900, fontSize: '0.72rem', textTransform: 'uppercase' }}>
                Set {index + 1} · {passage.wordLimit} · {passage.stages.length} diagram parts
              </p>
              <h3 style={{ margin: 0, color: 'var(--ink)', fontSize: '1.08rem' }}>{passage.title}</h3>
            </article>

            <DiagramLabelingEngine passage={passage} accent={accent} />
          </section>
        ))}
      </div>
    </section>
