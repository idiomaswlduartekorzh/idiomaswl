import ShortAnswerEngine from '@/components/exam-practice/ShortAnswerEngine';
import type { ShortAnswerPassage } from '@/data/practica-exams/seo-catalog';

export default function ShortAnswerPassageBank({
  passages,
  accent = '#be123c',
  eyebrow = 'Banco Short-answer',
  title = 'Pasajes para responder con evidencia mínima',
  intro,
}: {
  passages: ShortAnswerPassage[];
  accent?: string;
  eyebrow?: string;
  title?: string;
  intro?: string;
          ...
          <section key={passage.id} style={{ display: 'grid', gap: '0.85rem' }}>
            <article className="wl-card" style={{ padding: '1rem', borderRadius: 8, background: 'var(--bg)', borderLeft: `4px solid ${accent}` }}>
              <p style={{ margin: '0 0 0.35rem', color: accent, fontFamily: 'var(--mono)', fontWeight: 900, fontSize: '0.72rem', textTransform: 'uppercase' }}>
                Set {index + 1} · {passage.wordLimit} · {passage.questions.length} questions
              </p>
              <h3 style={{ margin: 0, color: 'var(--ink)', fontSize: '1.08rem' }}>{passage.title}</h3>
            </article>

            <ShortAnswerEngine passage={passage} accent={accent} />
          </section>
        ))}
      </div>
    </section>
