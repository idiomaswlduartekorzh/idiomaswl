...
export default function SummaryCompletionPracticeEngine({
  passage,
  accent = '#0369a1',
}: {
  passage: SummaryCompletionPassage;
  accent?: string;
}) {
  ...
  return (
    <section className="wl-card" style={{ padding: '1rem', borderRadius: 18, borderTop: `4px solid ${accent}` }}>
      ...
      <div style={{ display: 'grid', gap: '0.65rem', marginBottom: '1rem' }}>
        <p style={{ margin: 0, color: 'var(--muted)', lineHeight: 1.6 }}>{passage.summaryIntro}</p>
      ...
      {checkedCount === passage.questions.length && (
        <div style={{ marginTop: '1rem', padding: '1rem', borderRadius: 14, background: `${accent}10`, border: `1px solid ${accent}30` }}>
          ...
          <p style={{ margin: 0, color: 'var(--muted)', lineHeight: 1.6 }}>
            Si una respuesta falló, revisa tres cosas: si copiaste palabras exactas del texto, si respetaste el límite y si la frase encaja gramaticalmente en el resumen.
          </p>
        </div>
      )}
    </section>
