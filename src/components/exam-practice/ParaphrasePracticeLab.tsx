...
import {
  ...
  type ParaphraseDecision,
  type ParaphraseErrorCode,
  type ParaphraseTrainingPassage,
} from '@/data/practica-exams/ielts-reading-paraphrase-progress';
...
type StoredProgress = {
  ...
  errorCounts: Partial<Record<ParaphraseErrorCode, number>>;
  reviewQueue: string[];
...
const ERROR_LABELS: Record<ParaphraseErrorCode, string> = {
  'entity-shift': 'attached an action to the wrong person or thing',
  'scope-shift': 'changed who, how many or how much the claim covers',
  'certainty-shift': 'made a possibility stronger or weaker than the source',
  'polarity-shift': 'missed a negative or reversed the claim',
  'cause-or-sequence-shift': 'changed cause, purpose or event order',
  'comparison-shift': 'reversed or distorted a comparison',
};
...
function normalizeProgress(value: unknown): StoredProgress {
  ...
  const knownLevels = new Set(PARAPHRASE_LEVELS.map((level) => level.id));
  ...
  const unlockedLevel = Math.min(PARAPHRASE_LEVELS.length - 1, nonNegativeInteger(parsed.unlockedLevel));
...
function readProgress() {
  try { return normalizeProgress(JSON.parse(window.localStorage.getItem(PARAPHRASE_STORAGE_KEY) ?? window.localStorage.getItem(PARAPHRASE_LEGACY_STORAGE_KEY) ?? 'null')); }
  catch { return EMPTY_PROGRESS; }
}
function writeProgress(progress: StoredProgress) { try { window.localStorage.setItem(PARAPHRASE_STORAGE_KEY, JSON.stringify(progress)); } catch { /* The practice remains usable without storage. */ } }
...
function ChoiceCards({ options, selected, disabled, label, onSelect }: { options: string[]; selected: string; disabled: boolean; label: string; onSelect: (value: string) => void }) {
  const group = `para-${useId().replace(/:/g, '')}`;
...
function Passage({ passage }: { passage: ParaphraseTrainingPassage }) {
...
function SourceBoundary({ passage }: { passage: ParaphraseTrainingPassage }) {
...
function DecisionCard({ decision, selected, disabled, attemptSeed, showInvariant = false, onSelect }: { decision: ParaphraseDecision; selected: string; disabled: boolean; attemptSeed: number; showInvariant?: boolean; onSelect: (value: string) => void }) {
  return <><div className={styles.quoteBlock}><p className={styles.panelLabel}>Source claim</p><blockquote>{decision.source}</blockquote>{showInvariant && <p><strong>Meaning controls:</strong> {decision.invariant}</p>}</div><ChoiceCards options={getParaphraseOptions(decision, attemptSeed)} selected={selected} disabled={disabled} label={`Equivalent meaning for ${decision.source}`} onSelect={onSelect} /></>;
}

export function ParaphraseGuidedPractice({ passage }: { passage: ParaphraseTrainingPassage }) {
  ...
  return <section className={styles.lab} data-active-practice="true" aria-label="Guided paraphrase practice"><div className={styles.labTopline}><div><span className={styles.modeTag}>Guided practice · three comparisons</span><h3>{passage.title}</h3><p>Read the full claim, protect every meaning control, then choose the equivalent proposition.</p></div><button type="button" className="btn btn-ghost" onClick={reset}><RotateCcw size={16} aria-hidden="true" />{confirmReset ? 'Press again to reset' : 'Reset guided set'}</button></div><Passage passage={passage} /><div className={styles.engineQuestions}>{decisions.map((decision, index) => { const done = Boolean(checked[decision.id]); const correct = answers[decision.id] === decision.answer; return <article className={styles.engineQuestion} key={decision.id}><div className={styles.questionMeta}><span>{String(index + 1).padStart(2, '0')}</span><small>Protect → compare → reject</small></div><h3>Which option preserves the complete meaning?</h3><DecisionCard decision={decision} selected={answers[decision.id] ?? ''} disabled={done} attemptSeed={attemptSeed} showInvariant onSelect={(value) => { setAnswers((current) => ({ ...current, [decision.id]: value })); setConfirmReset(false); }} />{!done && <button type="button" className="btn btn-primary" disabled={!answers[decision.id]} onClick={() => setChecked((current) => ({ ...current, [decision.id]: true }))}>Check meaning</button>}{done && <div className={`${styles.feedback} ${correct ? styles.feedbackCorrect : styles.feedbackIncorrect}`} role="status">{correct ? <CheckCircle2 size={18} aria-hidden="true" /> : <XCircle size={18} aria-hidden="true" />}<div><strong>{correct ? 'Meaning preserved.' : `Equivalent claim: ${decision.answer}`}</strong><p>{correct ? decision.explanation : decision.trap}</p></div></div>}</article>; })}</div><SourceBoundary passage={passage} /></section>;
}

export function ParaphraseIndependentPractice({ passage }: { passage: ParaphraseTrainingPassage }) {
  const [answers, setAnswers] = useState<AnswerMap>({});
  ...
  return <section className={styles.lab} data-active-practice="true" aria-label="Independent paraphrase set"><div className={styles.labTopline}><div><span className={styles.modeTag}>Full passage · feedback closed</span><h3>{passage.title}</h3><p>Complete all five comparisons before feedback opens. The answer position changes on every clean retry.</p></div><span className={styles.counter} aria-live="polite">{answered}/5 compared</span></div><Passage passage={passage} /><div className={styles.independentGrid}>{passage.decisions.map((decision) => { const correct = answers[decision.id] === decision.answer; return <article className={styles.independentCard} key={decision.id}><p className={styles.panelLabel}>Meaning comparison</p><h3>Which option makes the same claim?</h3><DecisionCard decision={decision} selected={answers[decision.id] ?? ''} disabled={submitted} attemptSeed={attemptSeed} onSelect={(value) => { setAnswers((current) => ({ ...current, [decision.id]: value })); setConfirmReset(false); }} />{submitted && <div className={`${styles.feedback} ${correct ? styles.feedbackCorrect : styles.feedbackIncorrect}`} role="status">{correct ? <CheckCircle2 size={18} aria-hidden="true" /> : <XCircle size={18} aria-hidden="true" />}<div><strong>{correct ? 'Equivalent meaning.' : `Equivalent claim: ${decision.answer}`}</strong><p>{correct ? decision.explanation : decision.trap}</p></div></div>}</article>; })}</div><div className={styles.submitBar} aria-live="polite"><div>{submitted ? <><strong>{score}/5 correct</strong><span>{score >= 4 ? 'Independent transfer target reached.' : 'Review each changed proposition slot, then retry.'}</span></> : <><strong>One complete submission</strong><span>Feedback stays closed so every decision remains independent.</span></>}</div><div className={styles.actions}><button type="button" className="btn btn-ghost" onClick={reset}><RotateCcw size={16} aria-hidden="true" />{confirmReset ? 'Press again to reset' : 'Reset set'}</button>{!submitted && <button type="button" className="btn btn-primary" disabled={answered !== 5} onClick={() => setSubmitted(true)}>Submit comparison set</button>}</div></div><SourceBoundary passage={passage} /></section>;
}

type EngineQuestion = { passage: ParaphraseTrainingPassage; decision: ParaphraseDecision };
function buildQuestions(levelIndex: number): EngineQuestion[] {
  const level = PARAPHRASE_LEVELS[levelIndex];
  const passages = level.passageIds.map(getParaphrasePassage).filter((item): item is ParaphraseTrainingPassage => Boolean(item));
...
export function ParaphraseProgressEngine() {
  ...
  const completed = PARAPHRASE_LEVELS.filter((item) => progress.levels[item.id]?.mastered).length;
  const topErrors = Object.entries(progress.errorCounts).sort(([, a], [, b]) => (b ?? 0) - (a ?? 0)).slice(0, 3) as Array<[ParaphraseErrorCode, number]>;
