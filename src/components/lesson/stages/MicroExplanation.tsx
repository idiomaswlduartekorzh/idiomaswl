...
const CORE_BLOCKS: CoreBlock[] = [
  { korean: '저는', role: 'ejecutor', label: '¿Quién?', translation: 'yo (formal)', color: 'var(--wl-on-panel-link, #6c63ff)' },
  { korean: '학교에', role: 'complemento', label: '¿Dónde?', translation: 'a la escuela', color: '#e6930a' },
  { korean: '가요', role: 'verbo', label: '¿Qué pasa?', translation: 'voy — acción al final', color: 'var(--wl-on-panel-ok, #2d9b4e)' },
];
...
function HighlightLine({ text, highlight }: { text: string; highlight: string }) {
  ...
  return (
    <span>
      {before}
      <strong style={{ color: 'var(--wl-on-panel-ok, #2d9b4e)' }}>{mid}</strong>
      {after}
    </span>
  );
}
    ...
    return (
      <div key="complement" style={{ animation: 'fadeIn 0.3s ease' }}>
        ...
        <article style={{ background: 'var(--wl-surface-card)', border: '1px solid #e9ecef', borderRadius: 16, padding: 20, marginBottom: 20, overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 10 }}>
            ...
            <div style={{ background: 'rgba(45,155,78,0.1)', border: '2px solid #2d9b4e', borderRadius: 10, padding: '12px 16px', textAlign: 'center' }}>
              <p style={{ margin: '0 0 4px', fontSize: 9, color: 'var(--wl-on-panel-ok, #2d9b4e)', textTransform: 'uppercase', letterSpacing: '0.12em', fontWeight: 700 }}>
                VERBO · siempre al final
              </p>
              <p style={{ margin: 0, fontFamily: "'Noto Sans KR', sans-serif", fontSize: 20, fontWeight: 700, color: 'var(--wl-on-panel-ok, #2d9b4e)' }}>가요</p>
              <button
                type="button"
                onClick={() => playAudio('가요', 1)}
                style={{ marginTop: 6, border: 'none', background: 'transparent', fontSize: 13, color: 'var(--wl-on-panel-ok, #2d9b4e)', cursor: 'pointer' }}
              >
                🔊
              </button>
            </div>
        ...
        {unavailablePieces.map((piece) => (
          <article key={piece.id} style={{ background: 'var(--wl-panel-raised, #f8f9fa)', border: '1px dashed #e9ecef', borderRadius: 12, padding: 12, marginBottom: 8, opacity: 0.7, display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 16 }}>🔒</span>
            <p style={{ margin: 0, fontSize: 12, color: '#adb5bd', flex: 1 }}>{piece.name} — {piece.note}</p>
            <span style={{ fontSize: 9, color: '#adb5bd', background: 'var(--wl-panel-raised, #e9ecef)', borderRadius: 100, padding: '2px 8px' }}>Step próximo</span>
          </article>
        ))}
      ...
      <div key="complete" style={{ animation: 'fadeIn 0.3s ease' }}>
        ...
        <article style={{ background: 'var(--wl-surface-card)', border: '1px solid #e9ecef', borderRadius: 14, padding: 16, marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, justifyContent: 'center', fontSize: 11, color: 'var(--wl-ink-soft)' }}>
            ...
            <span>→</span>
            <span style={{ padding: '3px 8px', borderRadius: 100, background: 'var(--wl-panel-raised, #f8f9fa)' }}>objeto</span>
            <span>→</span>
            <span style={{ padding: '3px 8px', borderRadius: 100, background: 'var(--wl-panel-raised, #f8f9fa)' }}>manera</span>
            <span>→</span>
            <span style={{ padding: '3px 8px', borderRadius: 100, background: 'rgba(45,155,78,0.1)', color: 'var(--wl-on-panel-ok, #2d9b4e)' }}>VERBO</span>
          </div>
        </article>
