...
export default function ReactiveInteraction({ onComplete }: Props) {
  ...
  if (phase === 'intro') {
    return (
      <section style={{ maxWidth: 680, margin: '0 auto', padding: '2rem 1rem', animation: 'fadeIn 0.3s ease' }}>
        ...
        <h3 style={{ margin: '0 0 4px', fontSize: 24, fontWeight: 700, color: 'var(--wl-ink)' }}>Interacción reactiva</h3>
        <p style={{ margin: '0 0 24px', fontSize: 13, color: 'var(--wl-ink-soft)', lineHeight: 1.7 }}>
        ...
        <div style={{ marginBottom: 18 }}>
          {[
            ...
            <article key={rule.text} style={{ display: 'flex', gap: 10, alignItems: 'center', background: 'var(--wl-surface-card)', border: '1px solid #e9ecef', borderRadius: 10, padding: '10px 14px', marginBottom: 8 }}>
              <span style={{ fontSize: 16 }}>{rule.icon}</span>
              <p style={{ margin: 0, fontSize: 13, color: 'var(--wl-ink)' }}>{rule.text}</p>
            </article>
          ))}
        </div>

        <p style={{ margin: '0 0 20px', textAlign: 'center', fontSize: 12, color: 'var(--wl-ink-soft)' }}>5 conversaciones</p>
  ...
  if (phase === 'scenarios' && scenario) {
    return (
      <section key={`scenario-${scenario.id}`} style={{ maxWidth: 680, margin: '0 auto', padding: '2rem 1rem', animation: 'fadeIn 0.3s ease' }}>
        ...
        <p style={{ margin: '0 0 4px', fontSize: 11, color: 'var(--wl-ink-soft)', textAlign: 'right' }}>{`${currentScenario + 1} de ${SCENARIOS.length}`}</p>
        ...
        <article style={{ background: 'var(--wl-surface-card)', border: '1px solid #e9ecef', borderRadius: 16, overflow: 'hidden', marginBottom: 16 }}>
            ...
            {!scenario.isYouAsk ? (
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 4 }}>
                ...
                <div style={{ background: 'var(--wl-panel-raised, #f8f9fa)', borderRadius: '0 12px 12px 12px', padding: '10px 14px', flex: 1 }}>
                  <p style={{ margin: '0 0 2px', fontFamily: "'Noto Sans KR', sans-serif", fontSize: 22, fontWeight: 700, color: 'var(--wl-ink)' }}>
                    {scenario.question}
                  ...
                  <p style={{ margin: 0, fontSize: 11, color: 'var(--wl-ink-soft)' }}>{scenario.questionTranslation}</p>
                </div>
              </div>
            ) : (
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, flexDirection: 'row-reverse', marginBottom: 4 }}>
                ...
                <div style={{ background: 'rgba(108,99,255,0.08)', border: '1px solid rgba(108,99,255,0.15)', borderRadius: '12px 0 12px 12px', padding: '10px 14px' }}>
                  <p style={{ margin: 0, fontSize: 14, color: 'var(--wl-ink)', fontWeight: 500 }}>{scenario.question}</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div style={{ padding: '12px 16px 16px' }}>
            <p style={{ margin: '0 0 10px', fontSize: 10, color: 'var(--wl-ink-soft)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>Tu respuesta:</p>
                  ...
                  <div key={`${scenario.id}-${option.text}`} style={{ marginBottom: 8 }}>
                    ...
                    >
                      <button
                        ...
                        disabled={isChecked}
                      >
                        <span style={{ fontSize: 12, color: 'var(--wl-ink-soft)' }}>→</span>
                      ...
                      {!scenario.isYouAsk ? (
                        <button
                          ...
                          style={{ border: 'none', background: 'transparent', cursor: 'pointer', fontSize: 14, color: 'var(--wl-ink-soft)' }}
                        >
                          🔊
                        </button>
                      ) : null}
              ...
              <div style={{ background: isAnswerCorrect ? 'rgba(45,155,78,0.06)' : 'rgba(220,53,69,0.05)', border: `1px solid ${isAnswerCorrect ? 'rgba(45,155,78,0.2)' : 'rgba(220,53,69,0.15)'}`, borderRadius: 12, padding: 16, marginBottom: 12, animation: 'fadeIn 0.3s ease' }}>
                {isAnswerCorrect ? (
                  <>
                    ...
                    <p style={{ margin: '0 0 10px', fontSize: 13, color: 'var(--wl-ink)', lineHeight: 1.6 }}>{scenario.explanation}</p>
                    ...
                    <p style={{ margin: '0 0 8px', fontSize: 12, color: 'var(--wl-ink-soft)' }}>{scenario.correctTranslation}</p>
                    <button
                      type="button"
                  ...
                  <>
                    ...
                    <div style={{ background: 'var(--wl-surface-card)', border: '1px solid #e9ecef', borderRadius: 8, padding: '8px 12px', marginBottom: 8 }}>
                      <p style={{ margin: '0 0 4px', fontSize: 10, color: 'var(--wl-ink-soft)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>Por qué no:</p>
                    ...
                    <p style={{ margin: '0 0 4px', fontSize: 10, color: 'var(--wl-ink-soft)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>La respuesta completa es:</p>
                    <p style={{ margin: '0 0 2px', fontFamily: "'Noto Sans KR', sans-serif", fontSize: 20, color: 'var(--wl-on-panel-ok, #2d9b4e)', fontWeight: 700 }}>{correctOption}</p>
                    <p style={{ margin: '0 0 8px', fontSize: 12, color: 'var(--wl-ink-soft)' }}>{scenario.correctTranslation}</p>
                    <button
                      type="button"
  ...
  return (
    <section style={{ maxWidth: 560, margin: '0 auto', textAlign: 'center', padding: '2rem 1rem', animation: 'fadeIn 0.3s ease' }}>
      <style>{`@keyframes fadeIn { from { opacity: 0; transform: translateY(12px); } to { opacity: 1; transform: translateY(0); } }`}</style>
      <article style={{ background: 'var(--wl-surface-card)', border: '1px solid #e9ecef', borderRadius: 20, padding: '2.5rem 2rem', marginBottom: 20 }}>
        <div
          style={{
            width: 100,
            height: 100,
        ...
        >
          <span style={{ width: 74, height: 74, borderRadius: '50%', background: 'var(--wl-surface-card)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, fontWeight: 700, color: 'var(--wl-on-panel-link, #6c63ff)' }}>
            {`${score}/5`}
          </span>
        </div>
        <p style={{ margin: '0 0 4px', fontSize: 16, fontWeight: 600, color: 'var(--wl-ink)' }}>{scoreMessage(score)}</p>
        <p style={{ margin: 0, fontSize: 13, color: 'var(--wl-ink-soft)', lineHeight: 1.6 }}>
      ...
      <article style={{ background: 'rgba(108,99,255,0.06)', border: '1px solid rgba(108,99,255,0.15)', borderRadius: 12, padding: '14px 16px', marginBottom: 20, textAlign: 'left' }}>
        ...
        <div>
          {[
            ...
            <p key={item} style={{ margin: '0 0 4px', fontSize: 12, color: 'var(--wl-ink)', display: 'flex', gap: 8 }}>
      ...
      <div style={{ marginBottom: 10 }}>
        {SCENARIOS.map((item) => {
          ...
          return (
            <div key={item.id} style={{ background: 'var(--wl-surface-card)', border: '1px solid #e9ecef', borderRadius: 10, padding: '10px 14px', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ fontSize: 18 }}>{item.speakerAvatar}</span>
              <div style={{ flex: 1, textAlign: 'left' }}>
                <p style={{ margin: '0 0 2px', fontSize: 10, color: 'var(--wl-ink-soft)' }}>{item.contextES}</p>
