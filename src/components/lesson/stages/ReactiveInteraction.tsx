...
export default function ReactiveInteraction({ onComplete }: Props) {
  ...
  if (phase === 'intro') {
    return (
      <section style={{ maxWidth: 680, margin: '0 auto', padding: '2rem 1rem', animation: 'fadeIn 0.3s ease' }}>
        ...
        <h3 style={{ margin: '0 0 4px', fontSize: 24, fontWeight: 700, color: '#1a1a2e' }}>Interacción reactiva</h3>
        <p style={{ margin: '0 0 24px', fontSize: 13, color: '#6c757d', lineHeight: 1.7 }}>
        ...
        <div style={{ marginBottom: 18 }}>
          {[
            ...
            <article key={rule.text} style={{ display: 'flex', gap: 10, alignItems: 'center', background: '#fff', border: '1px solid #e9ecef', borderRadius: 10, padding: '10px 14px', marginBottom: 8 }}>
              <span style={{ fontSize: 16 }}>{rule.icon}</span>
              <p style={{ margin: 0, fontSize: 13, color: '#1a1a2e' }}>{rule.text}</p>
            </article>
          ))}
        </div>

        <p style={{ margin: '0 0 20px', textAlign: 'center', fontSize: 12, color: '#6c757d' }}>5 conversaciones</p>
  ...
  if (phase === 'scenarios' && scenario) {
    return (
      <section key={`scenario-${scenario.id}`} style={{ maxWidth: 680, margin: '0 auto', padding: '2rem 1rem', animation: 'fadeIn 0.3s ease' }}>
        ...
        <p style={{ margin: '0 0 4px', fontSize: 11, color: '#6c757d', textAlign: 'right' }}>{`${currentScenario + 1} de ${SCENARIOS.length}`}</p>
        ...
        <article style={{ background: '#fff', border: '1px solid #e9ecef', borderRadius: 16, overflow: 'hidden', marginBottom: 16 }}>
          ...
          <div style={{ padding: '16px 16px 0' }}>
            {!scenario.isYouAsk ? (
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 4 }}>
                <span style={{ width: 36, height: 36, borderRadius: '50%', background: '#f1f3f5', fontSize: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  {scenario.speakerAvatar}
                </span>
                <div style={{ background: '#f8f9fa', borderRadius: '0 12px 12px 12px', padding: '10px 14px', flex: 1 }}>
                  <p style={{ margin: '0 0 2px', fontFamily: "'Noto Sans KR', sans-serif", fontSize: 22, fontWeight: 700, color: '#1a1a2e' }}>
                    {scenario.question}
                  ...
                  <p style={{ margin: 0, fontSize: 11, color: '#6c757d' }}>{scenario.questionTranslation}</p>
                </div>
              </div>
            ) : (
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, flexDirection: 'row-reverse', marginBottom: 4 }}>
                ...
                <div style={{ background: 'rgba(108,99,255,0.08)', border: '1px solid rgba(108,99,255,0.15)', borderRadius: '12px 0 12px 12px', padding: '10px 14px' }}>
                  <p style={{ margin: 0, fontSize: 14, color: '#1a1a2e', fontWeight: 500 }}>{scenario.question}</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div style={{ padding: '12px 16px 16px' }}>
            <p style={{ margin: '0 0 10px', fontSize: 10, color: '#6c757d', textTransform: 'uppercase', letterSpacing: '0.12em' }}>Tu respuesta:</p>
                    ...
                    >
                      ...
                      {!scenario.isYouAsk ? (
                        <button
                          ...
                          style={{ border: 'none', background: 'transparent', cursor: 'pointer', fontSize: 14, color: '#6c757d' }}
                        >
                          🔊
                        </button>
                      ) : null}
                      {correct ? <span style={{ color: '#2d9b4e', fontWeight: 700 }}>✓</span> : null}
                      {wrongSelected ? <span style={{ color: '#dc3545', fontWeight: 700 }}>✗</span> : null}
                    </div>
                  </div>
                );
              })}
            ...
            {isChecked ? (
              <div style={{ background: isAnswerCorrect ? 'rgba(45,155,78,0.06)' : 'rgba(220,53,69,0.05)', border: `1px solid ${isAnswerCorrect ? 'rgba(45,155,78,0.2)' : 'rgba(220,53,69,0.15)'}`, borderRadius: 12, padding: 16, marginBottom: 12, animation: 'fadeIn 0.3s ease' }}>
                {isAnswerCorrect ? (
                  <>
                    <p style={{ margin: '0 0 6px', fontSize: 14, fontWeight: 700, color: '#2d9b4e' }}>✅ ¡Correcto!</p>
                    <p style={{ margin: '0 0 10px', fontSize: 13, color: '#1a1a2e', lineHeight: 1.6 }}>{scenario.explanation}</p>
                    <p style={{ margin: '0 0 2px', fontFamily: "'Noto Sans KR', sans-serif", fontSize: 20, color: '#2d9b4e', fontWeight: 700 }}>{correctOption}</p>
                    <p style={{ margin: '0 0 8px', fontSize: 12, color: '#6c757d' }}>{scenario.correctTranslation}</p>
                    <button
                      type="button"
                      onClick={() => playAudio(scenario.correctAudio, 1)}
                      style={{ background: 'rgba(45,155,78,0.1)', border: '1px solid rgba(45,155,78,0.2)', borderRadius: 100, padding: '4px 12px', fontSize: 11, color: '#2d9b4e', cursor: 'pointer' }}
                    >
                      🔊 Escuchar de nuevo
                    </button>
                  </>
                  ...
                  <>
                    <p style={{ margin: '0 0 6px', fontSize: 14, fontWeight: 700, color: '#dc3545' }}>❌ No exactamente</p>
                    <div style={{ background: '#fff', border: '1px solid #e9ecef', borderRadius: 8, padding: '8px 12px', marginBottom: 8 }}>
                      <p style={{ margin: '0 0 4px', fontSize: 10, color: '#6c757d', textTransform: 'uppercase', letterSpacing: '0.12em' }}>Por qué no:</p>
                      <p style={{ margin: 0, fontSize: 12, color: '#dc3545' }}>{selectedOption ? (scenario.whyWrong[selectedOption] ?? 'No es la forma más funcional en este contexto.') : ''}</p>
                    </div>
                    <p style={{ margin: '0 0 4px', fontSize: 10, color: '#6c757d', textTransform: 'uppercase', letterSpacing: '0.12em' }}>La respuesta completa es:</p>
                    <p style={{ margin: '0 0 2px', fontFamily: "'Noto Sans KR', sans-serif", fontSize: 20, color: '#2d9b4e', fontWeight: 700 }}>{correctOption}</p>
                    <p style={{ margin: '0 0 8px', fontSize: 12, color: '#6c757d' }}>{scenario.correctTranslation}</p>
                    <button
                      type="button"
  ...
  return (
    <section style={{ maxWidth: 560, margin: '0 auto', textAlign: 'center', padding: '2rem 1rem', animation: 'fadeIn 0.3s ease' }}>
      <style>{`@keyframes fadeIn { from { opacity: 0; transform: translateY(12px); } to { opacity: 1; transform: translateY(0); } }`}</style>
      <article style={{ background: '#fff', border: '1px solid #e9ecef', borderRadius: 20, padding: '2.5rem 2rem', marginBottom: 20 }}>
        <div
          style={{
            width: 100,
            height: 100,
        ...
        >
          <span style={{ width: 74, height: 74, borderRadius: '50%', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, fontWeight: 700, color: '#6c63ff' }}>
            {`${score}/5`}
          </span>
        </div>
        <p style={{ margin: '0 0 4px', fontSize: 16, fontWeight: 600, color: '#1a1a2e' }}>{scoreMessage(score)}</p>
      ...
      <article style={{ background: 'rgba(108,99,255,0.06)', border: '1px solid rgba(108,99,255,0.15)', borderRadius: 12, padding: '14px 16px', marginBottom: 20, textAlign: 'left' }}>
        <p style={{ margin: '0 0 8px', fontSize: 11, color: '#6c63ff', textTransform: 'uppercase', letterSpacing: '0.12em' }}>💡 Lo que practicaste:</p>
        <div>
          {[
            ...
            <p key={item} style={{ margin: '0 0 4px', fontSize: 12, color: '#1a1a2e', display: 'flex', gap: 8 }}>
              <span style={{ color: '#2d9b4e' }}>✓</span>
              <span>{item}</span>
            </p>
          ))}
        </div>
      ...
      <div style={{ marginBottom: 10 }}>
        {SCENARIOS.map((item) => {
          ...
          return (
            <div key={item.id} style={{ background: '#fff', border: '1px solid #e9ecef', borderRadius: 10, padding: '10px 14px', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ fontSize: 18 }}>{item.speakerAvatar}</span>
              <div style={{ flex: 1, textAlign: 'left' }}>
                <p style={{ margin: '0 0 2px', fontSize: 10, color: '#6c757d' }}>{item.contextES}</p>
                <p style={{ margin: 0, fontFamily: "'Noto Sans KR', sans-serif", fontSize: 14, color: '#2d9b4e' }}>{answer}</p>
              </div>
              <button
                type="button"
                onClick={() => playAudio(item.correctAudio, 1)}
                style={{ border: 'none', background: 'transparent', fontSize: 14, color: '#2d9b4e', cursor: 'pointer' }}
              >
                🔊
              </button>
            </div>
      ...
      <p style={{ margin: '12px 0 0', fontSize: 11, color: '#6c63ff', textAlign: 'center' }}>
