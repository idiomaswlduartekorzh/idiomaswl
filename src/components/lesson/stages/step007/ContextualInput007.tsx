...
import { KR_VIDEO_007, playAudio } from '@/lib/storage';
    ...
    bubbles: [
      {
        ...
        words: [
          ...
          { kr: '어때요?', rom: 'eo-ttae-yo', es: '¿qué tal? (del step006)' },
        ],
      },
      {
    ...
    bubbles: [
      {
        ...
        note: '저도 — Minsu usa la partícula 도 (también) que ya aprendiste en step006. ¡El reciclaje funciona!',
        words: [
          { kr: '어', rom: 'eo', es: 'eh (pausa)' },
          { kr: '저도', rom: 'jeo-do', es: 'yo también (도 = también, del step006)' },
      ...
      },
      {
        ...
        audio: '친구들이 많아요',
        note: '들 añade plural a cualquier sustantivo. 친구 (un amigo) → 친구들 (amigos). También conoces 사람들 del step006.',
        words: [
          { kr: '친구들', rom: 'chin-gu-deul', es: 'amigos (들=plural)' },
          { kr: '이', rom: 'i', es: 'partícula de sujeto' },
          { kr: '많아요', rom: 'ma-na-yo', es: 'hay muchos (del step006)' },
        ],
      },
      {
...
function WordChip({ w }: { w: Word }) {
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:2, padding:'8px 10px', borderRadius:10, background:'rgba(108,99,255,0.06)', border:'1px solid rgba(108,99,255,0.18)' }}>
      ...
      <span style={{ fontSize:10, color:'var(--wl-on-panel-link, #a78bfa)', fontFamily:'monospace' }}>{w.rom}</span>
    ...
    return (
      <div style={{ fontFamily:'system-ui,-apple-system,"Segoe UI",sans-serif', color:'var(--foreground)', padding:'20px 16px', display:'flex', flexDirection:'column', gap:16 }}>
        <style>{`
          ...
          @keyframes ci5-bubble { from{opacity:0;transform:translateY(14px) scale(0.97)} to{opacity:1;transform:none} }
        `}</style>

        <div style={{ animation:'ci5-in 0.4s ease both' }}>
        ...
        <div style={{ borderRadius:16, overflow:'hidden', background:'#000', animation:'ci5-in 0.4s 0.1s ease both' }}>
          <video
            src={scene.videoSrc}
            controls
            playsInline
        ...
        <div style={{ padding:'12px 14px', borderRadius:12, background:'rgba(108,99,255,0.06)', border:'1px solid rgba(108,99,255,0.2)', animation:'ci5-in 0.4s 0.15s ease both' }}>
        ...
        <button
          ...
          style={{
            ...
            animation:'ci5-in 0.4s 0.2s ease both',
          }}
        >
