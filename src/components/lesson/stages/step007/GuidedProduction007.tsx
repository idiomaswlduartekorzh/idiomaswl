...
const EXERCISES: Exercise[] = [
  {
    type: 'build',
    hint: 'Yo estudio en esta universidad',
    tiles: [
      { ko: '이 대학교에서', role: 'subject' },
      { ko: '공부해요',      role: 'verb'    },
    ],
    correctOrder: ['이 대학교에서', '공부해요'],
    audio: '이 대학교에서 공부해요',
  },
  {
    type: 'build',
    hint: 'Trabajo en el café',
    tiles: [
      { ko: '카페에서', role: 'subject' },
      { ko: '일해요',   role: 'verb'    },
    ],
    correctOrder: ['카페에서', '일해요'],
    audio: '카페에서 일해요',
  },
  {
    type: 'build',
    hint: 'Voy al café todos los días',
    tiles: [
      { ko: '매일',   role: 'subject' },
      { ko: '카페에', role: 'object'  },
      { ko: '가요',   role: 'verb'    },
    ],
    correctOrder: ['매일', '카페에', '가요'],
    audio: '매일 카페에 가요',
  },
  {
    type: 'choice',
    prompt: 'Quieres decir "Estudio en la biblioteca". ¿Qué partícula usas?',
    options: ['도서관에 공부해요', '도서관에서 공부해요', '도서관이 공부해요'],
    correct: '도서관에서 공부해요',
    feedbackCorrect: '¡Correcto! 에서 porque la acción (공부) ocurre EN la biblioteca.',
  },
  {
    type: 'choice',
    prompt: 'Minsu te pregunta "뭐 좋아해요?" Respondes que te gusta el café.',
    options: ['커피 좋아요', '커피가 있어요', '커피 좋아해요'],
    correct: '커피 좋아해요',
    feedbackCorrect: '¡Exacto! 좋아해요 = verbo de preferencia. No 좋아요.',
  },
  {
    type: 'choice',
    prompt: 'Quieres decir que vas a la universidad (no que estudias allí).',
    options: ['대학교에서 가요', '대학교에 가요', '대학교가 가요'],
    correct: '대학교에 가요',
    feedbackCorrect: '¡Bien! Con 가요 (ir) usas 에, no 에서.',
  },
];

const ACCENT = '#6c63ff';
const MAX_STARS = 18; // 6 × 3
      ...
      {[1, 2, 3].map(i => (
        <span key={i} style={{
          ...
          animation: animate && i <= count
            ? `gp5-star-pop 0.4s cubic-bezier(0.34,1.56,0.64,1) ${(i - 1) * 90}ms both`
            : 'none',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {chosenTiles.map((t, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              animation: 'gp5-pop 0.25s cubic-bezier(0.34,1.56,0.64,1) both',
            }}>
              {t.ko}
            </div>
          );
      ...
      {status === 'correct' && stars !== null && (
        <div style={{ padding: '12px 14px', borderRadius: 12, background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.3)', animation: 'gp5-in 0.3s ease both', display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
        {shuffledPool.map(t => {
          ...
          const tc = TILE_COLORS[t.role];
          return (
            <button key={t.ko} type="button" disabled={used || status !== 'building'} onClick={() => addTile(t.ko)}
              style={{
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        {status === 'building' && chosen.length > 0 && (
          <button type="button" onClick={() => setChosen(prev => prev.slice(0, -1))}
            style={{ padding: '10px 14px', borderRadius: 12, cursor: 'pointer', background: 'var(--secondary)', border: '1px solid var(--border)', fontSize: 12, fontWeight: 600, color: 'var(--muted-foreground)', flex: '0 0 auto' }}>
            ← Borrar última
          </button>
        )}
        {status === 'wrong' && (
        ...
        {status === 'building' && complete && (
          <button type="button" onClick={check}
            style={{ flex: 1, padding: '11px', borderRadius: 12, cursor: 'pointer', background: 'rgba(34,197,94,0.12)', border: '1px solid rgba(34,197,94,0.38)', fontSize: 13, fontWeight: 700, color: '#22c55e' }}>
            ✓ Comprobar
          </button>
        )}
        {status === 'correct' && (
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {picked && (
        <>
          <div style={{
            padding: '12px 15px', borderRadius: 12, animation: 'gp5-in 0.3s ease both',
          ...
          <button type="button" onClick={onNext}
            style={{ padding: '12px', borderRadius: 12, cursor: 'pointer', width: '100%', background: 'rgba(34,197,94,0.12)', border: '1px solid rgba(34,197,94,0.35)', fontSize: 13, fontWeight: 700, color: '#22c55e' }}>
            Siguiente →
          </button>
        </>
      )}
...
/* ─── Main component ─────────────────────────────────────────────────────── */
export default function GuidedProduction007({ onComplete }: Props) {
  ...
  if (done) {
    ...
    return (
      <div style={{ ...base, padding: '32px 20px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}>
        <style>{`
          @keyframes gp5-star-pop { from{opacity:0;transform:scale(0)} 70%{transform:scale(1.3)} to{opacity:1;transform:scale(1)} }
          @keyframes gp5-in  { from{opacity:0;transform:translateY(8px)} to{opacity:1;transform:none} }
          @keyframes gp5-pop { from{opacity:0;transform:scale(0.85)} to{opacity:1;transform:scale(1)} }
        `}</style>
  ...
  return (
    <div style={{ ...base, padding: '20px 16px', display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div key={exIdx} style={{
        ...
        animation: 'gp5-pop 0.4s cubic-bezier(0.34,1.56,0.64,1) both',
      }}>
        {ex.type === 'build' ? (
