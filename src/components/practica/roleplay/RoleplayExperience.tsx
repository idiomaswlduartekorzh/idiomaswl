...
function rolePath(scenario: RoleplayScenario, role: RoleId) {
  return `/practica/${scenario.language}/${scenario.level}/habla/acompanada/${scenario.slug}/${role}`
}

function scenarioPath(scenario: RoleplayScenario) {
  return `/practica/${scenario.language}/${scenario.level}/habla/acompanada/${scenario.slug}`
}
    ...
    <div className="wlp-page" style={roleplayStyle()}>
      <div className="wlp-shell">
        <nav className="wlp-breadcrumb" aria-label="Migas de pan">
          ...
          <Link href="/practica/ingles/a2">🇬🇧 Inglés A2</Link>
          <span aria-hidden="true">/</span>
          <span aria-current="page">Expresión oral</span>
        </nav>

        <header className="wlp-hero wlp-hero--compact">
          <p className="wlp-eyebrow">Speaking · Inglés A2</p>
        ...
        <section className="wlp-section" aria-labelledby="speaking-modes-title">
          ...
          <div className={s.modeGrid}>
            <Link className="wlp-card wlp-card--path" href="/practica/ingles/a2/habla/solo">
            ...
            <Link className="wlp-card wlp-card--path" href="/practica/ingles/a2/habla/acompanada">
              ...
              <ul className={s.cardFacts}>
                ...
                <li>5–8 minutos por escenario</li>
    ...
    <div className="wlp-page" style={roleplayStyle()}>
      <div className="wlp-shell">
        <nav className="wlp-breadcrumb" aria-label="Migas de pan">
          ...
          <Link href="/practica/ingles/a2">🇬🇧 Inglés A2</Link>
          <span aria-hidden="true">/</span>
          <Link href="/practica/ingles/a2/habla">Expresión oral</Link>
          <span aria-hidden="true">/</span>
          <span aria-current="page">Habla acompañada</span>
        </nav>

        <header className="wlp-hero wlp-hero--compact">
          <p className="wlp-eyebrow">Roleplay · Inglés A2</p>
          <h1>Habla acompañada</h1>
          <p className="wlp-hero-lead">
            Dos personas reciben información diferente y necesitan llegar a una salida hablando
            en inglés. No hay respuestas correctas ni un diálogo que memorizar.
          </p>
        ...
        <section className="wlp-section" aria-labelledby="scenarios-title">
          <div className="wlp-section-heading">
            <h2 id="scenarios-title">Ocho situaciones</h2>
          ...
          <div className={s.scenarioGrid}>
            {set.scenarios.map((scenario) => (
              <Link className={`wlp-card wlp-card--path ${s.scenarioCard}`} href={scenarioPath(scenario)} key={scenario.id}>
                <div className={s.scenarioNumber} aria-hidden="true">{String(scenario.sequence).padStart(2, '0')}</div>
                <span className="wlp-eyebrow wlp-eyebrow--card" lang="en">{scenario.titleTarget}</span>
        ...
        <div className={s.toolkitCallout}>
          <div>
            ...
            <h2>Caja de herramientas A2</h2>
            <p>Frases para abrir, cerrar, pedir repetición, ganar tiempo y decir que no.</p>
          </div>
          <Link className="wlp-btn wlp-btn--secondary" href="/practica/ingles/a2/habla/acompanada/herramientas">
            Ver herramientas →
          </Link>
        </div>
...
function RoleChoice({ scenario, role }: { scenario: RoleplayScenario; role: RoleplayRole }) {
  const href = rolePath(scenario, role.id)
  return (
    ...
    <div className="wlp-page" style={roleplayStyle()}>
      <div className="wlp-shell">
        <nav className="wlp-breadcrumb" aria-label="Migas de pan">
          <Link href="/practica/ingles/a2/habla">Expresión oral</Link>
          <span aria-hidden="true">/</span>
          <Link href="/practica/ingles/a2/habla/acompanada">Habla acompañada</Link>
          <span aria-hidden="true">/</span>
          <span aria-current="page">{scenario.title}</span>
        </nav>

        <header className="wlp-hero wlp-hero--compact">
          <p className="wlp-eyebrow">Escenario {scenario.sequence} · Inglés A2</p>
          <h1>{scenario.title}</h1>
          <p className={s.targetTitle} lang="en">{scenario.titleTarget}</p>
        ...
        <section className={s.preflight} aria-labelledby="preflight-title">
          ...
          <ul className={s.checkList}>
            ...
            <li>Hablen en inglés; usen la caja de herramientas si se bloquean.</li>
  ...
  return (
    <>
      <header className="wlp-hero wlp-hero--compact">
        ...
        <h1>{role.nameEs}</h1>
        <p className={s.targetTitle} lang="en">{role.headline}</p>
      ...
      <section className={s.briefing} aria-label="Cómo jugar este rol" lang="en">
      ...
      <section className={s.piece} aria-labelledby="your-brief-title">
        ...
        <div className={s.proseGrid} lang="en">
      ...
      <section className={s.piece} aria-labelledby="facts-title">
        ...
        <div className={s.panel}>
          ...
          {role.factsNote ? <p className={s.para} lang="en">{inline(role.factsNote)}</p> : null}
          <div className={s.tableWrap} lang="en">
      ...
      <section className={s.piece} aria-labelledby="vocab-title">
        ...
        <div className={s.panel}>
          ...
          <div className={s.tableWrap} lang="en">
            <table className={s.table}>
              <thead><tr><th>Word</th><th>What it is</th><th>Here</th></tr></thead>
              <tbody>
                {role.vocab.map((item) => (
                  <tr key={item.word}><td><strong>{inline(item.word)}</strong></td><td>{inline(item.whatItIs)}</td><td>{inline(item.here)}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
      ...
      <section className={s.piece} aria-labelledby="toolkit-link-title">
        ...
        <div className={s.toolkitCallout}>
          <div>
            <h2 id="toolkit-link-title">Caja de herramientas A2</h2>
            <p lang="en">{inline(role.toolkit)}</p>
          </div>
          <Link className="wlp-btn wlp-btn--secondary" href="/practica/ingles/a2/habla/acompanada/herramientas" target="_blank">
      ...
      <section className={s.piece} aria-labelledby="exponents-title">
        ...
        <div className={s.panel}>
          ...
          <div className={s.tableWrap} lang="en">
            <table className={s.table}>
              <thead><tr><th>Purpose</th><th>Form</th><th>Effect</th></tr></thead>
              <tbody>
                {role.exponents.map((item) => (
                  <tr key={`${item.purpose}-${item.form}`}><td>{inline(item.purpose)}</td><td>{inline(item.form)}</td><td>{inline(item.effect)}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
      ...
      <section className={s.piece} aria-labelledby="closing-title">
        ...
        <div className={s.panel} lang="en">
          <div className={s.panelHeading}><h2 id="closing-title">Closing criterion</h2></div>
          <Blocks blocks={scenario.closing} />
          <div className={s.success}><strong>Success:</strong> {inline(role.success)}</div>
        </div>
      </section>
        ...
        <ul className={s.refList}>
          {scenario.grammarReferences.map((ref) => (
            <li className={s.ref} key={`${ref.level}-${ref.slug}`}>
              <Link href={`/practica/ingles/${ref.level}/gramatica/${ref.slug}`}>{ref.title}</Link>
      ...
      <div className={s.roleSwitch}>
        ...
        <Link className="wlp-btn wlp-btn--secondary" href={rolePath(scenario, counterpart.id)}>
      ...
      <div className="wlp-next">
        <Link href={scenarioPath(scenario)}>← Volver a repartir roles</Link>
        <Link href="/practica/ingles/a2/habla/acompanada">Elegir otro escenario</Link>
        <Link href="/practica/ingles/a2/habla/solo">Practicar a solas</Link>
      </div>
    </>
  )
}
    ...
    <div className="wlp-page" style={roleplayStyle()}>
      <div className="wlp-shell">
        <nav className="wlp-breadcrumb" aria-label="Migas de pan">
          <Link href="/practica/ingles/a2/habla/acompanada">Habla acompañada</Link>
          <span aria-hidden="true">/</span>
          <Link href={scenarioPath(scenario)}>{scenario.title}</Link>
          <span aria-hidden="true">/</span>
        ...
        {scenario.card.toRole === role.id ? (
          <RoleplayCardScreen
            ...
            intro={<div lang="en"><Blocks blocks={scenario.card.openWhen} /></div>}
            card={<div lang="en"><Blocks blocks={scenario.card.blocks} /></div>}
          >
            {content}
          </RoleplayCardScreen>
        ) : content}
  ...
  return (
    <div className="wlp-page" style={roleplayStyle()}>
      <div className="wlp-shell">
        ...
        <header className="wlp-hero wlp-hero--compact">
          <p className="wlp-eyebrow">Support · Inglés A2</p>
          <h1>Caja de herramientas</h1>
          <p className="wlp-hero-lead">{set.toolkit.intro}</p>
        </header>

        <div className={s.toolkitBlocks} lang="en">
          {set.toolkit.blocks.map((block) => (
            <section className={s.toolkitBlock} key={block.number} aria-labelledby={`toolkit-${block.number}`}>
              ...
              <div className={s.tableWrap}>
                <table className={s.table}>
                  <thead><tr><th>Form</th><th>When</th><th>Register</th></tr></thead>
                  <tbody>
                    {block.rows.map((row) => (
                      <tr key={`${row.form}-${row.when}`}>
                        <td><code className={s.code}>{row.form}</code>{row.tag ? <span className={s.rowTag}>{row.tag}</span> : null}</td>
                        <td>{row.when}</td>
                        <td>{row.register ?? '—'}</td>
                      </tr>
                    ))}
        ...
        <div className="wlp-next">
          <Link href="/practica/ingles/a2/habla/acompanada">← Elegir un escenario</Link>
          <Link href="/practica/ingles/a2/habla">Cambiar de modo</Link>
          <Link href="/practica/ingles/a2/gramatica">Repasar gramática A2</Link>
        </div>
      </div>
    </div>
  )
