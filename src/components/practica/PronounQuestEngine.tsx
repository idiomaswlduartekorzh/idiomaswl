...
export default function PronounQuestEngine({ config, languageSlug }: { config: PronounQuestConfig<string>; languageSlug: string }) {
  ...
  return <div className="wlp-page" style={{ '--wlp-accent': SKILL_ACCENT.gramatica.var } as React.CSSProperties}>
    <div className="wlp-shell">
      <nav aria-label="Migas de pan" className="wlp-breadcrumb"><Link href="/herramientas">Herramientas</Link><span aria-hidden="true">/</span><Link href="/herramientas/quizes">Quizes</Link><span aria-hidden="true">/</span><Link href={`/herramientas/quizes/idiomas/${languageSlug}`}>{config.languageName}</Link><span aria-hidden="true">/</span><span aria-current="page">Pronombres</span></nav>
