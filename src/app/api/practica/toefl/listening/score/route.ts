...
export async function POST(request: Request) {
  try {
    ...
    if (!isRecord(body)
      ...
      return Response.json({ error: 'invalid_listening_score_request' }, { status: 400 });
    }

    const scoring = Object.hasOwn(TOEFL_FIXED_LISTENING_SCORING_BY_OBJECT_ID, body.objectId)
      ? TOEFL_FIXED_LISTENING_SCORING_BY_OBJECT_ID[body.objectId]
      : undefined;
    ...
    if (responseEntries.length > configuredIds.size
      || responseEntries.some(([itemId, value]) => !configuredIds.has(itemId) || (value !== null && !isSafeId(value)))
      || body.presentedItemIds.some((itemId) => !configuredIds.has(itemId))) {
      return Response.json({ error: 'invalid_listening_score_request' }, { status: 400 });
    }

    const result = scoreToeflListeningAttempt(scoring, {
