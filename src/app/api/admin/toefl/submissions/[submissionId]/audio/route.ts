...
export async function GET(_request: Request, context: { params: Promise<{ submissionId: string }> }): Promise<Response> {
  ...
  if (!TOEFL_SUBMISSION_ID_PATTERN.test(submissionId)) return fail(404, 'La entrega no es válida.')
  ...
  try {
    ...
    const signed = await Promise.all(entries.map(async ([questionId, path]) => {
      const { data, error: storageError } = await admin.storage.from(TOEFL_SPEAKING_BUCKET).createSignedUrl(path, 5 * 60)
