...
export async function GET(request:Request,{params}:{params:Promise<{orderId:string}>}) {
  ...
  try {
    ...
    return json({order:{id:order.id,reference:order.reference,selection:order.selection,amountInCents:order.amount_in_cents,classes:order.classes,sessions:order.sessions,createdAt:order.created_at,expiresAt:order.expires_at,acceptedAt:order.created_at,termsVersion:order.terms_version,legalSnapshot:order.contact?.acceptance?.snapshot??null},status:state.status});
