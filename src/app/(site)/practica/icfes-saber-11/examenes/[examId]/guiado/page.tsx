    ...
    <main className={styles.learningPage} style={{ '--part-color': part.color, '--part-soft': part.softColor } as React.CSSProperties}>
      ...
      <div className={styles.pageWrap}>
        ...
        <header className={styles.guidedHero}>
          <div><p className={styles.kicker}>Muestra histórica atribuida · modo guiado completo</p><h1>{exam.title}, explicado paso a paso</h1><p>Resuelve las {questions.length} preguntas del banco local y recibe análisis pedagógico original de WeLearn: evidencia, razonamiento, explicación de cada distractor y una microlección transferible.</p></div>
          <div className={styles.guidedModeCard}><span>Modo guiado</span><strong>{questions.length} preguntas</strong><p>Conserva la cantidad y el orden del banco local atribuido a esta muestra de {exam.year}; no se presenta como formato estándar 2026-2.</p><Link href={`/practica/icfes-saber-11/examenes/${exam.id}`}>Cambiar a modo examen →</Link></div>
        </header>
        <section className={styles.workbookMap} aria-labelledby="workbook-map-title"><div><p className={styles.kicker}>Mapa pedagógico local</p><h2 id="workbook-map-title">{exam.partMapping.canonicalSkillPartRanges.length} habilidades organizadas para esta muestra</h2><p>Esta organización usa la taxonomía didáctica vigente de WeLearn; no afirma reproducir las partes históricas del cuadernillo fuente.</p></div><ol>{exam.partMapping.canonicalSkillPartRanges.map((range) => { const config = ICFES_PARTS.find((item) => item.part === range.part); return <li key={range.part} style={{ '--map-color': config?.color } as React.CSSProperties}><span>Parte {range.part}</span><strong>{config?.shortTitle}</strong><small>Preguntas {range.from}–{range.to}</small></li>; })}</ol></section>
        ...
        <section className={styles.sourceSection}><div><span>Procedencia declarada</span><p>{exam.source} La atribución aún no cuenta con cotejo primario por ítem. Las explicaciones, la selección de evidencia y el análisis de distractores son elaboración editorial de Idiomas WeLearn.</p></div><div><span>Alcance histórico</span><p>Las {questions.length} preguntas conservan la cantidad y el orden del banco local asociado a la muestra de {exam.year}. Esta práctica no reproduce necesariamente la aplicación estándar 2026-2, no implica afiliación o aval y no predice un puntaje oficial.</p></div></section>
      </div>
    </main>
  );
}
