...
function normalizeTeacherReview(value: unknown): IcfesTeacherReviewStatusDto | null {
  ...
  const attribution = row.attribution === 'quality-approved-ai-assisted'
    ? row.attribution : row.attribution === null ? null : undefined;
  if (status === undefined || requestedAt === undefined || dueAt === undefined || completedAt === undefined || attribution === undefined) return null;
  return {
    canRequest: false, status, requestedAt, dueAt, completedAt, attribution,
    result: row.result && typeof row.result === 'object'
      ? row.result as IcfesTeacherReviewStatusDto['result'] : null,
  };
}

export default function IcfesPaidResultClient({ attemptId, transactionId }: { attemptId: string; transactionId: string | null }) {
  ...
  const refreshTeacherReview = useCallback(async (signal?: AbortSignal): Promise<boolean> => {
    try {
      ...
      if (!review) {
        setTeacherStatusError('No encontramos una solicitud de feedback asociada a este intento.');
        return false;
      }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(body.code === 'credit_exhausted'
          ...
          : 'No pudimos solicitar el feedback personalizado.');
      }
      setDetail((current) => current ? { ...current, teacherReview: {
        ...
        dueAt: body.review.dueAt, completedAt: null, attribution: null, result: null,
      } } : current);
    } catch (caught) {
      setTeacherError(caught instanceof Error ? caught.message : 'No pudimos solicitar el feedback personalizado.');
    } finally { setTeacherBusy(false); }
  };
  useEffect(() => {
    let active = true;
    const synchronizePayment = transactionId
      ? fetch('/api/icfes/pass/verify', {
        method: 'POST', headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ attemptId, transactionId }),
      }).catch(() => null)
      : Promise.resolve(null);
    synchronizePayment.then(() => fetch(`/api/icfes/attempts/${attemptId}/detail`, { cache: 'no-store' }))
      .then(async (response) => {
        const body = await response.json();
  ...
  return <div className="prac-shell"><div className="prac-results">
    ...
    {detail?.paymentStatus === 'APPROVED' && detail.result && <>
      ...
      {detail.teacherReview && (detail.teacherReview.canRequest || detail.teacherReview.status) && <section className="icfes-product-card" aria-labelledby="teacher-review-title">
        <h2 id="teacher-review-title">Feedback pedagógico personalizado</h2>
        {detail.teacherReview.canRequest && <>
          <p>Tu plan incluye un crédito por periodo. WeLearn preparará feedback personalizado con asistencia de IA y control de calidad interno.</p>
          <button className="btn" disabled={teacherBusy} onClick={requestTeacherReview}>
            {teacherBusy ? 'Solicitando…' : 'Solicitar mi feedback personalizado'}
          </button>
        </>}
        ...
        {detail.teacherReview.status === 'completed' && detail.teacherReview.result && <div>
          <p><strong>Feedback pedagógico personalizado de WeLearn con asistencia de IA.</strong></p>
      ...
      {detail.productCode === 'exam-teacher' && !detail.teacherReview && <section className="icfes-product-card icfes-product-card--muted" aria-labelledby="teacher-review-recovery-title">
        <h2 id="teacher-review-recovery-title">Feedback pedagógico personalizado</h2>
