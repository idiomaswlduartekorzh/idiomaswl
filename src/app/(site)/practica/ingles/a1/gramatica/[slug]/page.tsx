...
export default async function GrammarTopicPage({ params }: Props) {
    ...
    '@graph': [
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'Inglés', item: 'https://www.idiomaswl.com/practica/ingles' },
          { '@type': 'ListItem', position: 3, name: 'A1', item: 'https://www.idiomaswl.com/practica/ingles/a1' },
  ...
  const waText = encodeURIComponent(
    `Hola, tengo una pregunta sobre ${topic.shortTitle} en inglés A1.`,
  )
    ...
    <>
      ...
      >
        <div className="wrap">
            ...
            <div className="topic-hero__grid">
              <div>
                <p className="eyebrow" style={{ marginBottom: '0.5rem' }}>
                  <span className="ink-line" />Gramática · {topic.category} · {topic.level}
                </p>
              ...
              <div className="hero-brief">
                <div>
                  <span>✓</span>
                  <p>
                    <strong style={{ display: 'block', color: 'var(--ink)', marginBottom: '0.18rem' }}>Objetivo</strong>
                    {topic.guide.goal}
                  </p>
                </div>
                <div>
                  <span>≈</span>
                  <p>
                    <strong style={{ display: 'block', color: 'var(--ink)', marginBottom: '0.18rem' }}>Fórmula</strong>
                    {topic.guide.formula}
                  </p>
                </div>
                <div>
                  <span>⚑</span>
                  <p>
                    <strong style={{ display: 'block', color: 'var(--ink)', marginBottom: '0.18rem' }}>Logros</strong>
                    {topic.outcomes.join(' · ')}
                  </p>
                </div>
              </div>
          ...
          {/* SEO Article — Server-rendered so Google sees it without JS */}
          <article id="explicacion" className="seo-article">
            <span className="section-label">— Explicación del especialista</span>
            <h2 className="gram-h2">
              Aprende {topic.shortTitle}: guía para hispanohablantes
            </h2>
                ...
                {section.table && (
                  <div className="grammar-table" style={{ marginTop: '1rem' }}>
                    {section.table.map((row, ri) => (
                      <div
                        key={ri}
                        className={`grammar-table__row${ri === 0 ? ' is-head' : ''}`}
                ...
                {section.examples && (
                  <div className="example-stack">
                    {section.examples.map((ex, ei) => (
                      <div
                        key={ei}
                        style={{ gridTemplateColumns: `repeat(${ex.length}, minmax(0,1fr))` }}
                      >
            ...
            {/* WhatsApp CTA — editorial, at article end, never banner */}
            <div className="article-section">
              <p>
                ¿Tienes dudas sobre {topic.shortTitle} o quieres practicarlos en clase con retroalimentación personalizada?{' '}
                <a
                  href={`https://wa.me/573005004253?text=${waText}`}
                  rel="nofollow"
                  style={{ color: topic.color, fontWeight: 700, textDecoration: 'none' }}
                >
            ...
            >
              ...
              <span>
                Escrito por el <strong style={{ color: 'var(--ink)' }}>Equipo Idiomas WeLearn</strong>
                {' '}· Gramática {topic.level} para hispanohablantes
              </span>
            </div>
          </article>

          {/* Visual brief + practice engine — Client Component */}
          ...
          <nav className="topic-nav" aria-label="Navegación de temas">
            <Link href={`/practica/${IDIOMA}/${NIVEL}/gramatica`} className="topic-nav__link">
              ...
              <strong>Gramática A1</strong>
            </Link>
            <div
              className="topic-nav__link next"
              style={{
