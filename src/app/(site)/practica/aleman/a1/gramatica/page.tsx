...
import { TOPICS, GRAMMAR_COLOR } from '@/data/practica/aleman-a1-gramatica'
...
export default function Page() {
  ...
  return (
    <>
      <CourseSchema
        name="Gramática de Alemán A1 — 15 temas con explicación y ejercicios"
        description="Curso de gramática de alemán nivel A1: artículos (der/die/das), kein, plural, sein/haben, orden de palabras (verbo en 2), acusativo, verbos modales y separables, es gibt y más. Explicación, tablas, ejemplos y ejercicios."
        url="https://www.idiomaswl.com/practica/aleman/a1/gramatica"
        educationalLevel="A1"
        teaches="Gramática del alemán, nivel A1 (MCER)"
        inLanguage="de"
      />
      <section className="wl-section">
        <div className="wrap" style={{ maxWidth: 820 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '0.25rem' }}>
            <p className="eyebrow" style={{ margin: 0 }}><span className="ink-line" />Grammatik · Alemán A1</p>
            <XPStreak showMotivation />
          </div>
          <h1 style={{ fontSize: '2rem', letterSpacing: '-0.03em', margin: '0 0 0.5rem', fontWeight: 700 }}>Gramática de Alemán A1</h1>
          <p style={{ color: 'var(--muted)', fontSize: '1rem', maxWidth: 560, margin: '0 0 2rem' }}>
            ...
            traducidos, errores comunes de hispanohablantes (contraste español→alemán) y {totalQuestions}+ ejercicios con feedback inmediato.
          </p>

          {/* Lista de temas */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(330px, 1fr))', gap: '0.85rem' }}>
            {TOPICS.map(t => (
              <Link key={t.slug} href={`/practica/aleman/a1/gramatica/${t.slug}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block' }}>
                <div style={{
          ...
          <div style={{ marginTop: '2rem', padding: '0.9rem 1.2rem', borderRadius: 12, background: `${COLOR}0a`, border: `1px solid ${COLOR}22`, fontSize: '0.84rem', color: 'var(--muted)', lineHeight: 1.6 }}>
            ...
            (artículos, sein/haben) a lo más completo (acusativo, modales, verbos separables, es gibt). Cada uno te toma 5–10 minutos.
          </div>
        </div>
      </section>
    </>
