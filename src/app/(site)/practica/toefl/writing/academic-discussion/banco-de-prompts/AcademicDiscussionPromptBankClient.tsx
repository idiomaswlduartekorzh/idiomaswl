...
import { CheckCircle2, Filter, Lightbulb, MessageSquareText, RotateCcw } from 'lucide-react';

import {
  ACADEMIC_DISCUSSION_PROMPTS,
  PROMPT_CATEGORIES,
  type AcademicDiscussionPrompt,
} from './prompts';
...
const INTERNAL_LINKS = [
  ...
  { href: '/practica/toefl/writing/academic-discussion', label: 'Guía Academic Discussion' },
...
function PromptCard({ prompt, featured = false }: { prompt: AcademicDiscussionPrompt; featured?: boolean }) {
    ...
    <article className="wl-card" style={{ padding: '1rem', borderRadius: 16, borderTop: `4px solid ${featured ? ACCENT : 'var(--line-soft)'}` }}>
      ...
      <div style={{ display: 'grid', gap: '0.7rem' }}>
        <section style={{ padding: '0.9rem', borderRadius: 14, background: `${ACCENT}0d`, border: `1px solid ${ACCENT}25` }}>
          <p style={{ margin: '0 0 0.3rem', color: ACCENT, fontFamily: 'var(--mono)', fontWeight: 900, fontSize: '0.72rem', textTransform: 'uppercase' }}>Professor</p>
          <p style={{ margin: 0, color: 'var(--ink-2)', lineHeight: 1.6 }}>{prompt.professor}</p>
        </section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 230px), 1fr))', gap: '0.7rem' }}>
          <section style={{ padding: '0.85rem', borderRadius: 14, background: 'var(--bg-2)', border: '1px solid var(--line-soft)' }}>
            <p style={{ margin: '0 0 0.3rem', color: 'var(--muted)', fontFamily: 'var(--mono)', fontWeight: 900, fontSize: '0.72rem', textTransform: 'uppercase' }}>Student A</p>
            <p style={{ margin: 0, color: 'var(--ink-2)', lineHeight: 1.55, fontSize: '0.9rem' }}>{prompt.studentA}</p>
          </section>
          <section style={{ padding: '0.85rem', borderRadius: 14, background: 'var(--bg-2)', border: '1px solid var(--line-soft)' }}>
            <p style={{ margin: '0 0 0.3rem', color: 'var(--muted)', fontFamily: 'var(--mono)', fontWeight: 900, fontSize: '0.72rem', textTransform: 'uppercase' }}>Student B</p>
            <p style={{ margin: 0, color: 'var(--ink-2)', lineHeight: 1.55, fontSize: '0.9rem' }}>{prompt.studentB}</p>
          </section>
        </div>

        <section style={{ padding: '0.85rem', borderRadius: 14, background: 'var(--bg)', border: '1px solid var(--line-soft)' }}>
          <p style={{ margin: '0 0 0.3rem', fontWeight: 900, color: 'var(--ink)' }}>Question</p>
          <p style={{ margin: 0, color: 'var(--muted)', lineHeight: 1.58 }}>{prompt.question}</p>
        </section>
        ...
        <section>
          <p style={{ margin: '0 0 0.45rem', color: 'var(--ink)', fontWeight: 900 }}>Checklist de respuesta</p>
...
export default function AcademicDiscussionPromptBankClient() {
  const [category, setCategory] = useState<Category>('All');
  const [query, setQuery] = useState('');

  const filteredPrompts = useMemo(() => {
    const normalized = query.trim().toLowerCase();
    return ACADEMIC_DISCUSSION_PROMPTS.filter((prompt) => {
      const matchesCategory = category === 'All' || prompt.category === category;
      const matchesQuery =
        normalized.length === 0 ||
        [prompt.topic, prompt.category, prompt.professor, prompt.question, prompt.strategy]
          .join(' ')
          .toLowerCase()
          .includes(normalized);
    ...
    <main className="wl-section">
      <div className="wrap" style={{ maxWidth: 1080 }}>
        <nav aria-label="Breadcrumb" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.5rem', fontSize: '0.82rem', fontFamily: 'var(--mono)', color: 'var(--muted)', flexWrap: 'wrap' }}>
          ...
          <span>/</span>
          <Link href="/practica/toefl/writing/academic-discussion" style={{ color: 'var(--muted)', textDecoration: 'none' }}>Academic Discussion</Link>
          <span>/</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 320px), 1fr))', gap: '1rem', alignItems: 'stretch', marginBottom: '1.4rem' }}>
          <section>
            ...
            <h1 style={{ fontSize: 'clamp(2rem, 5vw, 3.1rem)', lineHeight: 1.04, letterSpacing: 0, margin: '0 0 0.85rem', color: 'var(--ink)' }}>
              TOEFL Academic Discussion: banco de prompts
            </h1>
            <p style={{ color: 'var(--muted)', fontSize: '1rem', lineHeight: 1.75, margin: 0, maxWidth: 760 }}>
              Practica con prompts originales de Academic Discussion organizados por tema. Cada ejercicio incluye conversación de clase, pregunta, estrategia WeLearn, trampa común, checklist, frases útiles y modelo explicado.
            </p>
          </section>

          <aside className="wl-card" style={{ padding: '1rem', borderRadius: 16, display: 'grid', gap: '0.75rem', alignContent: 'center' }}>
            {[
              { label: 'Prompts', value: ACADEMIC_DISCUSSION_PROMPTS.length.toString(), sub: 'originales' },
              { label: 'Formato', value: 'actual', sub: 'Academic Discussion' },
              { label: 'Salida', value: '100-150', sub: 'palabras útiles' },
            ].map((item) => (
        ...
        <section className="wl-card" style={{ padding: '1rem 1.1rem', borderRadius: 16, marginBottom: '1.2rem' }}>
          ...
          <p style={{ margin: '0 0 0.65rem', color: 'var(--muted)', lineHeight: 1.65, fontSize: '0.9rem' }}>
            <strong style={{ color: 'var(--ink)' }}>Formato oficial:</strong> ETS describe Write for an Academic Discussion como una contribución a una discusión de clase en la que debes expresar y apoyar tu opinión, responder puntos de vista y mantener tono académico.
          </p>
          <p style={{ margin: '0 0 0.65rem', color: 'var(--muted)', lineHeight: 1.65, fontSize: '0.9rem' }}>
            <strong style={{ color: 'var(--ink)' }}>Estrategia WeLearn:</strong> entrenamos un flujo repetible: posición inmediata, conexión con un estudiante, razón desarrollada, ejemplo concreto y cierre que regresa a la pregunta.
          </p>
        ...
        <section className="wl-card" style={{ padding: '1rem', borderRadius: 16, marginBottom: '1.2rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '0.8rem', flexWrap: 'wrap', marginBottom: '0.8rem' }}>
            <div>
              ...
              <h2 style={{ margin: 0, fontSize: '1.25rem', letterSpacing: 0 }}>Elige un tema y escribe</h2>
            </div>
            <button
              type="button"
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 230px), 1fr))', gap: '0.75rem' }}>
            <label style={{ display: 'grid', gap: '0.35rem' }}>
              ...
              <select
                ...
                style={{ width: '100%', border: '1px solid var(--line-soft)', borderRadius: 10, padding: '0.65rem 0.75rem', background: 'var(--bg)', color: 'var(--ink)', font: 'inherit' }}
              >
            ...
            <label style={{ display: 'grid', gap: '0.35rem' }}>
              ...
              <input
                ...
                placeholder="AI, attendance, feedback..."
                style={{ width: '100%', border: '1px solid var(--line-soft)', borderRadius: 10, padding: '0.65rem 0.75rem', background: 'var(--bg)', color: 'var(--ink)', font: 'inherit', boxSizing: 'border-box' }}
              />
            </label>
          </div>
        </section>
        ...
        <section aria-labelledby="featured-heading" style={{ marginBottom: '1.2rem' }}>
          ...
          <PromptCard prompt={ACADEMIC_DISCUSSION_PROMPTS[0]} featured />
        </section>
        ...
        <section className="wl-card" style={{ padding: '1.1rem', borderRadius: 16, marginTop: '1.3rem' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '28px 1fr', gap: '0.65rem', alignItems: 'start' }}>
            <MessageSquareText size={20} color={ACCENT} />
            <div>
              <h2 style={{ margin: '0 0 0.45rem', fontSize: '1.15rem' }}>Cómo usar este banco sin memorizar respuestas</h2>
              <p style={{ margin: 0, color: 'var(--muted)', lineHeight: 1.65, fontSize: '0.9rem' }}>
                Escribe primero sin mirar el modelo. Después compara tu respuesta con el checklist y revisa si tienes una postura clara, una conexión real con la conversación y un ejemplo que pruebe tu idea. Los modelos son guías originales, no respuestas oficiales ni textos para memorizar.
              </p>
            </div>
          </div>
        </section>
