...
export default function BodyTwoPracticeEngine({ essayType }: { essayType: EssayTypeId }) {
  ...
  const mcq = useMemo(() => {
    if (level === 0) return { question: `What is the function of the ${exercise.blocks[2].label.toLowerCase()} block?`, context: exercise.blocks[2].text, correct: exercise.blocks[2].purpose, options: rotateOptions([exercise.blocks[2].purpose, exercise.blocks[0].purpose, exercise.blocks[3].purpose, exercise.blocks[4].purpose], `body2|${lesson.id}`, level), feedback: `For this ${lesson.shortLabel} task, the ${exercise.blocks[2].label.toLowerCase()} performs a specific job: ${exercise.blocks[2].purpose}` };
    ...
    return { question: `Which sentence best completes the ${exercise.blocks[2].label} block?`, context: `${exercise.blocks[0].text} ${exercise.blocks[1].text} [${exercise.blocks[2].label.toUpperCase()} MISSING] ${exercise.blocks[3].text} ${exercise.blocks[4].text}`, correct: exercise.blocks[2].text, options: rotateOptions([exercise.blocks[2].text, ...lesson.examples.filter((item) => item !== exercise).slice(0, 3).map((item) => item.blocks[2].text)], `body2|${lesson.id}`, level), feedback: `The model ${exercise.blocks[2].label.toLowerCase()} performs the relationship required by this question type.` };
