...
export default function FinalReviewPracticeEngine({ essayType }: { essayType: EssayTypeId }) {
  ...
  const mcq = useMemo(() => {
    if (level === 0) return { q: 'Which review layer should diagnose this issue?', c: example.issue, a: example.category, o: rotate([example.category, ...pickDistractors(correctOrder, example.category, example.issue)], `review|${lesson.id}`, level), f: example.explanation };
    if (level === 1) return { q: `What is the decisive ${lesson.shortLabel} task check?`, c: example.prompt, a: lesson.decisiveCheck, o: rotate([lesson.decisiveCheck, 'Did I use at least five different linking phrases spread evenly across the four paragraphs?', 'Did I include a statistic or a named research study in every single body paragraph?', 'Did I write exactly four paragraphs, each of them roughly the same length as the others?'], `review|${lesson.id}`, level), f: lesson.instruction };
    if (level === 2) return { q: 'Which diagnosis protects Task Response most directly?', c: example.issue, a: example.explanation, o: rotate([example.explanation, 'The draft needs a more advanced synonym for its most frequently repeated noun.', 'The draft must include a rhetorical question so that it engages the examiner directly.', 'The draft needs a longer opening sentence in order to signal an academic register.'], `review|${lesson.id}`, level), f: 'Review the answer and logic before polishing individual words.' };
    return { q: 'Which revision fixes the identified problem without changing the essay topic?', c: example.issue, a: example.revision, o: rotate([example.revision, 'This topic is important and has a great many advantages and disadvantages for everyone involved.', 'A famous study at a well-known university proves that this issue affects absolutely everyone.', 'In the future, society will certainly solve this problem without needing any further intervention.'], `review|${lesson.id}`, level), f: example.explanation };
  ...
  return <section id="practice-engine" className={`${styles.section} ${styles.practiceSection}`}><div className={styles.sectionHeading}><p className={styles.kicker}>WeLearn progressive engine</p><h2>Review the essay by level</h2><p>Move from identifying one issue to running the complete pre-submission sequence.</p></div><div className={styles.levelRail}>{LEVELS.map((title, index) => <button key={title} type="button" className={`${styles.levelButton} ${level === index ? styles.levelButtonActive : ''} ${completed.includes(index) ? styles.levelButtonDone : ''}`} onClick={() => go(index)}><span>{completed.includes(index) ? '✓' : index + 1}</span><strong>{title}</strong></button>)}</div><div className={styles.enginePanel}><div className={styles.engineHeader}><div><p>Level {level + 1} of 8</p><h3>{LEVELS[level]}</h3></div><strong>{completed.length}/8 complete</strong></div><div className={styles.exerciseBody}><span className={styles.typeTag}>{lesson.shortLabel}</span>
    ...
    <div className={styles.exerciseActions}><button type="button" className={styles.secondaryButton} onClick={reset}><RotateCcw size={16} /> Reset</button><button type="button" onClick={check} disabled={(mode === 'mcq' && !selected) || (mode === 'order' && ordered.length !== correctOrder.length) || (mode === 'write' && words(writing) < 12) || (mode === 'checklist' && !ticks.every(Boolean))}><CheckCircle2 size={17} /> {mode === 'transfer' ? 'Mark transfer ready' : 'Check this level'}</button></div>{checked && <div className={`${styles.feedback} ${passed ? styles.feedbackCorrect : styles.feedbackIncorrect}`}><CheckCircle2 size={20} /><div><strong>{passed ? SE_CORRIGE.has(mode) ? 'Level complete.' : 'Ready to compare.' : 'Revise this attempt.'}</strong><p>{mode === 'mcq' ? mcq.f : mode === 'order' ? `Use ${correctOrder.join(' → ')}.` : mode === 'write' ? `Compare the function of your revision with: ${example.revision}` : mode === 'checklist' ? 'The checklist is complete only when every tick is supported by visible essay evidence.' : 'Transfer ready; no automated band has been assigned.'}</p></div></div>}
