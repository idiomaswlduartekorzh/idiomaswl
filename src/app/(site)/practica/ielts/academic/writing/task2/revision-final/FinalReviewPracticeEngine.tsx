...
export default function FinalReviewPracticeEngine({ essayType }: { essayType: EssayTypeId }) {
  ...
  const mcq = useMemo(() => {
    if (level === 0) {
      ...
      return { q: 'Which review layer should diagnose this issue?', c: example.issue, a: example.category, o: rotate([example.category, ...pickDistractors(correctOrder, example.category, example.issue)], `review|${lesson.id}`, level), why };
    }
    if (level === 1) {
