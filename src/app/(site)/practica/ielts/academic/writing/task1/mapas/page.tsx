...
const EXERCISES: MapExercise[] = [
  {
    title: 'Centro urbano de Millfield: 1990 vs 2020',
    changes: [
  ...
  {
    title: 'Isla turística: antes vs después del desarrollo',
    changes: [
    ...
    <section className="wl-section">
      <div className="wrap">
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          ...
          <p className="eyebrow" style={{ marginBottom: '0.5rem' }}><span className="ink-line" />🗺️ Sub-habilidad 6 — Mapas</p>
          <h1 style={{ fontSize: '1.75rem', letterSpacing: '-0.03em', margin: '0 0 0.4rem', fontWeight: 700 }}>Describir mapas</h1>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '1.5rem' }}>
            <div style={{ padding: '0.9rem', borderRadius: 10, background: 'rgba(15,61,140,0.05)', border: '1px solid rgba(15,61,140,0.15)' }}>
              <p style={{ fontSize: '0.68rem', fontWeight: 800, color: '#0f3d8c', fontFamily: 'var(--mono)', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 0.4rem' }}>Ubicación</p>
            ...
            <div style={{ padding: '0.9rem', borderRadius: 10, background: 'rgba(220,38,38,0.05)', border: '1px solid rgba(220,38,38,0.15)' }}>
              <p style={{ fontSize: '0.68rem', fontWeight: 800, color: '#dc2626', fontFamily: 'var(--mono)', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 0.4rem' }}>Cambio</p>
              <div style={{ display: 'flex', gap: '0.3rem', flexWrap: 'wrap' }}>
          ...
          <div className="wl-card" style={{ padding: '1.25rem', marginBottom: '1.25rem' }}>
            ...
            <p style={{ fontSize: '0.78rem', color: 'var(--muted)', margin: '0.5rem 0 0', fontStyle: 'italic' }}>Haz clic en un elemento para practicar esa oración.</p>
          </div>
          ...
          {change && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '1.25rem' }}>
              <div style={{ padding: '0.9rem 1.1rem', borderRadius: 10, background: 'rgba(15,61,140,0.06)', border: '1px solid rgba(15,61,140,0.18)', fontSize: '0.87rem', color: 'var(--ink-2)', lineHeight: 1.6 }}>
                <strong style={{ color: '#0f3d8c' }}>Cambio seleccionado:</strong>{' '}
                <em>{change.before}</em> → <em>{change.after}</em>
                {' '}— ubicación: {change.location}
              </div>
              <textarea
                value={text}
                onChange={e => setText(e.target.value)}
                placeholder="Escribe una oración describiendo este cambio..."
                rows={3}
              ...
              {text.trim().length > 10 && !revealed && (
                <button className="btn btn-sm" onClick={() => setRevealed(true)} style={{ alignSelf: 'flex-start' }}>Ver modelo →</button>
              )}
              {revealed && (
                <div className="wl-card" style={{ padding: '1.1rem', borderLeft: '3px solid #059669' }}>
                  <p style={{ fontSize: '0.7rem', fontWeight: 800, color: '#059669', fontFamily: 'var(--mono)', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 0.4rem' }}>Oración modelo</p>
          ...
          <div className="wl-card" style={{ padding: '1.25rem', borderLeft: '3px solid #7c3aed', marginBottom: '1rem' }}>
            <p style={{ fontSize: '0.7rem', fontWeight: 800, color: '#7c3aed', fontFamily: 'var(--mono)', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 0.5rem' }}>Párrafo modelo completo</p>
          ...
          <button className="btn btn-sm" onClick={next}>
            {exIdx < EXERCISES.length - 1 ? 'Siguiente mapa →' : 'Volver al inicio →'}
          </button>
        </div>
      </div>
    </section>
