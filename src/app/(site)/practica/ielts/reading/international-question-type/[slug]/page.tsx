...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const isDiagramLabeling = slug === 'diagram-labeling';
  const isShortAnswer = slug === 'short-answer';
  ...
  const summaryCompletionGuided = isSummaryCompletion ? getSummaryCompletionPassage(SUMMARY_COMPLETION_GUIDED_PASSAGE_ID) : undefined;
  const summaryCompletionIndependent = isSummaryCompletion ? getSummaryCompletionPassage(SUMMARY_COMPLETION_INDEPENDENT_PASSAGE_ID) : undefined;
  const noteCompletionGuided = isNoteCompletion ? getNoteCompletionPassage(NOTE_COMPLETION_GUIDED_PASSAGE_ID) : undefined;
  ...
  const diagramLabelingGuided = isDiagramLabeling ? getDiagramLabelingPassage(DIAGRAM_LABELING_GUIDED_PASSAGE_ID) : undefined;
  const diagramLabelingIndependent = isDiagramLabeling ? getDiagramLabelingPassage(DIAGRAM_LABELING_INDEPENDENT_PASSAGE_ID) : undefined;
  const shortAnswerGuided = isShortAnswer ? getShortAnswerPassage(SHORT_ANSWER_GUIDED_PASSAGE_ID) : undefined;
  const shortAnswerIndependent = isShortAnswer ? getShortAnswerPassage(SHORT_ANSWER_INDEPENDENT_PASSAGE_ID) : undefined;
  ...
  return <InternationalReadingSkillLesson
    ...
    practice={guidedPassage
      ...
      : summaryCompletionGuided
        ? <SummaryCompletionGuidedPractice passage={summaryCompletionGuided} />
      : noteCompletionGuided
      ...
      : diagramLabelingGuided
        ? <DiagramLabelingGuidedPractice passage={diagramLabelingGuided} />
      : shortAnswerGuided
        ? <ShortAnswerGuidedPractice passage={shortAnswerGuided} />
    ...
    independentPracticeExperience={independentPassage
      ...
      : summaryCompletionIndependent
        ? <SummaryCompletionIndependentPractice passage={summaryCompletionIndependent} />
      : noteCompletionIndependent
      ...
      : diagramLabelingIndependent
        ? <DiagramLabelingIndependentPractice passage={diagramLabelingIndependent} />
      : shortAnswerIndependent
        ? <ShortAnswerIndependentPractice passage={shortAnswerIndependent} />
    ...
    sourceReview={isMatchingHeadings ? (
      ...
      <SkillReviewSourceBlock
        ...
        sources={[
          { label: 'Official IELTS Academic Reading format', href: IELTS_ACADEMIC_URL, note: 'Confirms Matching Features as matching statements with a list of options and testing recognition of relationships and connections.' },
      ...
      <SkillReviewSourceBlock
        ...
        reviewedFocus={[
          'Guided, independent and Progress Engine passage pools are separated.',
          'Every response is checked against the displayed word limit, a literal passage span and the grammar of the rebuilt summary.',
          'The learner must map the connected overview before solving individual gaps.',
          'The inherited “the roof” alternative is excluded because it creates the ungrammatical frame “the the roof”.',
        ]}
        sources={[
          { label: 'Official IELTS Academic Reading format', href: IELTS_ACADEMIC_URL, note: 'Confirms Summary Completion as completing a summary with words from the text or a supplied list and that any stated word limit is binding.' },
      ...
      <SkillReviewSourceBlock
        ...
        reviewedFocus={[
          'Guided, independent and Progress Engine passage pools are separated.',
          'Every response is checked against its note heading, literal evidence span, rebuilt bullet grammar and displayed limit.',
          'Full-set feedback stays closed until submission and guided errors reopen for repair.',
          'Candidate sources remain context with explicit authorship, rights and factual-review limitations.',
        ]}
        sources={[
          { label: 'Official IELTS Academic Reading format', href: IELTS_ACADEMIC_URL, note: 'Confirms Note Completion as completing notes with words from the text or a supplied list and that any stated word limit is binding.' },
      ...
      <SkillReviewSourceBlock
        ...
        reviewedFocus={[
          'Guided, independent and Progress Engine passage pools are separated.',
          'Every response is checked against both headers, literal evidence, rebuilt cell grammar and the displayed limit.',
          'The inherited cooling-cell ambiguity accepts all three reasonable exact spans.',
          'Publication authorization and candidate factual context remain explicitly distinct.',
        ]}
        sources={[
      ...
      <SkillReviewSourceBlock
        ...
        reviewedFocus={[
          'Guided, independent and Progress Engine passage pools are separated.',
          'Every response is checked against its visible marker, zone, landmark, exact evidence, rebuilt label grammar and displayed limit.',
          'Feedback diagnoses wrong zones, neighbouring parts, direction, grammar, answer boundaries and copied context.',
          'WeLearn publication authorization and candidate factual context remain explicitly distinct.',
        ]}
        sources={[
          { label: 'Official IELTS Academic Reading format', href: IELTS_ACADEMIC_URL, note: 'Confirms that Diagram Label Completion relates a detailed textual description to labels on a machine, building or other visual information and that the stated word limit is binding.' },
          { label: 'WeLearn practice blueprint', note: 'Defines visual-zone mapping, held-back transfer, local persistence and the client-key security boundary.' },
        ]}
      />
    ) : isShortAnswer ? (
      <SkillReviewSourceBlock
        ...
        reviewedFocus={[
          'Guided, independent and Progress Engine passage pools are separated.',
          'Every response is checked against a named factual target, ordered literal evidence and the displayed maximum.',
          'Feedback diagnoses wrong targets, wrong evidence zones, nearby details, copied context, word limits and outside knowledge.',
          'WeLearn publication authorization and candidate factual context remain distinct from official format guidance.',
        ]}
        sources={[
          { label: 'Official IELTS Academic Reading format', href: IELTS_ACADEMIC_URL, note: 'Confirms Short-answer Questions as factual-detail questions answered with words and/or numbers from the text, in passage order, under a binding word limit.' },
    ...
    faqs={route.faqs}
    officialNote={isMatchingHeadings || isMatchingInformation || isMatchingFeatures || isMatchingSentenceEndings || isSentenceCompletion || isSummaryCompletion || isNoteCompletion || isTableCompletion || isFlowChartCompletion || isDiagramLabeling || isShortAnswer
