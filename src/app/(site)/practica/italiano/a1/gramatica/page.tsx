...
import { TOPICS, GRAMMAR_COLOR } from '@/data/practica/italiano-a1-gramatica'
...
export default function Page() {
  ...
  return (
    <>
      <CourseSchema
        name="Gramática de Italiano A1 — 15 temas con explicación y ejercicios"
        description="Curso de gramática de italiano nivel A1: artículos (il/lo/gli), essere, avere, verbos -are/-ere/-ire, preposiciones articuladas, posesivos, c'è/ci sono y más. Explicación, tablas, ejemplos y ejercicios."
        url="https://www.idiomaswl.com/practica/italiano/a1/gramatica"
        educationalLevel="A1"
        teaches="Gramática del italiano, nivel A1 (MCER)"
        inLanguage="it"
      />
      <section className="wl-section">
        <div className="wrap" style={{ maxWidth: 820 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.5rem', fontSize: '0.82rem', fontFamily: 'var(--mono)', color: 'var(--muted)', flexWrap: 'wrap' }}>
            ...
            <span>/</span>
            <span style={{ color: COLOR, fontWeight: 800 }}>📐 Grammatica</span>
          </div>

          {/* Header */}
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '0.25rem' }}>
            <p className="eyebrow" style={{ margin: 0 }}><span className="ink-line" />Grammatica · Italiano A1</p>
            <XPStreak showMotivation />
          </div>
          <h1 style={{ fontSize: '2rem', letterSpacing: '-0.03em', margin: '0 0 0.5rem', fontWeight: 700 }}>Gramática de Italiano A1</h1>
          <p style={{ color: 'var(--muted)', fontSize: '1rem', maxWidth: 560, margin: '0 0 2rem' }}>
            {TOPICS.length} temas esenciales del nivel A1. Cada tema tiene explicación clara, tablas, ejemplos
            traducidos, errores comunes de hispanohablantes (contraste español→italiano) y {totalQuestions}+ ejercicios con feedback inmediato.
          </p>

          {/* Lista de temas */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(330px, 1fr))', gap: '0.85rem' }}>
            {TOPICS.map(t => (
              <Link key={t.slug} href={`/practica/italiano/a1/gramatica/${t.slug}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block' }}>
                <div style={{
          ...
          <div style={{ marginTop: '2rem', padding: '0.9rem 1.2rem', borderRadius: 12, background: `${COLOR}0a`, border: `1px solid ${COLOR}22`, fontSize: '0.84rem', color: 'var(--muted)', lineHeight: 1.6 }}>
            ...
            (artículos, essere, avere) a lo más completo (preposiciones articuladas, c&apos;è/ci sono, verbos irregulares). Cada uno te toma 5–10 minutos.
          </div>
        </div>
      </section>
    </>
