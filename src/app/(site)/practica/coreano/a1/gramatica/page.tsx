...
import { TOPICS, GRAMMAR_COLOR } from '@/data/practica/coreano-a1-gramatica'
...
export default function Page() {
  ...
  return (
    <>
      <CourseSchema
        name="Gramática de Coreano A1 — 15 temas con explicación y ejercicios"
        description="Curso de gramática de coreano nivel A1: Hangul, 이에요/예요, partículas (은/는 이/가 을/를 에/에서), presente -아/어요, números, negación, pasado, -고 싶어요 y más. Con romanización, tablas, ejemplos y ejercicios."
        url="https://www.idiomaswl.com/practica/coreano/a1/gramatica"
        educationalLevel="A1"
        teaches="Gramática del coreano, nivel A1 (TOPIK I / MCER)"
        inLanguage="ko"
      />
      <section className="wl-section">
        <div className="wrap" style={{ maxWidth: 820 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.5rem', fontSize: '0.82rem', fontFamily: 'var(--mono)', color: 'var(--muted)', flexWrap: 'wrap' }}>
            ...
            <span>/</span>
            <Link href="/practica/coreano/a1" style={{ color: 'var(--muted)', textDecoration: 'none' }}>🇰🇷 한국어 A1</Link>
            <span>/</span>
            <span style={{ color: COLOR, fontWeight: 800 }}>📐 문법</span>
          </div>

          {/* Header */}
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '0.25rem' }}>
            <p className="eyebrow" style={{ margin: 0 }}><span className="ink-line" />문법 · Coreano A1</p>
            <XPStreak showMotivation />
          </div>
          <h1 style={{ fontSize: '2rem', letterSpacing: '-0.03em', margin: '0 0 0.5rem', fontWeight: 700 }}>Gramática de Coreano A1</h1>
          <p style={{ color: 'var(--muted)', fontSize: '1rem', maxWidth: 560, margin: '0 0 2rem' }}>
            {TOPICS.length} temas esenciales del nivel A1 (TOPIK I). Cada tema tiene explicación clara con romanización, tablas,
            ejemplos traducidos, errores comunes de hispanohablantes (contraste español→coreano) y {totalQuestions}+ ejercicios con feedback inmediato.
          </p>

          {/* Lista de temas */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(330px, 1fr))', gap: '0.85rem' }}>
            {TOPICS.map(t => (
              <Link key={t.slug} href={`/practica/coreano/a1/gramatica/${t.slug}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block' }}>
                <div style={{
          ...
          <div style={{ marginTop: '2rem', padding: '0.9rem 1.2rem', borderRadius: 12, background: `${COLOR}0a`, border: `1px solid ${COLOR}22`, fontSize: '0.84rem', color: 'var(--muted)', lineHeight: 1.6 }}>
            💡 <strong style={{ color: 'var(--ink)' }}>Consejo:</strong> sigue los temas en orden — empieza por el <strong style={{ color: 'var(--ink)' }}>Hangul</strong> (aprender a leer)
            y avanza hacia las partículas, el presente y el pasado. Cada tema te toma 5–10 minutos.
          </div>
        </div>
      </section>
    </>
