...
import { TOPICS, getTopic, getTopicNav, GRAMMAR_COLOR } from '@/data/practica/coreano-a1-gramatica';
...
export default async function Page({ params }: Props) {
  ...
  const url = `https://www.idiomaswl.com/practica/coreano/a1/gramatica/${topic.slug}`;
  ...
  return (
    <>
      <GrammarLessonSchema
        ...
        keywords={topic.keywords}
        inLanguage="ko"
        course={{ name: 'Gramática de Coreano A1', url: 'https://www.idiomaswl.com/practica/coreano/a1/gramatica' }}
      />
      <QuizSchema
        name={`Ejercicios — ${topic.title}`}
        url={url}
        description={`Ejercicios interactivos con feedback inmediato sobre ${topic.shortTitle.toLowerCase()} en coreano A1.`}
      />
      ...
      <section className="wl-section">
        <div className="wrap" style={{ maxWidth: 780 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.5rem', fontSize: '0.82rem', fontFamily: 'var(--mono)', color: 'var(--muted)', flexWrap: 'wrap' }}>
            ...
            <span>/</span>
            <Link href="/practica/coreano/a1/gramatica" style={{ color: 'var(--muted)', textDecoration: 'none' }}>📐 문법</Link>
            <span>/</span>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '0.25rem' }}>
            <p className="eyebrow" style={{ margin: 0 }}><span className="ink-line" />문법 · Coreano A1 · Tema {topic.order} de {TOPICS.length}</p>
          ...
          <GrammarExplainer topic={topic} color={color} targetLang="coreano" />
          ...
          <nav style={{ display: 'flex', justifyContent: 'space-between', gap: '0.75rem', marginTop: '2.25rem', flexWrap: 'wrap' }}>
            {prev ? (
              <Link href={`/practica/coreano/a1/gramatica/${prev.slug}`} style={{ textDecoration: 'none', color: 'inherit', flex: '1 1 200px' }}>
            ...
            {next ? (
              <Link href={`/practica/coreano/a1/gramatica/${next.slug}`} style={{ textDecoration: 'none', color: 'inherit', flex: '1 1 200px' }}>
              ...
              <Link href="/practica/coreano/a1/gramatica" style={{ textDecoration: 'none', color: 'inherit', flex: '1 1 200px' }}>
                <div style={{ padding: '0.9rem 1.1rem', borderRadius: 12, border: `1px solid ${color}33`, background: `${color}08`, textAlign: 'right' }}>
