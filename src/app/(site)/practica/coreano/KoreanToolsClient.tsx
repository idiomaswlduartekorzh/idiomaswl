...
export default function KoreanToolsClient() {
        ...
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          ...
          <div style={{
            ...
            <p className="eyebrow" style={{ marginBottom: '0.5rem' }}>
              <span className="ink-line" />🇰🇷 Herramientas interactivas
            </p>
            <h2 style={{ fontSize: '1.75rem', letterSpacing: '-0.03em', margin: '0 0 0.5rem', fontWeight: 700 }}>
              Hangul — lectura y pronunciación
            </h2>
              ...
              >
                ...
                {tab.badge && (
                  <span style={{
                    ...
                    padding:'1px 4px', borderRadius:4, letterSpacing:'0.06em', lineHeight:1.4,
                  }}>{tab.badge}</span>
                )}
              </button>
            ))}
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.6rem', marginBottom: '1.25rem' }}>
      {Object.entries(JAMO_COLORS).map(([key, c]) => (
        <div key={key} style={{
          background: `${c.hex}0d`, border: `1.5px solid ${c.hex}33`, borderRadius: 12, padding: '0.75rem 0.85rem',
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.3rem' }}>
            <span style={{ display: 'inline-block', width: 10, height: 10, borderRadius: '50%', background: c.hex, flexShrink: 0 }} />
            <span style={{ fontSize: '0.68rem', fontWeight: 800, color: c.hex, fontFamily: 'var(--mono)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{c.label}</span>
          </div>
          <div style={{ fontSize: '0.82rem', fontWeight: 700, color: 'var(--ink)', marginBottom: '0.2rem' }}>{c.labelEs}</div>
          <div style={{ fontSize: '0.73rem', color: 'var(--muted)', lineHeight: 1.45 }}>{c.desc}</div>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:'1.25rem' }}>
      <div className="wl-card" style={{ padding:'1.5rem' }}>
        <p className="eyebrow" style={{ marginBottom:'0.75rem' }}><span className="ink-line" />Escribe texto en coreano</p>
        <div style={{ display:'flex', gap:'0.75rem', flexWrap:'wrap' }}>
          <input
            value={input} onChange={e => setInput(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter') handleAnalyze(); }}
            placeholder="예: 안녕하세요"
            style={{ flex:'1 1 180px', padding:'0.75rem 1rem', fontSize:'1.1rem', borderRadius:10, border:'1.5px solid var(--line-soft)', background:'var(--bg)', color:'var(--ink)', fontFamily:'inherit', outline:'none' }}
          />
          <button onClick={handleAnalyze} className="btn btn-sm" style={{ fontSize:'0.9rem', padding:'0.75rem 1.25rem' }}>Analizar →</button>
        </div>
        <div style={{ marginTop:'0.85rem' }}>
          ...
          <div style={{ display:'inline-flex', gap:'0.4rem', flexWrap:'wrap', marginTop:'0.3rem' }}>
            {STARTER_TEXTS.map(t => (
              <button key={t} onClick={() => handleQuick(t)} className="btn btn-ghost btn-sm" style={{ fontSize:'0.78rem', padding:'0.25rem 0.65rem' }}>{t}</button>
            ))}
          </div>
        </div>
        {history.length > 1 && (
      ...
      {syllables.length > 0 && (
        <div className="wl-card" style={{ padding:'1.5rem' }}>
          ...
          <JamoColorLegend />
          <p style={{ textAlign:'center', fontSize:'1rem', color:'var(--muted)', fontStyle:'italic', fontFamily:'var(--mono)', margin:'0 0 1.25rem', letterSpacing:'0.04em', background:'var(--bg-2, rgba(0,0,0,0.03))', borderRadius:8, padding:'0.5rem 1rem', border:'1px solid var(--line-soft)' }}>
            [{fullRoman}]
          </p>
          <div style={{ display:'flex', gap:'0.75rem', flexWrap:'wrap', justifyContent:'center' }}>
            {syllables.map((s, idx) => (
              <SyllableCard key={idx} s={s} idx={idx} selected={selected} onSelect={handleClickSyllable} />
            ))}
          </div>
        </div>
      )}
      ...
      <div style={{ padding:'0.85rem 1.1rem', borderRadius:12, background:'rgba(83,74,183,0.06)', border:'1px solid rgba(83,74,183,0.15)', fontSize:'0.83rem', color:'var(--muted)', lineHeight:1.6 }}>
        💡 <strong style={{ color:'var(--ink)' }}>Tip:</strong> Haz clic en cada sílaba para escucharla (+5 XP). Usa <strong style={{ color:'var(--ink)' }}>Lento</strong> para apreciar cada fonema.
      </div>
    </div>
  );
}
...
function SyllableCard({
  ...
  s: SyllableData; idx: number; selected: number | null;
  onSelect: (idx: number, char: string) => void; highlight?: 'cho' | 'jung' | 'jong';
}) {
  ...
  return (
    ...
    >
      {s.isHangul ? (
        <>
          <span style={{ fontSize:'2rem', fontWeight:800, lineHeight:1.1, color: isSelected ? '#534AB7' : 'var(--ink)' }}>{s.char}</span>
          ...
          <span style={{ fontSize:'0.72rem', color:'var(--muted)', marginTop:'0.3rem', fontStyle:'italic', fontFamily:'var(--mono)' }}>{s.romanization}</span>
...
function JamoTable({ addXp }: { addXp: (n: number) => void }) {
  ...
  const consGroups = Object.entries(CONS_TYPES).map(([type, meta]) => ({ ...meta, type, items: CONSONANTS.filter(c => c.type === type) }));
  const vowGroups  = Object.entries(VOW_TYPES).map(([type, meta])  => ({ ...meta, type, items: VOWELS.filter(v => v.type === type) }));
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:'1.25rem' }}>
      <div className="wl-card" style={{ padding:'1.5rem' }}>
        ...
        <p style={{ margin:'0 0 1rem', fontSize:'0.9rem', color:'var(--muted)', lineHeight:1.6 }}>
          El Hangul tiene <strong style={{ color:'var(--ink)' }}>14 consonantes base + 5 tensas</strong> y <strong style={{ color:'var(--ink)' }}>21 vocales</strong>. Haz clic en cualquier jamo para escuchar (+5 XP).
        </p>
        <div style={{ display:'flex', gap:'0.5rem' }}>
          <button onClick={() => setView('consonants')} className={view === 'consonants' ? 'btn btn-sm' : 'btn btn-ghost btn-sm'} style={{ fontSize:'0.85rem' }}>자음 Consonantes</button>
          <button onClick={() => setView('vowels')}     className={view === 'vowels'     ? 'btn btn-sm' : 'btn btn-ghost btn-sm'} style={{ fontSize:'0.85rem' }}>모음 Vocales</button>
        </div>
      </div>
      ...
      {view === 'consonants' && consGroups.map(group => (
        <div key={group.type} className="wl-card" style={{ padding:'1.25rem', borderTop:`3px solid ${group.color}` }}>
          <div style={{ display:'flex', alignItems:'baseline', gap:'0.6rem', marginBottom:'0.3rem' }}>
            <span style={{ fontSize:'0.75rem', fontWeight:800, color:group.color, fontFamily:'var(--mono)', textTransform:'uppercase', letterSpacing:'0.06em' }}>{group.label}</span>
          ...
          <div style={{ display:'flex', flexWrap:'wrap', gap:'0.6rem', marginTop:'0.75rem' }}>
            {group.items.map(c => (
              <button key={c.jamo} onClick={() => handlePlay(c.jamo)} style={{ display:'flex', flexDirection:'column', alignItems:'center', padding:'0.75rem 0.85rem', borderRadius:12, minWidth:72, border:`1.5px solid ${played.has(c.jamo) ? group.color + '88' : 'var(--line-soft)'}`, background: played.has(c.jamo) ? `${group.color}0d` : 'var(--bg)', cursor:'pointer', fontFamily:'inherit', transition:'all 0.15s' }}>
                ...
                <span style={{ fontSize:'0.58rem', color: played.has(c.jamo) ? group.color : 'var(--muted)', marginTop:'0.2rem', fontWeight:700 }}>{played.has(c.jamo) ? '✓' : '🔊'}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      {view === 'vowels' && vowGroups.map(group => (
        <div key={group.type} className="wl-card" style={{ padding:'1.25rem', borderTop:`3px solid ${group.color}` }}>
          ...
          <div style={{ display:'flex', flexWrap:'wrap', gap:'0.6rem' }}>
            {group.items.map(v => (
              <button key={v.jamo} onClick={() => handlePlay(v.jamo.length <= 2 ? v.jamo : v.jamo.slice(-1))} style={{ display:'flex', flexDirection:'column', alignItems:'center', padding:'0.75rem 0.85rem', borderRadius:12, minWidth:72, border:`1.5px solid ${played.has(v.jamo) ? group.color + '88' : 'var(--line-soft)'}`, background: played.has(v.jamo) ? `${group.color}0d` : 'var(--bg)', cursor:'pointer', fontFamily:'inherit', transition:'all 0.15s' }}>
                ...
                <span style={{ fontSize:'0.58rem', color: played.has(v.jamo) ? group.color : 'var(--muted)', marginTop:'0.2rem', fontWeight:700 }}>{played.has(v.jamo) ? '✓' : '🔊'}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div style={{ padding:'0.85rem 1.1rem', borderRadius:12, background:'rgba(83,74,183,0.06)', border:'1px solid rgba(83,74,183,0.15)', fontSize:'0.83rem', color:'var(--muted)', lineHeight:1.6 }}>
        💡 <strong style={{ color:'var(--ink)' }}>Dato:</strong> ㅇ es muda en posición inicial y suena como <strong style={{ color:'var(--ink)' }}>[ng]</strong> en posición final.
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:'1rem' }}>
        ...
        <div style={{ display:'flex', gap:'0.5rem', flexWrap:'wrap', marginTop:'0.9rem' }}>
          {BATCHIM_RULES.map(r => (
            <button key={r.id} onClick={() => setOpenRule(openRule === r.id ? null : r.id)}
              ...
              style={{ fontSize:'0.78rem', ...(openRule === r.id ? { background: r.color, borderColor: r.color } : { color: r.color, borderColor: r.color }) }}
      ...
      {BATCHIM_RULES.map(rule => (
        <div key={rule.id} className="wl-card" style={{ overflow:'hidden', border: openRule === rule.id ? `1.5px solid ${rule.color}55` : undefined }}>
          <button
            onClick={() => setOpenRule(openRule === rule.id ? null : rule.id)}
            style={{ width:'100%', display:'flex', alignItems:'center', gap:'0.75rem', padding:'1.1rem 1.25rem', background: openRule === rule.id ? `${rule.color}0d` : 'transparent', border:'none', cursor:'pointer', textAlign:'left', fontFamily:'inherit', transition:'background 0.2s' }}
          >
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ fontSize:'0.76rem', color:'var(--muted)', marginTop:'0.1rem', fontFamily:'var(--mono)' }}>{rule.trigger}</div>
            </div>
          ...
          {openRule === rule.id && (
            <div style={{ padding:'0 1.25rem 1.25rem' }}>
              ...
              <p style={{ margin:'0 0 0.5rem', fontSize:'0.72rem', color:rule.color, fontWeight:800, textTransform:'uppercase', letterSpacing:'0.06em', fontFamily:'var(--mono)' }}>Ejemplos — haz clic para escuchar</p>
                ...
                {rule.examples.map((ex, i) => {
                  ...
                  return (
                    <button key={i} onClick={() => handleExample(ex, rule.id)}
                      style={{ display:'grid', gridTemplateColumns:'1fr 1fr auto', alignItems:'center', gap:'0.75rem', padding:'0.85rem 1rem', background:`${rule.color}0a`, border:`1px solid ${rule.color}22`, borderRadius:12, cursor:'pointer', textAlign:'left', fontFamily:'inherit' }}
                    >
                      <div>
                      ...
                      <div style={{ textAlign:'right' }}>
                        <div style={{ background: done ? '#059669' : rule.color, color:'#fff', borderRadius:9, padding:'0.4rem 0.75rem', fontSize:'0.78rem', fontWeight:700, marginBottom:'0.3rem', whiteSpace:'nowrap', transition:'background 0.2s' }}>{done ? '✓ +15 XP' : '▶ +15 XP'}</div>
...
function buildQuestion(): QuizQuestion | null {
  ...
  if (type === 'romanization') {
    ...
    const distractors = hangulPool.filter(s => s.romanization !== correct).map(s => s.romanization).filter((v, i, a) => a.indexOf(v) === i).sort(() => Math.random() - 0.5).slice(0, 3);
    return { type, syllable, options: [...distractors, correct].sort(() => Math.random() - 0.5), correct };
  }

  const targets: Array<'cho' | 'jung' | 'jong'> = syllable.jong ? ['cho', 'jung', 'jong'] : ['cho', 'jung'];
  const target = targets[Math.floor(Math.random() * targets.length)];
  const correct = syllable[target];
  if (!correct) return null;

  const pool = target === 'cho' ? CHOSEONG : target === 'jung' ? JUNGSEONG : JONGSEONG.filter(j => j !== '');
  const distractors = pool.filter(j => j !== correct).sort(() => Math.random() - 0.5).slice(0, 3);
  return { type, syllable, target, options: [...distractors, correct].sort(() => Math.random() - 0.5), correct };
}
...
function SyllableQuiz({ addXp }: { addXp: (n: number) => void }) {
  ...
  function handleAnswer(opt: string) {
    ...
    if (isCorrect) { setStreak(s => s + 1); addXp(20); } else { setStreak(0); }
  }

  function nextQuestion() { setQuestion(buildQuestion()); setAnswered(null); }
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:'1.25rem' }}>
      ...
      <div className="wl-card" style={{ padding:'2rem', textAlign:'center' }}>
        ...
        <p style={{ fontSize:'0.88rem', color:'var(--muted)', marginBottom:'0.75rem', lineHeight:1.5 }}>
          {isRomMode
        ...
        <div style={{ fontSize:'5rem', fontWeight:900, lineHeight:1, margin:'0.5rem 0 0.75rem', color:'var(--ink)', textShadow:'0 2px 12px rgba(83,74,183,0.15)' }}>
        ...
        {isRomMode && !answered && <div style={{ fontSize:'0.78rem', color:'var(--muted)', fontFamily:'var(--mono)', marginBottom:'0.5rem', opacity:0.5 }}>[? ? ?]</div>}
        {(answered || !isRomMode) && (
          <div style={{ fontSize:'0.8rem', color:'var(--muted)', fontFamily:'var(--mono)', marginBottom:'1.25rem', fontStyle:'italic' }}>[{question.syllable.romanization}]</div>
        )}
          ...
          {question.options.map(opt => {
            ...
            return (
              <button key={opt} onClick={() => handleAnswer(opt)}
                style={{ padding:'0.9rem', borderRadius:12, border, background:bg, color, fontSize: isRomMode ? '1rem' : '1.6rem', fontWeight:800, cursor: answered ? 'default' : 'pointer', fontFamily: isRomMode ? 'var(--mono)' : 'inherit', fontStyle: isRomMode ? 'italic' : 'normal', transition:'all 0.18s', display:'flex', alignItems:'center', justifyContent:'center', gap:'0.4rem' }}
              >
                {isRomMode ? `[${opt}]` : opt}
...
function JamoPill({ label, colorKey, highlight }: { label: string; colorKey: 'cho' | 'jung' | 'jong'; highlight?: boolean }) {
  if (!label) return null;
  const { hex } = JAMO_COLORS[colorKey];
  return (
    <span style={{
      background: highlight ? `${hex}33` : `${hex}18`, color: hex,
      border: `1px solid ${highlight ? `${hex}88` : `${hex}44`}`,
      borderRadius: 5, padding: '0 5px', fontWeight: 800, fontSize: '0.78rem', lineHeight: '1.5',
      transition: 'all 0.15s', boxShadow: highlight ? `0 0 0 2px ${hex}44` : 'none',
    }}>
      {label}
    </span>
  );
