...
function GoetheAudioEvidence({ submissionId, paths }: { submissionId: string; paths: Record<string, string> | null | undefined }) {
  const [errors, setErrors] = useState<Record<string, boolean>>({})
  const files = Object.entries(paths ?? {}).sort(([a], [b]) => a.localeCompare(b))

  return <section aria-label="Grabaciones privadas de Sprechen">
    <h4 style={{ margin: '0 0 6px', fontSize: 11 }}>Sprechen · evidencia privada</h4>
    {files.length === 0 && <p style={{ color: MUTED, fontSize: 10 }}>Esta entrega no tiene grabaciones asociadas.</p>}
    <div style={{ display: 'grid', gap: 7 }}>{files.map(([questionId, path]) => {
      const src = `/api/admin/goethe/submissions/${encodeURIComponent(submissionId)}/audio/${encodeURIComponent(questionId)}`
      const extension = path.split('.').at(-1)?.replace(/[^a-z0-9]/gi, '') || 'audio'
      return <div key={questionId} style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 8, padding: 8 }}>
        <strong style={{ fontSize: 10 }}>{questionId.replace(/g-a1-\d+-sp/, 'Sprechen Teil ')}</strong>
        <audio controls preload="none" src={src} aria-label={`Escuchar ${questionId}`} onError={() => setErrors(previous => ({ ...previous, [questionId]: true }))} onCanPlay={() => setErrors(previous => ({ ...previous, [questionId]: false }))} style={{ width: '100%', marginTop: 5 }} />
        {errors[questionId] && <p role="alert" style={{ color: '#b91c1c', fontSize: 10, margin: '5px 0' }}>No se pudo reproducir aquí. Comprueba tu sesión o descarga la grabación.</p>}
        <a href={src} download={`goethe-${questionId}.${extension}`} style={{ display: 'inline-block', marginTop: 5, fontSize: 10 }}>Descargar audio</a>
      </div>
    })}</div>
  </section>
}
...
export default function GoetheReviewPanel({ items }: { items: ExamSubmission[] }) {
  ...
  const writingCriteria = active?.mock_id === 'a1-2'
    ? ['Motivo del mensaje', 'Llegada a las 17:30', 'Lugar del Schlüssel']
    : ['Motivo del mensaje', 'Zimmer für zwei', 'Preis mit Frühstück']
  return (
    <section style={{ background: CARD, border: `2px solid ${A}`, borderRadius: 16, padding: 20, boxShadow: '0 1px 6px rgba(0,0,0,.06)' }} aria-labelledby="goethe-review-heading">
      ...
      {visible.length === 0 ? <p style={{ color: MUTED }}>No hay entregas pendientes.</p> : <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 330px), 1fr))', gap: 14 }}>
        ...
        {active && <div style={{ background: BG, borderRadius: 12, padding: 14, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <GoetheAudioEvidence key={active.id} submissionId={active.id} paths={active.speaking_audio_paths} />
          {!active.reviewed_at && <section style={{ borderTop: `1px solid ${BORDER}`, paddingTop: 12, display: 'grid', gap: 10 }}><div><strong style={{ fontSize: 11 }}>Schreiben Teil 2 · 10 Rohpunkte</strong><p style={{ margin: '3px 0', color: MUTED, fontSize: 10 }}>Cada contenido: 0 / 1,5 / 3. Anrede und Gruß: 0 / 0,5 / 1.</p></div><Picker label={writingCriteria[0]} values={[0, 1.5, 3]} value={scores.content1} onChange={value => setScore('content1', value)} /><Picker label={writingCriteria[1]} values={[0, 1.5, 3]} value={scores.content2} onChange={value => setScore('content2', value)} /><Picker label={writingCriteria[2]} values={[0, 1.5, 3]} value={scores.content3} onChange={value => setScore('content3', value)} /><Picker label="Anrede und Gruß" values={[0, 0.5, 1]} value={scores.conventions} onChange={value => setScore('conventions', value)} /><div><strong style={{ fontSize: 11 }}>Sprechen · 15 Rohpunkte</strong></div><Picker label="Teil 1 · Sich vorstellen" values={[0, .5, 1, 1.5, 2, 2.5, 3]} value={scores.part1} onChange={value => setScore('part1', value)} /><Picker label="Teil 2 · Fragen/Antworten" values={Array.from({ length: 13 }, (_, i) => i / 2)} value={scores.part2} onChange={value => setScore('part2', value)} /><Picker label="Teil 3 · Bitten/Reaktionen" values={Array.from({ length: 13 }, (_, i) => i / 2)} value={scores.part3} onChange={value => setScore('part3', value)} /><label style={{ fontSize: 11, fontWeight: 800 }}>Evidencia y feedback<textarea value={notes} onChange={event => setNotes(event.target.value)} rows={5} style={{ display: 'block', width: '100%', marginTop: 4, padding: 9, border: `1px solid ${BORDER}`, borderRadius: 8 }} /></label><button type="button" onClick={save} disabled={saving} style={{ border: 0, borderRadius: 8, background: A, color: '#fff', padding: 10, fontWeight: 800, cursor: 'pointer', opacity: saving ? .65 : 1 }}>{saving ? 'Guardando…' : 'Cerrar revisión y calcular score Goethe'}</button></section>}
