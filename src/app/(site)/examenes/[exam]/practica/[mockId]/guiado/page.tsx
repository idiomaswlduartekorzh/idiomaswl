...
export default async function GuidedMockPage({ params }: Props) {
  ...
  if (!hasMembership) {
    const membershipHref = user
      ? '/suscripcion/examenes'
      : '/registro?path=exam&language=ingles&exam=icfes&plan=exam-auto';
    return (
          ...
          <header className={styles.guidedHero}>
            <div>
              ...
              <div className="prac-results__actions">
                ...
                <Link href={membershipHref} className="btn">Ver plan de COP 49.900</Link>
              </div>
            </div>
            <div className={styles.guidedModeCard}>
              <span>Incluido en membresía</span>
              <strong>45 preguntas</strong>
              <p>También está disponible el plan de COP 99.900 con un crédito de feedback pedagógico personalizado de WeLearn con asistencia de IA por periodo.</p>
            </div>
          </header>
        </div>
      </main>
