...
const reportGeneratedAt = '2026-08-09T05:54:00-05:00';
const base = 'output/audits/ielts-reading-rights-ynng-2026-08-09';
const paths = {
...
const sources = [
  { id: 'validation', label: 'Executable F0.2b.3 validation', path: paths.validation },
  { id: 'baseline', label: 'Pinned YNNG content and learner-runtime baseline', path: paths.baseline },
  ...
  sqlSource({
    id: 'summary_query',
    label: 'F0.2b.3 scope and disposition summary',
...
GROUP BY ALL;`,
  ...
  sqlSource({
    id: 'label_query',
    label: 'Stored YNNG response-label counts',
    tablesUsed: [paths.validation],
    sql: `UNPIVOT (
...
const cue = validation.antiBias.storedKeyProfile.degreeLanguageImpliesNo;
const ambiguousIds = validation.expertReview.materialAmbiguityQuestionIds.join(', ');
...
const artifact = {
  ...
  manifest: {
    version: 1,
    surface: 'report',
    title: 'IELTS Reading Yes / No / Not Given — audit gate',
    description: 'F0.2b.3 dossier for provenance, writer-view attribution, learning value and shortcut risk.',
    generatedAt: reportGeneratedAt,
    cards: [
      {
        id: 'scope',
        description: 'Passages and statements in the scoped formative bank.',
        dataset: 'summary',
      ...
      {
        id: 'agreement',
        description: 'Independent decisions matching stored labels.',
        dataset: 'summary',
        sourceId: 'summary_query',
        metrics: [
    ...
    charts: [
      {
        id: 'label_counts',
        title: 'Stored response-label counts',
        subtitle: 'Question: how are the fixed YES, NO and NOT GIVEN labels distributed? n=22 is descriptive, not a fairness certification.',
        type: 'bar',
        dataset: 'labels',
      ...
      },
      {
        ...
        subtitle: 'Question: how did direct source review classify 13 declared claims? These assessments are not clearance or human verification.',
        type: 'bar',
        dataset: 'claims',
    ...
    sources,
    blocks: [
      { id: 'title', type: 'markdown', body: '# IELTS Reading Yes / No / Not Given — audit gate' },
      {
        ...
        body:
          '## Executive Summary\n\n' +
          `- The F0.2b.3 control passes; the ${validation.scope.passages} passages do not. All remain quarantined.\n` +
          `- Independent review covered ${validation.scope.questions}/${validation.scope.questions} statements, matched all stored labels and found ${validation.expertReview.materialAmbiguityCount} material ambiguities.\n` +
          `- A degree-language cue predicts NO on ${cue.hits} of ${cue.eligible} eligible statements; content certification remains blocked.\n` +
      ...
      { id: 'metrics', type: 'metric-strip', cardIds: ['scope', 'quarantine', 'agreement', 'ambiguity'] },
      {
        ...
        body:
          ...
          'Hashes pin identity, incomplete rights records fail closed, and candidate evidence remains separate from verification. Matching every stored label is not enough: two statements still admit another reasonable answer, and no author, license, human factual review or human approval is recorded.',
      },
      { id: 'decision_table', type: 'table', tableId: 'asset_decisions', layout: 'full' },
      {
        ...
        body:
          '## Writer attribution and absence are explicit contracts\n\n' +
          `YES and NO require a decisive writer-view span; NOT GIVEN requires a related passage zone plus the exact missing claim. The two material ambiguities are ${ambiguousIds}. They must be rewritten and independently re-adjudicated before release.`,
      },
      {
        id: 'label_context',
        type: 'markdown',
        sourceId: 'validation',
        body:
          '## Label counts alone do not certify balance\n\n' +
          `The stored labels are YES ${validation.antiBias.storedKeyProfile.answerCounts.YES}, NO ${validation.antiBias.storedKeyProfile.answerCounts.NO} and NOT GIVEN ${validation.antiBias.storedKeyProfile.answerCounts['NOT GIVEN']}. The targeted degree-language rule applies to ${cue.eligible} statements and predicts NO correctly on ${cue.hits} (${Math.round(cue.conditionalAccuracy * 100)}%). Sequence, length, lexical overlap, writer verbs and title cues are audited separately.`,
      },
      { id: 'label_chart', type: 'chart', chartId: 'label_counts', layout: 'full' },
      {
        ...
        body:
          ...
          'The direct-source pass found only two of 13 declared claims supported as written; seven were oversimplified and four untraceable. Candidate institutional sources can contextualize a claim without proving the passage, its authorship or its license.',
      },
      { id: 'claim_chart', type: 'chart', chartId: 'claim_assessments', layout: 'full' },
      {
        ...
        body:
          ...
          'The clean walkthrough traces voice shifts, concessions, conditions, scope and missing claims across all 22 statements. Its value is prospective and diagnostic: it assigns no labels or bands and does not represent a real learner study.',
      },
      {
        id: 'panel_scope',
        type: 'markdown',
      ...
      { id: 'panel_table', type: 'table', tableId: 'panel_verdicts', layout: 'full' },
      {
        ...
        body:
          ...
          '1. Rewrite the two materially ambiguous statements and repeat blind adjudication.\n' +
          '2. Reduce title and wording cues, then remeasure the multidimensional shortcut profile.\n' +
          '3. Resolve authorship, rights, exact factual support and independent human review for all three passages.\n' +
          '4. Keep F0.2b open; begin F0.2b.4 only in the next loop iteration.',
      },
      {
        id: 'caveats',
        type: 'markdown',
        ...
        body:
          ...
          'The provenance search is directed and non-exhaustive. Source reachability is not verification. Both independent reviews are AI simulations without human signatures. The 22-question sample cannot certify statistical fairness. Runtime quarantine enforcement and learner-facing UI remain separate open units.',
      },
    ],
  },
  snapshot: {
