...
const BLIND_PACKETS_GENERATED_AT = '2026-08-09T16:36:00Z';
const FACTUAL_PACKET_GENERATED_AT = '2026-08-09T16:53:00Z';
...
const NEXT_ENGINE_PATH = ENGINE_PATH;
const NEXT_BANK_PATH = BANK_PATH;
...
const VALIDATOR_PATH = 'scripts/check-ielts-reading-sentence-completion-rights.mjs';
const TEST_PATH = 'tests/ielts-reading-sentence-completion-rights.test.mjs';
...
const EXPECTED_ASSET_IDS = EXPECTED_SET_IDS.map(id => `formative:sentence-completion:${id}`);
const EXPECTED_QUESTION_IDS = [
  ...Array.from({ length: 6 }, (_, index) =>
    `sentence-wetlands-${String(index + 1).padStart(2, '0')}`),
  ...Array.from({ length: 6 }, (_, index) =>
    `sentence-makerspaces-${String(index + 1).padStart(2, '0')}`),
  ...Array.from({ length: 6 }, (_, index) =>
    `sentence-night-markets-${String(index + 1).padStart(2, '0')}`),
];
...
const OFFICIAL_EVIDENCE_ID = 'ielts-sentence-completion-official-format';
const SPELLING_POLICY_EVIDENCE_ID = 'ielts-british-american-spelling-policy';
const OFFICIAL_FORMAT_URL =
  'https://ielts.org/take-a-test/test-types/ielts-academic-test/ielts-academic-format-reading';
const SPELLING_POLICY_URL =
  'https://ielts.org/news-and-insights/ielts-is-the-most-trusted-english-test-in-the-world-heres-why';
...
export const REQUIRED_FACTUAL_CLAIM_SPANS = {
  'formative:sentence-completion:sentence-wetland-parks': [
    'Instead of moving rainwater away as quickly as possible through concrete channels, these parks hold water temporarily and allow it to filter through plants and soil.',
    'During heavy storms, this storage capacity can reduce pressure on underground pipes and lower the risk of street flooding.',
    'Wetland parks also create habitats for insects, birds and amphibians.',
    'Plants must be monitored, litter needs to be removed and water levels should be checked after extreme weather.',
    'If a park is neglected, blocked channels can reduce water flow and the area may become less effective during storms.',
  ],
  'formative:sentence-completion:sentence-library-makerspaces': [
    'In many towns, they now also provide makerspaces: small areas where visitors can use tools such as sewing machines, audio recorders, 3D printers and basic electronics kits.',
    'Supporters argue that these spaces help libraries move from storing information to helping people create things with it.',
    'A library makerspace works best when staff can introduce tools safely and connect visitors with realistic first projects.',
    'Expensive equipment can sit unused if the library buys it without understanding local interests.',
    'Successful libraries therefore separate noisy activities from study zones and choose tools that match community demand.',
  ],
  'formative:sentence-completion:sentence-night-markets': [
    'A closed street, a group of food vendors and a simple lighting plan can create an evening economy without requiring a permanent shopping centre.',
    'For small businesses, the short trading window lowers some costs and allows them to test new products.',
    'A night market beside a transit stop is easier to reach than one placed in an isolated car park.',
    'Organizers may need to limit amplified music, provide waste bins and keep pedestrian routes clear so that the market feels lively without becoming disruptive.',
    'The strongest markets collect feedback after each event. Vendor sales, crowd movement, noise complaints and cleaning costs reveal whether the market is growing in a healthy way.',
  ],
};
...
const KNOWN_ACCEPTED_ANSWERS = {
  'sentence-wetlands-01': ['concrete channels'],
  'sentence-wetlands-02': ['underground pipes'],
  'sentence-wetlands-03': ['shallow pools'],
  'sentence-wetlands-04': ['infrastructure'],
  'sentence-wetlands-05': ['water flow'],
  'sentence-wetlands-06': ['purpose'],
  'sentence-makerspaces-01': ['3D printers', '3d printers'],
  'sentence-makerspaces-02': ['storing'],
  'sentence-makerspaces-03': ['first projects'],
  'sentence-makerspaces-04': ['interests'],
  'sentence-makerspaces-05': ['study zones'],
  'sentence-makerspaces-06': ['public role'],
  'sentence-night-markets-01': ['shopping centre', 'shopping center'],
  'sentence-night-markets-02': ['new products'],
  'sentence-night-markets-03': ['transit stop'],
  'sentence-night-markets-04': ['pedestrian routes'],
  'sentence-night-markets-05': ['longer'],
  'sentence-night-markets-06': ['long-term success'],
};
...
const QUESTION_ID_PATTERN = /\bsentence-(?:wetlands|makerspaces|night-markets)-0[1-6]\b/giu;
...
export function findSentenceCompletionAssignmentLeaks(
  value, path = '$', findings = [], inheritedQuestionId = null, assignmentContext = false,
) {
  if (Array.isArray(value)) {
    value.forEach((entry, index) => findSentenceCompletionAssignmentLeaks(
  ...
  for (const [key, entry] of Object.entries(value)) {
    ...
    findSentenceCompletionAssignmentLeaks(key.replace(/[-_]+/gu, ' '), `${path}.${key}:key`,
      findings, scoped, fieldIsAssignment);
    findSentenceCompletionAssignmentLeaks(entry, `${path}.${key}`, findings, scoped,
...
function assertCleanPacket(packet, label) {
  ...
  assert(findSentenceCompletionAssignmentLeaks(packet).length === 0,
...
function sourceAssets(catalog) {
  const sets = catalog.IELTS_SENTENCE_COMPLETION_PASSAGES;
  assert(Array.isArray(sets), 'Falta IELTS_SENTENCE_COMPLETION_PASSAGES.');
  return sets.map((set, setIndex) => ({
    assetId: `formative:sentence-completion:${set.id}`,
    id: set.id,
    setIndex,
    title: set.title,
...
function structuralRiskProfile(assets) {
  ...
  const occurrenceRows = questions.map(({ asset, question }) => ({
    ...
    occurrences: occurrenceCount(normalizeRuntimeAnswer(asset.passage),
      normalizeRuntimeAnswer(question.answer)),
  }));
  const evidenceRows = questions.map(({ asset, question }) => ({
    assetId: asset.assetId,
    questionId: question.id,
    offset: normalizeRuntimeAnswer(asset.passage).indexOf(normalizeRuntimeAnswer(question.answer)),
  }));
  ...
  const locationHintIds = questions.filter(({ question }) =>
    /\b(?:paragraph|sentence|phrase|list|object|after|follows?|final|risk|appears?|find)\b/iu
...
export function assertUnitCardinality(assets) {
  ...
  'Sentence Completion: IDs o cardinalidad 3/18/18 inválidos.');
  return true;
}
...
export function expectedNextUnitPin() {
  ...
  const nextSets = catalog.IELTS_SUMMARY_COMPLETION_PASSAGES;
  assert(Array.isArray(nextSets) && nextSets.length === 3,
    'F0.2b.9 no conserva exactamente tres objetos de catálogo.');
  return {
    unit: 'F0.2b.9',
    format: 'summary-completion',
    sourceSha256: Object.fromEntries(NEXT_UNIT_PATHS.map(path => [path, sourceSha256(path)])),
    routeObjectSha256: sha256(stableJson(routeObject(catalog, 'summary-completion'))),
    sourceObjectSha256: Object.fromEntries(nextSets.map(set => [set.id, sha256(stableJson(set))])),
    scope: {
      passages: 3,
      questions: 18,
      renderDependencyClosurePaths: renderDependencyClosure([
        'src/app/layout.tsx', 'src/app/(site)/layout.tsx', NEXT_ROUTE_PATH,
      ]).length,
    },
    sharedDependencyBoundary: [ENGINE_PATH, BANK_PATH, CATALOG_PATH],
  };
}
...
export function assertNextUnitPinned(baseline) {
  ...
  assert(stableJson(baseline.nextUnit) === stableJson(expectedNextUnitPin()),
    'F0.2b.9 cambió o su pin route/engine/bank/objetos es incompleto.');
  return true;
}
...
export function assertBoardStopBoundary(loopDocText) {
  ...
  exactOnce(priorClosed, '↳ F0.2b.7 Matching Sentence Endings');
  exactOnce(currentClosed, '↳ F0.2b.8 Sentence Completion');
...
export function buildPromptOnlyPacket() {
  ...
  const packet = basePacket(
    'ielts-reading-sentence-completion-prompt-only.v1',
    'F0.2b.8 — grammar and sentence-frame shortcut audit over 18 open responses',
    'Use only sentence frames and word limits. Do not open passages, hints, keys, blind packets, registry or prior reviews.',
    ...
    assets.map(asset => ({
      ...
      sentenceFrames: asset.questions.map(question => ({
        questionId: question.id,
...
export function buildBlindReviewPacket() {
  ...
  assertUnitCardinality(assets);
  const packet = basePacket(
    'ielts-reading-sentence-completion-blind-review.v1',
    'F0.2b.8 — clean exact one-best-answer adjudication over three passages',
    ...
    'Complete every sentence with a contiguous span from its passage. Use no more than two words; a hyphenated expression counts as one word and contractions are not tested. The questions must follow passage order.',
    'Persist the completed sentence, exact passage span, evidence, closest plausible competing span and ambiguity. Do not use hints or outside knowledge.',
...
export function buildFactualSourceReviewPacketFromRegistry(registry, prerequisiteHashes) {
  ...
  const records = EXPECTED_ASSET_IDS.map(assetId => {
    ...
    assert(record.factualSourceResearch?.status === 'candidate-sources-collected' &&
      ...
      stableJson(record.factualSourceResearch.sourceEvidenceIds) ===
        stableJson(REQUIRED_FACTUAL_SOURCE_IDS[assetId]) &&
      typeof record.factualSourceResearch.limitation === 'string' &&
      record.factualSourceResearch.limitation.trim(),
    `${assetId}: se esperaban cinco fuentes factuales candidatas únicas.`);
    return {
      assetId,
  ...
  const packet = {
    schemaVersion: 'ielts-reading-sentence-completion-factual-source-review.v1',
    generatedAt: FACTUAL_PACKET_GENERATED_AT,
    reviewScope: 'F0.2b.8 — second-pass factual and accepted-spelling-policy review only',
    reviewerIsolation:
...
function validatePromptOnlyVerdict(verdict, assets) {
  ...
  assert(verdict.schemaVersion === 'ielts-reading-sentence-completion-prompt-only-verdict.v1',
  ...
  assert(verdict.reviewer.packetSha256 === sourceSha256(PROMPT_ONLY_PACKET_PATH) &&
...
function validateFirstPass(firstPass, assets) {
  ...
  assert(firstPass.schemaVersion === 'ielts-reading-sentence-completion-expert-first-pass.v1',
  ...
  assert(firstPass.reviewer.blindPacketSha256 === sourceSha256(BLIND_REVIEW_PATH) &&
...
function validateAcceptedAnswerAssessments(rows, assets) {
  ...
  assert(Array.isArray(rows) && rows.length === inventory.length,
    'acceptedAnswerAssessments: cardinalidad 20 inválida.');
  ...
  for (const row of rows) {
    ...
    assert(source && row.normalizedText === source.normalizedText &&
      ...
      ['verbatim', 'normalized-duplicate', 'official-spelling-variant', 'unsupported']
        .includes(row.passageStatus) &&
      ...
      row.evidenceIds.every(id => id === SPELLING_POLICY_EVIDENCE_ID) &&
    ...
    if (row.role === 'canonical') {
      assert(row.passageStatus === 'verbatim' &&
        ...
        normalizeRuntimeAnswer(context.asset.passage).includes(row.normalizedText),
      `${row.questionId}: la respuesta canónica dejó de ser literal.`);
    } else if (row.questionId === 'sentence-makerspaces-01') {
      assert(row.passageStatus === 'normalized-duplicate' && row.evidenceIds.length === 0,
        'sentence-makerspaces-01: 3d printers debe declararse duplicate normalizado.');
    } else if (row.questionId === 'sentence-night-markets-01') {
      assert(['official-spelling-variant', 'unsupported'].includes(row.passageStatus),
        'sentence-night-markets-01: estado de shopping center inválido.');
      assert(stableJson(row.evidenceIds) === stableJson([SPELLING_POLICY_EVIDENCE_ID]),
        'shopping center requiere exactamente la fuente oficial de spelling.');
    }
    statuses.push(row);
  }
  return statuses;
...
function validateExpertVerdict(verdict, firstPass, firstTrace, assets, factualPacket) {
  ...
  assert(verdict.schemaVersion === 'ielts-reading-sentence-completion-expert-verdict.v1',
  ...
  const expectedEvidenceIds = [SPELLING_POLICY_EVIDENCE_ID, ...factualPacket.records.flatMap(row =>
...
function validateStudentWalkthrough(walkthrough, assets) {
  ...
  assert(walkthrough.schemaVersion ===
    'ielts-reading-sentence-completion-student-walkthrough.v1',
  ...
  assert(walkthrough.reviewer.humanSignature === false &&
    ...
    walkthrough.reviewer.blindPacketSha256 === sourceSha256(BLIND_REVIEW_PATH) &&
...
function validateSourceAvailability(ledger, registry, unitRecords) {
  assertExactKeys(ledger,
    ...
    'sourceAvailability');
  assert(ledger.schemaVersion === 'ielts-reading-sentence-completion-source-availability.v1' &&
  ...
  const expectedIds = [OFFICIAL_EVIDENCE_ID, SPELLING_POLICY_EVIDENCE_ID,
  ...
  assert(ledger.coverageContract.officialSources === 2 &&
    ...
    ledger.coverageContract.factualSourcesPerAsset === 5 &&
    stableJson(ledger.coverageContract.officialEvidenceIds) ===
      stableJson([OFFICIAL_EVIDENCE_ID, SPELLING_POLICY_EVIDENCE_ID]) &&
  ...
  assertExactKeys(ledger.observedAvailability,
    ['http200', 'http403', 'redirected', 'requestedContentRetrieved',
...
function validateProvenanceSearch(ledger, registry) {
  assertExactKeys(ledger,
    ...
    'provenanceSearch');
  assert(ledger.schemaVersion === 'ielts-reading-sentence-completion-provenance-search.v1' &&
  ...
  for (const query of ledger.queries) {
    ...
    assert(typeof query.query === 'string' && query.query.trim() &&
      ...
      new Set(query.candidateFactualSourcesCompared).size === 5 &&
...
function validateAuditVerdicts(audit, facts = null) {
  assertExactKeys(audit, ['schemaVersion', 'reviewedAt', 'status', 'passMeaning', 'rows'],
    'auditVerdicts');
  assert(audit.schemaVersion === 'ielts-reading-sentence-completion-audit-verdicts.v1' &&
...
export function validateChronology(input) {
  ...
  assert(baselineAt <= retrievalStartedAt &&
    ...
    blindPacketAt <= walkthroughAt && factualPacketAt <= expertAt &&
...
function validateBaseline(baseline, assets, storedRisk, renderClosurePaths) {
  ...
  assert(baseline.schemaVersion === 'ielts-reading-sentence-completion-baseline.v1' &&
    baseline.scope.unit === 'F0.2b.8' && baseline.scope.format === 'sentence-completion' &&
  ...
  assertExactKeys(structural.answerSurfaceProfile,
    ['questions', 'oneWordPrimaryAnswers', 'twoWordPrimaryAnswers',
      ...
      'normalizedRedundantAlternativeEntries',
  ...
  assert(structural.answerSurfaceProfile.questions === 18 &&
    ...
    structural.answerSurfaceProfile.questionsWithAcceptedAlternativeEntry === 2 &&
    structural.answerSurfaceProfile.normalizedRedundantAlternativeEntries === 1 &&
    structural.answerSurfaceProfile.manuallyClassifiedNominalOrNounPhrasePrimaryAnswers === 16 &&
    structural.answerSurfaceProfile.manualClassificationCaveat.includes('reviewer judgment'),
  'baseline.answerSurfaceProfile contradice 18/20/19.');
  ...
  assertExactKeys(structural.shortcutSignals,
    ['questionsWhoseImmediateLeftPassageTokenMatchesPromptContext',
      'questionsWithHintsAvailableBeforeResponse', 'preResponseHintsWithAnchorOrLocationLanguage',
      'preResponseHintsExplicitlyProvidingParaphrase', 'questionsWithPostCheckExplanation',
  ...
  assert(structural.shortcutSignals.questionsWhoseImmediateLeftPassageTokenMatchesPromptContext ===
      13 && structural.shortcutSignals.questionsWithHintsAvailableBeforeResponse === 18 &&
    structural.shortcutSignals.preResponseHintsWithAnchorOrLocationLanguage === 17 &&
    structural.shortcutSignals.preResponseHintsExplicitlyProvidingParaphrase === 1 &&
  ...
  assertExactKeys(structural.normalizationContract,
    ['trimOuterWhitespace', 'lowercase', 'collapseInternalWhitespace',
      ...
      'britishAmericanSpellingOnlyWhenExplicitAlternativeExists'],
  ...
  assert(stableJson(structural.normalizationContract) === stableJson({
    ...
    britishAmericanSpellingOnlyWhenExplicitAlternativeExists: true,
  ...
  const expectedRuntime = {
    ...
    breadcrumbHasNavAndCurrentSemantics: false,
  ...
  const currentRouteHash = sha256(stableJson(routeObject(loadTsModule(CATALOG_PATH),
    'sentence-completion')));
  assert(baseline.routeObjectSha256 === currentRouteHash,
  ...
  assert(baseline.interpretation.includes('audit-only') &&
    baseline.interpretation.includes('F0.2b.9 Summary Completion') &&
...
function validateUnitChangeManifest(manifest, renderClosurePaths) {
  ...
  assert(manifest.schemaVersion ===
    'ielts-reading-sentence-completion-unit-change-manifest.v1' &&
    manifest.unit === 'F0.2b.8' &&
  ...
  assert(manifest.nextUnitBoundary.unit === 'F0.2b.9' &&
    manifest.nextUnitBoundary.format === 'summary-completion' &&
    manifest.nextUnitBoundary.status === 'not-started',
  'unitChangeManifest: frontera F0.2b.9 inválida.');
...
export function buildValidationArtifacts() {
  ...
  assert(evidenceById.get(OFFICIAL_EVIDENCE_ID)?.kind === 'official-policy' &&
    ...
    evidenceById.get(SPELLING_POLICY_EVIDENCE_ID)?.kind === 'official-policy' &&
    evidenceById.get(SPELLING_POLICY_EVIDENCE_ID)?.url === SPELLING_POLICY_URL,
  ...
  assert(findSentenceCompletionAssignmentLeaks(closureTexts).length === 0,
  ...
  const leakControls = {
    englishAnswer: findSentenceCompletionAssignmentLeaks({
      instruction: 'Answer for sentence-wetlands-01 is concrete channels',
    }),
    englishCompletion: findSentenceCompletionAssignmentLeaks({
      instruction: 'Complete sentence-makerspaces-02 with storing',
    }),
    spanishCompletion: findSentenceCompletionAssignmentLeaks({
      nested: { repair: 'sentence-wetlands-02 se completa con underground pipes' },
    }),
    spanishAnswer: findSentenceCompletionAssignmentLeaks({
      repair: 'La respuesta de sentence-night-markets-05 es longer',
    }),
    nestedField: findSentenceCompletionAssignmentLeaks({
  ...
  const validation = {
    schemaVersion: 'ielts-reading-sentence-completion-rights-validation.v1',
    generatedAt: audit.reviewedAt,
    unit: 'F0.2b.8 — three formative Sentence Completion passages',
    status: 'pass',
    passMeaning:
    ...
    checks: {
      exactCoverageThreeEighteenEighteenTwentyNineteen: true,
      ...
      officialFormatAndSpellingSourcesRetrieved:
        [OFFICIAL_EVIDENCE_ID, SPELLING_POLICY_EVIDENCE_ID].every(evidenceId =>
      ...
      threePacketsContainNoKeysAlternativesEditorialFeedbackHintsPiiOrAssignments:
        [promptOnlyPacket, blindPacket, factualPacket].every(packet =>
          ...
          findSentenceCompletionAssignmentLeaks(packet).length === 0 &&
      ...
      acceptedAnswerInventoryTwentyExact: expert.acceptedAssessments.length === 20,
      normalizedDuplicateAndNonVerbatimAlternativeDetected:
        stableJson(storedRisk.normalizedDuplicateAlternativeQuestionIds) ===
          stableJson(['sentence-makerspaces-01']) &&
        stableJson(storedRisk.nonVerbatimAcceptedAlternativeQuestionIds) ===
          stableJson(['sentence-night-markets-01']),
      ...
      multidimensionalShortcutAuditComplete:
        storedRisk.alwaysTwoWordAnswerPredictor.hits === 13 &&
        storedRisk.sameQuestionPositionAcrossSetsModalAnswerWordCount.hits === 14 &&
        storedRisk.canonicalAnswerTokenVisibleInSentenceFrame.questionsWithOverlap === 1,
  ...
  const failed = Object.entries(validation.checks).filter(([, passed]) => !passed)
    .map(([name]) => name);
  assert(failed.length === 0, `Falló gate F0.2b.8: ${failed.join(', ')}`);
...
export function validateFinalReportArtifacts(validation) {
  ...
  assert(artifact.surface === 'report' && artifact.manifest?.surface === 'report' &&
    artifact.manifest.title === 'IELTS Reading Sentence Completion — audit gate' &&
  ...
  assert(html.includes('IELTS Reading Sentence Completion') &&
  ...
  assert(verification.schemaVersion ===
    'ielts-reading-sentence-completion-report-verification.v1' &&
