...
check('guided-mock-registry', 'Los 20 mocks publicados tienen una fuente única', GUIDED_MOCK_IDS.length === 20 && new Set(GUIDED_MOCK_IDS).size === 20 && !GUIDED_MOCK_IDS.some((id) => ['mock-21', 'mock-22', 'mock-23'].includes(id)), 'critical', `${GUIDED_MOCK_IDS.length}/20 IDs únicos; 21–23 retenidos`, 'Corregir guided-registry.ts antes de exponer rutas.');
...
check('primary-hub-intent', 'El hub de exámenes concentra la intención simulacro ICFES Inglés', examHubPage.includes('Simulacro ICFES Inglés gratis: ${icfesResources} recursos Saber 11') && examHubPage.includes('exam.mocks.length + 1') && examHero.includes('Simulacro ICFES Inglés: practica gratis para Saber 11'), 'critical', 'Title dinámico desde catálogo y H1 específico en /examenes/icfes', 'Mantener la consulta principal en el hub de exámenes.');
...
check('inventory-language', 'La portada diferencia recursos únicos de modos', mockGrid.includes('{icfesUniqueResources} recursos distintos') && mockGrid.includes('{icfesModes} rutas o modos de uso, pero no {icfesModes} exámenes diferentes') && examGuide.includes('Por qué ves 56 rutas, pero solo 31 recursos'), 'critical', '31 recursos = 20 propios + 10 atribuidos + 1 guiado; 56 modos; 3 borradores retenidos', 'No contar modos como exámenes distintos.');
...
check('intent-separation', 'Aprendizaje, bancos históricos y tutoría conservan intención propia', hub.includes('Cómo resolver las 7 partes de Inglés Saber 11') && workbookCatalogPage.includes('Muestras históricas ICFES Inglés: catálogo atribuido') && preparationPage.includes('Preparación ICFES Inglés con tutor | WeLearn'), 'high', 'Tres titles distintos de la consulta principal', 'Evitar que las páginas satélite copien el title del hub.');
