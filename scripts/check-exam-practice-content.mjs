...
function validateIeltsMultipleChoiceProgressContract() {
  ...
  if (!rootLayoutText.includes("process.env.NODE_ENV === 'production' && ENTORNO === 'production'")) {
    fail('Root analytics must remain limited to the canonical Vercel production environment.');
  }
}

function validateIeltsTfngProgressContract() {
  const dataPath = path.join(root, 'src/data/practica-exams/ielts-reading-tfng-progress.ts');
  const componentPath = path.join(root, 'src/components/exam-practice/TfngPracticeLab.tsx');
  const publicTemplatePath = path.join(root, 'src/app/(site)/practica/ielts/reading/international-question-type/[slug]/page.tsx');
  if (!fs.existsSync(dataPath)) fail('IELTS True False Not Given progress data is missing.');
  if (!fs.existsSync(componentPath)) fail('IELTS True False Not Given practice lab is missing.');
  if (!fs.existsSync(dataPath) || !fs.existsSync(componentPath)) return;
  const dataText = fs.readFileSync(dataPath, 'utf8'); const componentText = fs.readFileSync(componentPath, 'utf8'); const publicTemplateText = fs.readFileSync(publicTemplatePath, 'utf8');
  for (const requiredText of ['TFNG_STORAGE_KEY', 'TFNG_LEGACY_STORAGE_KEY', 'TFNG_LEVELS', 'TFNG_OPTIONS', "'paraphrase-missed'", "'opposite-not-proved'", "'silence-misclassified'", "'quantifier-shift'", "'time-or-degree-shift'", "'outside-knowledge'"]) if (!dataText.includes(requiredText)) fail(`IELTS True False Not Given progress data must include "${requiredText}".`);
  for (const requiredText of ['type="radio"', 'localStorage', 'Press again to reset', 'feedback closed', 'WeLearn Progress Engine', 'not an IELTS band', 'not a secure Practice, Exam or proctored mode']) if (!componentText.includes(requiredText)) fail(`IELTS True False Not Given practice lab must include "${requiredText}".`);
  for (const requiredText of ['isTfng', 'TfngGuidedPractice', 'TfngIndependentPractice', 'TfngProgressEngine']) if (!publicTemplateText.includes(requiredText)) fail(`IELTS public question-type rewrite template must include "${requiredText}".`);
}
...
function validateRoute(route, index, routeMapText) {
  ...
  if (isIeltsReadingQuestionType && route.status === 'published') {
    ...
    if (!progressiveQuestionType && !pageText.includes('Formato oficial vs estrategia WeLearn') && !pageText.includes('OfficialStrategyCard')) {
  ...
  if (route.path === '/practica/ielts/reading/tipos-de-preguntas/true-false-not-given' && route.status === 'published') {
    const pagePath = routeToPagePath(route.path);
    const pageText = fs.existsSync(pagePath) ? fs.readFileSync(pagePath, 'utf8') : '';
    for (const requiredText of ['TfngGuidedPractice', 'TfngIndependentPractice', 'TfngProgressEngine', 'TFNG_GUIDED_PASSAGE_ID', 'TFNG_INDEPENDENT_PASSAGE_ID']) if (!pageText.includes(requiredText)) fail(`${label} must include ${requiredText}.`);
  }
  ...
  if (route.path === '/practica/ielts/reading/tipos-de-preguntas/sentence-completion' && route.status === 'published') {
    const pagePath = routeToPagePath(route.path);
    const pageText = fs.existsSync(pagePath) ? fs.readFileSync(pagePath, 'utf8') : '';
    for (const requiredText of ['SentenceCompletionGuidedPractice', 'SentenceCompletionIndependentPractice', 'SentenceCompletionProgressEngine', 'SENTENCE_COMPLETION_GUIDED_PASSAGE_ID', 'SENTENCE_COMPLETION_INDEPENDENT_PASSAGE_ID']) {
      if (!pageText.includes(requiredText)) fail(`${label} must include ${requiredText}.`);
    }
  }
  ...
  if (route.path === '/practica/ielts/reading/tipos-de-preguntas/summary-completion' && route.status === 'published') {
    const pagePath = routeToPagePath(route.path);
    const pageText = fs.existsSync(pagePath) ? fs.readFileSync(pagePath, 'utf8') : '';
    for (const requiredText of ['SummaryCompletionGuidedPractice', 'SummaryCompletionIndependentPractice', 'SummaryCompletionProgressEngine', 'SUMMARY_COMPLETION_GUIDED_PASSAGE_ID', 'SUMMARY_COMPLETION_INDEPENDENT_PASSAGE_ID']) {
      if (!pageText.includes(requiredText)) fail(`${label} must include ${requiredText}.`);
    }
  }
  ...
  if (route.path === '/practica/ielts/reading/tipos-de-preguntas/table-completion' && route.status === 'published') {
    const pagePath = routeToPagePath(route.path);
    const pageText = fs.existsSync(pagePath) ? fs.readFileSync(pagePath, 'utf8') : '';
    for (const requiredText of ['TableCompletionGuidedPractice', 'TableCompletionIndependentPractice', 'TableCompletionProgressEngine', 'TABLE_COMPLETION_GUIDED_PASSAGE_ID', 'TABLE_COMPLETION_INDEPENDENT_PASSAGE_ID']) {
      if (!pageText.includes(requiredText)) fail(`${label} must include ${requiredText}.`);
    }
  }
  ...
  if (route.path === '/practica/ielts/reading/tipos-de-preguntas/diagram-labeling' && route.status === 'published') {
    ...
    for (const requiredText of ['DiagramLabelingGuidedPractice', 'DiagramLabelingIndependentPractice', 'DiagramLabelingProgressEngine', 'DIAGRAM_LABELING_GUIDED_PASSAGE_ID', 'DIAGRAM_LABELING_INDEPENDENT_PASSAGE_ID']) {
      if (!pageText.includes(requiredText)) fail(`${label} must include ${requiredText}.`);
      if (!publicRewriteText.includes(requiredText)) fail(`${label} public rewrite must include ${requiredText}.`);
    }
    if (!publicRewriteText.includes("isDiagramLabeling = slug === 'diagram-labeling'")) fail(`${label} public rewrite must select the progressive Diagram Labeling experience.`);
  }
  ...
  if (route.path === '/practica/ielts/reading/tipos-de-preguntas/matching-headings' && route.status === 'published') {
    const pagePath = routeToPagePath(route.path);
    const pageText = fs.existsSync(pagePath) ? fs.readFileSync(pagePath, 'utf8') : '';
    for (const requiredText of ['MatchingHeadingsGuidedPractice', 'MatchingHeadingsIndependentPractice', 'MatchingHeadingsProgressEngine', 'MATCHING_HEADINGS_GUIDED_PASSAGE_ID', 'MATCHING_HEADINGS_INDEPENDENT_PASSAGE_ID']) {
      if (!pageText.includes(requiredText)) fail(`${label} must include ${requiredText}.`);
    }
  }
  ...
  if (route.path === '/practica/ielts/reading/tipos-de-preguntas/matching-information' && route.status === 'published') {
    const pagePath = routeToPagePath(route.path);
    const pageText = fs.existsSync(pagePath) ? fs.readFileSync(pagePath, 'utf8') : '';
    for (const requiredText of ['MatchingInformationGuidedPractice', 'MatchingInformationIndependentPractice', 'MatchingInformationProgressEngine', 'MATCHING_INFORMATION_GUIDED_PASSAGE_ID', 'MATCHING_INFORMATION_INDEPENDENT_PASSAGE_ID']) {
      if (!pageText.includes(requiredText)) fail(`${label} must include ${requiredText}.`);
    }
  }
  ...
  if (route.path === '/practica/ielts/reading/tipos-de-preguntas/matching-sentence-endings' && route.status === 'published') {
    const pagePath = routeToPagePath(route.path);
    const pageText = fs.existsSync(pagePath) ? fs.readFileSync(pagePath, 'utf8') : '';
    for (const requiredText of ['MatchingSentenceEndingsGuidedPractice', 'MatchingSentenceEndingsIndependentPractice', 'MatchingSentenceEndingsProgressEngine', 'SENTENCE_ENDINGS_GUIDED_PASSAGE_ID', 'SENTENCE_ENDINGS_INDEPENDENT_PASSAGE_ID']) {
      if (!pageText.includes(requiredText)) fail(`${label} must include ${requiredText}.`);
    }
  }
}
...
function main() {
  ...
  validateIeltsTfngProgressContract();
