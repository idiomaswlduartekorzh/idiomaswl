...
const PUBLIC_FEEDBACK_LABEL = 'feedback pedagógico personalizado de WeLearn con asistencia de IA';
const REQUIRED_GATES = ['editorial', 'seo', 'geo-aeo-ai', 'pricing', 'security', 'privacy', 'payments', 'teacher-ops', 'release'];
const REQUIRED_GOVERNANCE_ARTIFACTS = [
  'config/icfes-launch-harness/approval.schema.json',
  'config/icfes-launch-harness/campaign.json',
  'config/icfes-launch-harness/candidate-ledger.schema.json',
  'config/icfes-launch-harness/policy.json',
  'config/icfes-launch-harness/release-manifest.schema.json',
  ...
  'docs/ICFES-OWN-MOCK-EXPANSION-PIPELINE.md',
  ...
  'tests/icfes-own-expansion-v2.test.mjs',
...
export function hasMergeConflictMarkers(value) {
  const text = Buffer.isBuffer(value) ? value.toString('utf8') : String(value);
  return /^(<<<<<<< |=======|>>>>>>> )/m.test(text);
}

export function parseNullSeparatedPaths(value) {
  return [...new Set(String(value).split('\0').filter(Boolean))];
}

export function listUnmergedPaths(repoRoot) {
  const result = spawnSync('git', ['-C', repoRoot, 'diff', '--name-only', '--diff-filter=U', '-z'], { encoding: 'utf8' });
  if (result.status !== 0) {
    const detail = result.stderr.trim() || `git terminó con código ${result.status}`;
    throw new Error(`No se pudo comprobar el índice antes de regenerar digests: ${detail}`);
  }
...
function validatePolicy(policy, failures) {
  ...
  push(JSON.stringify(policy.gateOrder) === JSON.stringify(REQUIRED_GATES), failures, 'policy: gateOrder debe contener los nueve gates contractuales en orden');
  for (const gate of REQUIRED_GATES) {
    const suffix = gate.toUpperCase().replaceAll('-', '_');
    push(policy.states.includes(`FAIL_${suffix}`), failures, `policy: falta estado FAIL_${suffix}`);
    push(policy.states.includes(`BLOCKED_${suffix}`), failures, `policy: falta estado BLOCKED_${suffix}`);
  }
  ...
  const expectedTiers = [
    ['icfes-detail-attempt-v1', 12900, null, 0],
    ['exam-auto', 49900, 30, 0],
    ['exam-teacher', 99900, 30, 1],
  ];
  ...
  const assistedFeedbackTier = tierBySku.get('exam-teacher');
  push(assistedFeedbackTier?.humanReviewSlaHours === 12, failures, 'policy: el feedback asistido debe fijar el objetivo operativo de 12h');
  push(assistedFeedbackTier?.feedbackCreditsPerPeriod === 1, failures, 'policy: el tier de 99,9k debe incluir un único crédito de feedback por periodo');
  push(assistedFeedbackTier?.publicBenefitLabel === PUBLIC_FEEDBACK_LABEL, failures, 'policy: el beneficio público de 99,9k debe usar el nombre aprobado');
  push(assistedFeedbackTier?.aiAssistanceDisclosureRequired === true, failures, 'policy: el beneficio de 99,9k debe revelar la asistencia de IA');
  push(assistedFeedbackTier?.humanApprovalEvidenceRequiredBeforeDelivery === true, failures, 'policy: el feedback asistido exige evidencia de aprobación humana antes de entregar');
  push(assistedFeedbackTier?.publicTeacherAuthorshipClaimForbidden === true, failures, 'policy: el copy público no puede atribuir autoría docente');
  push(assistedFeedbackTier?.deliveryTargetConditionalOnReservedCapacity === true, failures, 'policy: el objetivo de 12h debe depender de capacidad reservada');
  push(assistedFeedbackTier?.deliveryTargetIsGuarantee === false, failures, 'policy: el objetivo de 12h no puede representarse como garantía');
  push(policy.teacherOps?.queueIntakeRequired === true, failures, 'policy: toda solicitud de feedback debe entrar a la bandeja');
  push(policy.teacherOps?.weLearnNotificationRequired === true, failures, 'policy: toda solicitud debe notificar a WeLearn');
  push(policy.teacherOps?.humanApprovalRequiredBeforeStudentDelivery === true, failures, 'policy: la entrega al estudiante exige aprobación humana');
  push(policy.teacherOps?.humanApprovalEvidenceRequired === true, failures, 'policy: la aprobación humana debe dejar evidencia');
  push(policy.teacherOps?.aiAssistanceDisclosureRequired === true, failures, 'policy: teacher-ops debe conservar el disclosure de IA');
  push(policy.teacherOps?.publicTeacherAuthorshipClaimForbidden === true, failures, 'policy: teacher-ops debe prohibir el claim de autoría docente');
  push(policy.teacherOps?.slaConditionalOnReservedCapacity === true && policy.teacherOps?.slaIsGuarantee === false,
    failures, 'policy: las 12h son un objetivo condicional y no una garantía');
  ...
  push(!upgrades.has('icfes-detail-attempt-v1->exam-auto'), failures, 'policy: no debe inventar crédito 12,9k→49,9k');
  push(!upgrades.has('icfes-detail-attempt-v1->exam-teacher'), failures, 'policy: no debe inventar crédito 12,9k→99,9k');
  push(upgrades.get('exam-auto->exam-teacher')?.creditCop === 49900
    && upgrades.get('exam-auto->exam-teacher')?.payableCop === 50000,
  failures, 'policy: upgrade 49,9k→99,9k debe reconocer 49,9k y cobrar 50k');
  const discovery = policy.discovery;
  push(discovery?.gateId === 'geo-aeo-ai', failures, 'policy: falta contrato GEO/AEO/IA');
  push(discovery?.publicEntryRoute === '/examenes/icfes', failures, 'policy: GEO/AEO/IA debe conservar un único hub público');
  push(discovery?.publicDemoRoute === '/practica/icfes-saber-11/simulacro-guiado', failures, 'policy: GEO/AEO/IA debe conservar una demo pública explícita');
  push(discovery?.requireVisibleAnswerFirstSummary === true, failures, 'policy: GEO/AEO/IA exige resumen visible y directo');
  push(discovery?.requireSourceAndMethodDisclosure === true, failures, 'policy: GEO/AEO/IA exige fuente y método visibles');
  push(discovery?.requireStructuredDataParity === true, failures, 'policy: GEO/AEO/IA exige paridad entre datos estructurados y contenido visible');
  push(discovery?.forbidPrivateAnswerRetrieval === true, failures, 'policy: GEO/AEO/IA debe excluir respuestas privadas');
  push(discovery?.forbidRankingOrCitationGuarantees === true, failures, 'policy: GEO/AEO/IA no puede prometer ranking ni citación');
  push(discovery?.measurementMode === 'observed-only', failures, 'policy: GEO/AEO/IA solo acepta resultados observados');
...
function validateWorkOrders(harness, failures) {
  ...
  for (const order of workOrders) {
    ...
    const inputPaths = new Set(order.inputArtifacts.map(({ path: artifactPath }) => artifactPath));
    for (const requiredPath of REQUIRED_GOVERNANCE_ARTIFACTS) {
      push(inputPaths.has(requiredPath), failures, `${order.workOrderId}: falta artefacto de gobernanza ${requiredPath}`);
    }
    ...
    for (const tier of policy.commercialModel.tiers) {
      ...
      push(orderTier?.priceCop === tier.priceCop
        ...
        && orderTier?.feedbackCreditsPerPeriod === tier.feedbackCreditsPerPeriod
        && orderTier?.publicBenefitLabel === tier.publicBenefitLabel
        && orderTier?.aiAssistanceDisclosureRequired === tier.aiAssistanceDisclosureRequired
        && orderTier?.humanApprovalEvidenceRequiredBeforeDelivery === tier.humanApprovalEvidenceRequiredBeforeDelivery
        && orderTier?.publicTeacherAuthorshipClaimForbidden === tier.publicTeacherAuthorshipClaimForbidden
        && orderTier?.deliveryTargetConditionalOnReservedCapacity === tier.deliveryTargetConditionalOnReservedCapacity
        && orderTier?.deliveryTargetIsGuarantee === tier.deliveryTargetIsGuarantee,
...
function validateCandidates(harness, failures) {
  ...
  for (const candidate of candidateLedger.candidates) {
    ...
    const subjectPaths = new Set(candidate.subjectArtifacts.map(({ path: artifactPath }) => artifactPath));
    for (const requiredPath of REQUIRED_GOVERNANCE_ARTIFACTS) {
      push(subjectPaths.has(requiredPath), failures, `${candidate.candidateId}: falta subject de gobernanza ${requiredPath}`);
    }
...
export function refreshHarnessDigests(repoRoot) {
  const root = 'config/icfes-launch-harness';
  const unmergedPaths = listUnmergedPaths(repoRoot);
  if (unmergedPaths.length > 0) {
    throw new Error(`No se regeneran digests con rutas sin resolver en el índice: ${unmergedPaths.join(', ')}.`);
  }
  const campaign = readJson(repoRoot, `${root}/campaign.json`);
  const approvals = readJson(repoRoot, `${root}/approvals.json`);
  const releaseManifest = readJson(repoRoot, `${root}/release-manifest.json`);
  ...
  const sourceWorkOrders = campaign.workOrders.map((relativePath) => ({ relativePath, order: readJson(repoRoot, relativePath) }));
  const ledgerPath = `${root}/candidate-ledger.json`;
  const ledger = readJson(repoRoot, ledgerPath);
  const artifactPaths = new Set([
    ...sourceWorkOrders.flatMap(({ order }) => order.inputArtifacts.map(({ path: artifactPath }) => artifactPath)),
    ...ledger.candidates.flatMap((candidate) => candidate.subjectArtifacts.map(({ path: artifactPath }) => artifactPath)),
  ]);
  for (const artifactPath of artifactPaths) {
    const absolute = path.join(repoRoot, artifactPath);
    if (!existsSync(absolute)) throw new Error(`No se regeneran digests: falta ${artifactPath}.`);
    if (hasMergeConflictMarkers(readFileSync(absolute))) {
      throw new Error(`No se regeneran digests sobre conflictos sin resolver: ${artifactPath}.`);
    }
  }

  const workOrders = sourceWorkOrders.map(({ relativePath, order }) => {
...
export function inventory(harness) {
  return {
    ...
    targetTiers: harness.policy.commercialModel.tiers.map(({
      sku, priceCop, durationDays, humanReviewCredits, humanReviewSlaHours, feedbackCreditsPerPeriod,
      publicBenefitLabel, aiAssistanceDisclosureRequired, humanApprovalEvidenceRequiredBeforeDelivery,
      publicTeacherAuthorshipClaimForbidden, deliveryTargetConditionalOnReservedCapacity, deliveryTargetIsGuarantee,
    }) => ({
      sku, priceCop, durationDays, humanReviewCredits,
      ...(humanReviewSlaHours ? { humanReviewSlaHours } : {}),
      ...(feedbackCreditsPerPeriod ? { feedbackCreditsPerPeriod } : {}),
      ...(publicBenefitLabel ? { publicBenefitLabel } : {}),
      ...(aiAssistanceDisclosureRequired ? { aiAssistanceDisclosureRequired } : {}),
      ...(humanApprovalEvidenceRequiredBeforeDelivery ? { humanApprovalEvidenceRequiredBeforeDelivery } : {}),
      ...(publicTeacherAuthorshipClaimForbidden ? { publicTeacherAuthorshipClaimForbidden } : {}),
      ...(deliveryTargetConditionalOnReservedCapacity ? { deliveryTargetConditionalOnReservedCapacity } : {}),
      ...(deliveryTargetIsGuarantee === false ? { deliveryTargetIsGuarantee } : {}),
    })),
