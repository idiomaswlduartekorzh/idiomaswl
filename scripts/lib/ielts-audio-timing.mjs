...
const bundledBin = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../output/tools/bin');
export function mediaBinary(name) {
  const configured = process.env.IELTS_FFMPEG_DIR ? path.join(process.env.IELTS_FFMPEG_DIR, name) : null;
  if (configured && existsSync(configured)) return configured;
  const bundled = path.join(bundledBin, name);
...
export function analyzeAudioTiming(filePath, thresholds = IELTS_AUDIO_TIMING_THRESHOLDS) {
  ...
  const silence = spawnSync(mediaBinary('ffmpeg'), [
