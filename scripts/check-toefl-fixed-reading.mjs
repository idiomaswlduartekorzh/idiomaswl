...
const [publicSource, privateSource, enhancerSource, readingRegistrySource, ctwRegistrySource] = await Promise.all([
...
assert.equal(TOEFL_READING_MODULE2_SETS_1_TO_5.length, 5, 'batch has Sets 1–5');
assert.doesNotMatch(publicSource, /correctOptionIds|expectedMissing|\banswer\s*:/, 'public Module 2 data contains no answer key');
assert.match(privateSource, /import 'server-only'/, 'Module 2 keys have an explicit server-only boundary');
assert.match(readingRegistrySource, /\.\.\.module1\.items, \.\.\.module2\.items/, 'Reading scoring combines both modules');
assert.match(ctwRegistrySource, /TOEFL_CTW_MODULE2_SCORING_BY_OBJECT_ID/, 'CTW registry includes Module 2 keys');
...
const keyBlock = privateSource.slice(privateSource.indexOf('const CTW_KEYS'), privateSource.indexOf('// Five Read'));
...
const allIds = [];
for (const set of TOEFL_READING_MODULE2_SETS_1_TO_5) {
...
console.log('✓ TOEFL fixed Reading Module 2: Sets 1–5 have 20 interactions, private keys, official-family shape, and no audio changes');
