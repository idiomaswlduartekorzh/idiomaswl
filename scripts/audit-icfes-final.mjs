...
const productE2e = JSON.parse(readFileSync('artifacts/icfes-audit/icfes-product-e2e-2026-09-08.json', 'utf8'));
...
check('inventory-scope', 'Inventario cubre el alcance completo', inventory.ownMocks === 23 && inventory.attributedSaber11 === 7 && inventory.relatedIcfes === 3 && inventory.standaloneGuided === 1 && inventory.allInventoryResources === 34, 'critical', `23 propios · 7 atribuidos Saber 11 · 3 relacionados · 1 guiado autónomo · 34 total`, 'Reconciliar catálogo, archivos y rutas.');
...
check('seo-gate', 'SEO/producto no tiene hallazgos abiertos', reports.seo.findings.length === 0 && reports.seo.scope.checks === 30, 'high', '30/30 controles', 'Corregir canonical, sitemap, metadata o accesibilidad.');
...
check('regression-gate', 'La compuerta principal encadena todas las auditorías sin servidor', packageJson.scripts['check:icfes-superhub'].includes('check:icfes-expansion') && packageJson.scripts['check:icfes-superhub'].includes('audit-icfes-100-users.mjs') && packageJson.scripts['check:icfes-superhub'].includes('audit-icfes-seo-product.mjs'), 'high', 'Expansión, SEO y cohorte fuente incluidas', 'Restaurar auditorías en check:icfes-superhub.');
check('documentation-current', 'Documentación refleja cinco muestras y 145 explicaciones', documentation.includes('cinco muestras históricas atribuidas a Saber 11') && documentation.includes('145 preguntas') && documentation.includes('4 de agosto de 2026'), 'medium', 'Alcance y fecha actualizados', 'Actualizar el handoff editorial.');
const browserFindings = Array.isArray(productE2e.findings) ? productE2e.findings : [];
const browserDevices = new Set(browserFindings.map((finding) => finding.device));
const browserFailures = browserFindings.filter((finding) => finding.consoleErrors?.length || finding.sensitiveScoringKeys?.length);
check('browser-e2e-gate', 'El resultado gratuito funciona en escritorio y móvil sin errores ni filtración de scoring', productE2e.ok === true && browserDevices.has('desktop') && browserDevices.has('mobile') && browserFailures.length === 0, 'critical', `${browserFindings.length}/2 dispositivos · ${browserFailures.length} fallos`, 'Repetir capture-icfes-product-evidence.mjs y corregir el flujo o el DTO.');
...
const report = {
  ...
  scope: {
    ...
    guidedAttributedEligible: 5,
    guidedAttributedExcluded: 2,
    syntheticStudents: 100,
    renderedRoutes200: 48,
    deliberateRoutes404: 2,
  },
  browserEvidence: browserFindings.map((finding) => ({
    check: `free-result-${finding.device}`,
    result: 'passed',
    evidence: `${finding.device} ${finding.viewport.width}×${finding.viewport.height}: resultado gratuito visible, cero errores de consola y cero claves sensibles de scoring.`,
