...
import { HYPERGAMY_AUDIO_TRACKS } from '../src/data/practica/advanced-hypergamy-audio.ts'
...
assert.equal(HYPERGAMY_AUDIO_TRACKS.length, 2, 'the hypergamy listening pair must contain exactly two tracks')

const jobs = HYPERGAMY_AUDIO_TRACKS.map((track) => ({
  ...track,
...
console.log('Hypergamy dual listening budget')
...
if (!generate) {
  console.log('\nDry run only. Add --generate to make the two approved paid API requests.')
  process.exit(0)
}
