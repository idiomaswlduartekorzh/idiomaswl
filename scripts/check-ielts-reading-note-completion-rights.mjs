...
const BLIND_PACKETS_GENERATED_AT = '2026-08-11T16:08:56Z';
// Move this only once, after the persisted clean first pass exists; chronology fails closed.
const FACTUAL_PACKET_GENERATED_AT = '2026-08-11T16:15:28Z';
...
const ENGINE_PATH = 'src/components/exam-practice/NoteCompletionEngine.tsx';
const BANK_PATH = 'src/components/exam-practice/NoteCompletionPassageBank.tsx';
const NEXT_ROUTE_PATH =
  'src/app/(site)/practica/ielts/reading/tipos-de-preguntas/table-completion/page.tsx';
const NEXT_ENGINE_PATH = 'src/components/exam-practice/TableCompletionEngine.tsx';
const NEXT_BANK_PATH = 'src/components/exam-practice/TableCompletionPassageBank.tsx';
...
const VALIDATOR_PATH = 'scripts/check-ielts-reading-note-completion-rights.mjs';
const TEST_PATH = 'tests/ielts-reading-note-completion-rights.test.mjs';
const LOOP_DOC_PATH = 'docs/ielts-reading-loop.md';
const OUTPUT_DIRECTORY =
  'output/audits/ielts-reading-rights-note-completion-2026-08-11';
...
// self-updated verification receipt from blessing manually rewritten fallback HTML.
const EXPECTED_REPORT_HTML_SHA256 =
  '0616603e5ccc0921506395c1e6809383c0cbea347c7a3e90ee91396845eda1c4';
...
const EXPECTED_ASSET_IDS = EXPECTED_SET_IDS.map(id => `formative:note-completion:${id}`);
const EXPECTED_QUESTION_IDS = [
  ...Array.from({ length: 6 }, (_, index) =>
    `note-mobile-libraries-${String(index + 1).padStart(2, '0')}`),
  ...Array.from({ length: 6 }, (_, index) =>
    `note-seed-banks-${String(index + 1).padStart(2, '0')}`),
  ...Array.from({ length: 6 }, (_, index) =>
    `note-night-libraries-${String(index + 1).padStart(2, '0')}`),
];
...
// F0.2b.10 adds records under the unchanged v8 policy/schema.
const EXPECTED_POLICY_VERSION = '2026-08-09.v8';
const EXPECTED_PASSAGE_SHA256 = {
  'note-mobile-libraries': 'c22dfef22bc06bd72409d3952715faee8ade98a21f9a019384b0a3c46b6fb343',
  'note-seed-banks': 'e754e89bcb82bc8f9655028851c8696014be32fcbcd8007f6faf2b245ee38709',
  'note-night-libraries': 'c7d601de88b071a6061fa7b20ebb292656934f2e1808f0a9d69809a1f16df218',
};
...
const EXPECTED_INTENDED_ANSWER_OFFSETS = {
  'note-mobile-libraries-01': 138,
  'note-mobile-libraries-02': 289,
  'note-mobile-libraries-03': 442,
  'note-mobile-libraries-04': 641,
  'note-mobile-libraries-05': 1029,
  'note-mobile-libraries-06': 1382,
  'note-seed-banks-01': 99,
  'note-seed-banks-02': 317,
  'note-seed-banks-03': 496,
  'note-seed-banks-04': 742,
  'note-seed-banks-05': 822,
  'note-seed-banks-06': 1320,
  'note-night-libraries-01': 124,
  'note-night-libraries-02': 254,
  'note-night-libraries-03': 501,
  'note-night-libraries-04': 721,
  'note-night-libraries-05': 899,
  'note-night-libraries-06': 1179,
};
const EXPECTED_INTENDED_ANSWER_PARAGRAPHS = {
  'note-mobile-libraries-01': 1, 'note-mobile-libraries-02': 1,
  'note-mobile-libraries-03': 2, 'note-mobile-libraries-04': 2,
  'note-mobile-libraries-05': 3, 'note-mobile-libraries-06': 4,
  'note-seed-banks-01': 1, 'note-seed-banks-02': 1,
  'note-seed-banks-03': 2, 'note-seed-banks-04': 2,
  'note-seed-banks-05': 3, 'note-seed-banks-06': 4,
  'note-night-libraries-01': 1, 'note-night-libraries-02': 1,
  'note-night-libraries-03': 2, 'note-night-libraries-04': 2,
  'note-night-libraries-05': 3, 'note-night-libraries-06': 4,
};
const OFFICIAL_EVIDENCE_ID = 'ielts-note-completion-official-format';
const SAMPLE_TASKS_EVIDENCE_ID = 'ielts-note-completion-official-samples';
...
const EXPECTED_RENDER_CLOSURE_SHA256 = {
  ...
  [ROUTE_PATH]: 'fa2f8f73b4ea5e407fa97ee1d5b5946752dddb84dbbba0337e9a1c339446678b',
  ...
  [BANK_PATH]: 'e553cf1f0b2fa30eafedf3d94ed4686dd9008a772c8a88cd23bd33e22b23ebf4',
  [ENGINE_PATH]: 'd032c0726e0cbf1f0426a52ffded5c3dff0582d58a6a4af636587004ccd000a5',
...
const EXPECTED_NEXT_UNIT_SOURCE_SHA256 = {
  [NEXT_ROUTE_PATH]: '9aa433566f10715f42412c11d7f5e645894a892f0edb9d054b3606764db3bff2',
  [NEXT_ENGINE_PATH]: 'c224f7cff507efecbe6753475ddd98bbf31a1744250bc414a56c5a5494ccfb7b',
  [NEXT_BANK_PATH]: '7cca09dc031a7377e92d0a6d481767558a87e4cd64b526f45254e448e71e929b',
...
export const REQUIRED_FACTUAL_CLAIM_SPANS = {
  'formative:note-completion:note-mobile-libraries': [
    'mobile libraries have become a practical way to keep reading services available when permanent branches are too expensive to maintain.',
    'vans carry books, magazines and digital materials to schools, markets and community halls on a regular schedule.',
    'In farming areas, visits are often arranged after market days',
    'a predictable timetable matters as much as the size of the collection',
    'Fuel, vehicle repairs and trained staff all create costs that may rise faster than local budgets.',
  ],
  'formative:note-completion:note-seed-banks': [
    'farmers can borrow, grow and return after harvest',
    'Many of these varieties have been selected over generations for taste, drought tolerance or resistance to local pests.',
    'Volunteers record the name of each seed, the place where it was collected and the conditions in which it usually grows well.',
    'Seeds must be kept dry, cool and protected from insects.',
    'Bank managers also test older seeds by planting a small sample; if too few germinate, the variety must be multiplied before the supply disappears.',
  ],
  'formative:note-completion:note-night-libraries': [
    'opening selected rooms in the evening for students who do not have quiet places to work at home.',
    "visitors can use desks, power sockets and wireless internet, but borrowing counters, children's areas and archives remain closed.",
    'Libraries usually start with a pilot programme before extending evening access.',
    'A small pilot also reveals practical issues, such as poor lighting outside the building or a shortage of cleaning staff after closing time.',
    'Younger students may need parental permission',
  ],
};
...
const KNOWN_ACCEPTED_ANSWERS = {
  'note-mobile-libraries-01': ['expensive'],
  'note-mobile-libraries-02': ['community halls'],
  'note-mobile-libraries-03': ['market days'],
  'note-mobile-libraries-04': ['timetable'],
  'note-mobile-libraries-05': ['public offices'],
  'note-mobile-libraries-06': ['volunteer groups'],
  'note-seed-banks-01': ['harvest'],
  'note-seed-banks-02': ['drought'],
  'note-seed-banks-03': ['conditions'],
  'note-seed-banks-04': ['identify'],
  'note-seed-banks-05': ['cool'],
  'note-seed-banks-06': ['beans'],
  'note-night-libraries-01': ['home'],
  'note-night-libraries-02': ['wireless internet'],
  'note-night-libraries-03': ['security'],
  'note-night-libraries-04': ['building'],
  'note-night-libraries-05': ['areas'],
  'note-night-libraries-06': ['permission'],
};
...
const QUESTION_ID_PATTERN = /\bnote-(?:mobile-libraries|seed-banks|night-libraries)-0[1-6]\b/giu;
...
export function findNoteCompletionAssignmentLeaks(
  value, path = '$', findings = [], inheritedQuestionId = null, assignmentContext = false,
) {
  if (Array.isArray(value)) {
    value.forEach((entry, index) => findNoteCompletionAssignmentLeaks(
  ...
  for (const [key, entry] of Object.entries(value)) {
    ...
    findNoteCompletionAssignmentLeaks(key.replace(/[-_]+/gu, ' '), `${path}.${key}:key`,
      findings, scoped, fieldIsAssignment);
    findNoteCompletionAssignmentLeaks(entry, `${path}.${key}`, findings, scoped,
...
function assertCleanPacket(packet, label) {
  ...
  assert(findNoteCompletionAssignmentLeaks(packet).length === 0,
...
function sourceAssets(catalog) {
  const sets = catalog.IELTS_NOTE_COMPLETION_PASSAGES;
  assert(Array.isArray(sets), 'Falta IELTS_NOTE_COMPLETION_PASSAGES.');
  return sets.map((set, setIndex) => ({
    assetId: `formative:note-completion:${set.id}`,
    id: set.id,
    setIndex,
    title: set.title,
    ...
    notesTitle: set.notesTitle,
    noteGroups: set.noteGroups,
    questions: set.noteGroups.flatMap((group, groupIndex) => group.items.map(
      (question, itemIndex) => ({
        ...question,
        groupIndex,
        itemIndex,
        groupHeading: group.heading,
        questionIndex: groupIndex * 2 + itemIndex,
      }),
    )),
...
function structuralRiskProfile(assets) {
  ...
  const locationHintIds = questions.filter(({ question }) =>
    /\b(?:first|last|final|start|end|paragraph|sentence|near|before|after|between|mentioned|example|involving|look at|look for|find|follows?|contrasts|changes|copy|third|list|documentation|storage|cooking|signage|pilot|farming|location|adjectives?|nouns?|verbs?)\b/iu
  ...
  return {
    ...
    canonicalAnswerTokenVisibleInNoteFrame: {
...
export function assertUnitCardinality(assets) {
  ...
  'Note Completion: IDs o cardinalidad 3/18/18 inválidos.');
  if (canonicalAnswers === 18) {
    const inventory = acceptedInventory(assets);
    assert(inventory.length === 18 &&
      new Set(inventory.map(row => row.normalizedText)).size === 18,
    'Note Completion: accepted surface 18/18 inválida.');
    assert(assets.every(asset => asset.wordLimit === 'NO MORE THAN TWO WORDS' &&
      ...
      asset.instructions === EXPECTED_INSTRUCTIONS && asset.noteGroups.length === 3 &&
      asset.noteGroups.every(group => group.items.length === 2) &&
    ...
    'Note Completion: wordLimit/instructions/3x2x3 inválido.');
    ...
    'Note Completion: hashes de objetos/pasajes cambiaron.');
  }
  return true;
}

export function assertCurrentNoteUnitPinned() {
  const catalog = loadTsModule(CATALOG_PATH);
  const assets = sourceAssets(catalog);
  assertUnitCardinality(assets);
  const routeObjectSha256 = sha256(stableJson(routeObject(catalog, 'note-completion')));
  ...
  assert(routeObjectSha256 === EXPECTED_ROUTE_OBJECT_SHA256,
    'Note Completion: route object cambió.');
  assert(closure.length === 15 &&
  ...
  'Note Completion: clausura renderizable 15/hash cambió.');
  return {
...
export function expectedNextUnitPin() {
  const catalog = loadTsModule(CATALOG_PATH);
  const nextSets = catalog.IELTS_TABLE_COMPLETION_PASSAGES;
  assert(Array.isArray(nextSets) && nextSets.length === 3,
    'F0.2b.11 no conserva exactamente tres objetos de catálogo.');
  const currentSourceSha256 = Object.fromEntries(
    NEXT_UNIT_PATHS.map(path => [path, sourceSha256(path)]));
  const currentRouteObjectSha256 = sha256(stableJson(routeObject(catalog, 'table-completion')));
  ...
  const tableRouteText = readFileSync(resolve(ROOT, NEXT_ROUTE_PATH), 'utf8');
  const historicalSourcesRemainPinned = Object.entries(EXPECTED_NEXT_UNIT_SOURCE_SHA256)
    .filter(([path]) => ![NEXT_ROUTE_PATH, NEXT_ENGINE_PATH].includes(path))
    .every(([path, expected]) => currentSourceSha256[path] === expected);
  const remediatedRouteIsPinned =
    sourceSha256(NEXT_ROUTE_PATH) ===
      '0907ee7ba7db6b3bab0987ab25dcb12b1958a2f80e2a91e551058aa3d6835895' &&
    sourceSha256(NEXT_ENGINE_PATH) ===
      'c49bd375fb061ff7ca35afb0219194393c5a65bd7e44e1f68cc6338bfe329a14' &&
    sourceSha256('src/data/practica-exams/ielts-table-completion-remediated.ts') ===
      '74d08ab43fa69659d90167ca48a076b8675847eb37f8749dab9080b35a7eed76' &&
    sourceSha256('src/lib/ielts/table-completion-publication.ts') ===
      '9e05decd6c9d1d1a7e06d65f73aaf277b573ad95040a47aa013a1a74598c65c4' &&
    sourceSha256('src/components/exam-practice/TableCompletionReviewSourceBlock.tsx') ===
      '528dc846c306ef45cd3d00026bcac1f4528d65eeebf76a5885baf51a7af3d700' &&
    tableRouteText.includes('getTableCompletionPublicationState') &&
    tableRouteText.includes('data-testid="table-completion-quarantine"');
  assert(historicalSourcesRemainPinned && remediatedRouteIsPinned &&
    currentRouteObjectSha256 === EXPECTED_NEXT_ROUTE_OBJECT_SHA256 &&
    stableJson(currentSourceObjectSha256) ===
      stableJson(EXPECTED_NEXT_SOURCE_OBJECT_SHA256) && closureCount === 19,
  'F0.2b.11 Table Completion no conserva el snapshot histórico o su remediación pública exacta.');
  return {
    unit: 'F0.2b.11',
    format: 'table-completion',
    sourceSha256: EXPECTED_NEXT_UNIT_SOURCE_SHA256,
    routeObjectSha256: EXPECTED_NEXT_ROUTE_OBJECT_SHA256,
    sourceObjectSha256: EXPECTED_NEXT_SOURCE_OBJECT_SHA256,
    scope: {
      passages: 3,
      questions: 18,
      renderDependencyClosurePaths: 15,
    },
...
export function assertNextUnitPinned(baseline) {
  ...
  assert(stableJson(baseline.nextUnit) === stableJson(expectedNextUnitPin()),
    'F0.2b.11 cambió o su pin route/engine/bank/objetos es incompleto.');
  return true;
}
...
export function assertBoardStopBoundary(loopDocText) {
  ...
  const stop = 'Siguiente subunidad, sin iniciarla: ' + String.fromCharCode(96) +
    'F0.2b.11 Table Completion' + String.fromCharCode(96) + '.';
  ...
  exactOnce(nextOpen, '↳ F0.2b.11 Table Completion');
  exactOnce(parentOpen, '↳ F0.2b Adjudicación de bancos formativos — padre');
  assert(loopDocText.split(stop).length - 1 === 1,
    'El corte antes de F0.2b.11 debe aparecer exactamente una vez.');
  const nextMentions = rows.filter(row => /F0\.2b\.11 Table Completion/iu.test(row));
  assert(!nextMentions.some(row =>
    /(?:iniciad[oa]|started|en curso|cerrad[oa]|completed)/iu.test(row) && !row.includes(stop)),
  'F0.2b.11 tiene una declaración contradictoria con el corte de no inicio.');
  return true;
}

function assertHistoricalOrAdvancedBoardBoundary(loopDocText) {
  try {
    return assertBoardStopBoundary(loopDocText);
  } catch {
    const rows = loopDocText.split(/\r?\n/gu);
    const exactRow = (label, expected) => {
      const matches = rows.filter(row => row.includes(label));
      assert(matches.length === 1 && matches[0] === expected,
        `${label}: el tablero avanzado no conserva una fila exacta.`);
    };
    exactRow('↳ F0.2b.10 Note Completion',
      '| 0 | 　　　 ↳ F0.2b.10 Note Completion | ✅ | ✅ | ✅ | ✅ | ✅ | ➖ | ➖ |');
    exactRow('↳ F0.2b.11 Table Completion',
      '| 0 | 　　　 ↳ F0.2b.11 Table Completion | ✅ | ✅ | ✅ | ✅ | ✅ | ➖ | ➖ |');
    exactRow('↳ F0.2b.12 Flow-chart Completion',
      '| 0 | 　　　 ↳ F0.2b.12 Flow-chart Completion | — | — | — | — | — | — | — |');
    exactRow('↳ F0.2b Adjudicación de bancos formativos — padre',
      '| 0 | 　 ↳ F0.2b Adjudicación de bancos formativos — padre | 🟡 | 🟡 | 🟡 | 🟡 | 🟡 | 🟡 | 🟡 |');
    const stop = 'Siguiente subunidad, sin iniciarla: `F0.2b.12 Flow-chart Completion`.';
    assert(loopDocText.split(stop).length - 1 === 1,
      'El tablero avanzado no conserva un único corte antes de F0.2b.12.');
    return true;
  }
}
...
export function buildPromptOnlyPacket() {
  ...
  const packet = basePacket(
    'ielts-reading-note-completion-prompt-only.v1',
    'F0.2b.10 — grammar and visible-note shortcut audit over 18 open responses',
    'Use only the visible note frames and word limits. Do not open passages, hints, keys, blind packets, registry or prior reviews.',
    ...
    assets.map(asset => ({
      ...
      notesTitle: asset.notesTitle,
      noteGroups: asset.noteGroups.map(group => ({
...
export function buildBlindReviewPacket() {
  ...
  assertUnitCardinality(assets);
  const packet = basePacket(
    'ielts-reading-note-completion-blind-review.v1',
    'F0.2b.10 — clean exact one-best-answer adjudication over three passages',
    ...
    'Complete every note gap with a contiguous span from its passage. Use no more than two words; a hyphenated expression counts as one word and contractions are not tested. For official type 9, answers may not occur in passage order and usually come from one part rather than the whole text; judge this bank as written.',
    'Persist the completed note sentence, exact passage span, evidence, closest plausible competing span and ambiguity. Also judge whether the six separated cards function as a genuine continuous note. Do not use hints or outside knowledge.',
...
export function buildFactualSourceReviewPacketFromRegistry(registry, prerequisiteHashes) {
  ...
  const packet = {
    schemaVersion: 'ielts-reading-note-completion-factual-source-review.v1',
    generatedAt: FACTUAL_PACKET_GENERATED_AT,
    reviewScope: 'F0.2b.10 — second-pass factual-source review only',
    reviewerIsolation:
...
function validatePromptOnlyVerdict(verdict, assets) {
  ...
  assert(verdict.schemaVersion === 'ielts-reading-note-completion-prompt-only-verdict.v1',
...
function validateFirstPass(firstPass, assets) {
  ...
  assert(firstPass.schemaVersion === 'ielts-reading-note-completion-expert-first-pass.v1',
...
function validateExpertVerdict(verdict, firstPass, firstTrace, assets, factualPacket) {
  ...
  assert(verdict.schemaVersion === 'ielts-reading-note-completion-expert-verdict.v1',
...
function validateStudentWalkthrough(walkthrough, assets) {
  ...
  assert(walkthrough.schemaVersion ===
    'ielts-reading-note-completion-student-walkthrough.v1',
...
function validateSourceAvailability(ledger, registry, unitRecords) {
  assertExactKeys(ledger,
    ...
    'sourceAvailability');
  assert(ledger.schemaVersion === 'ielts-reading-note-completion-source-availability.v1' &&
  ...
  assert(expectedIds.length === 17 && new Set(expectedIds).size === 17,
    'La unidad requiere 2 fuentes oficiales + 15 fuentes factuales únicas.');
  ...
  assert(ledger.coverageContract.officialSources === 2 &&
    ledger.coverageContract.factualSources === 15 &&
    ledger.coverageContract.totalSources === 17 &&
    stableJson(ledger.coverageContract.factualSourcesPerAsset) === stableJson({
      'formative:note-completion:note-mobile-libraries': 5,
      'formative:note-completion:note-seed-banks': 5,
      'formative:note-completion:note-night-libraries': 5,
    }) &&
  ...
  'sourceAvailability.coverageContract contradice 2+15.');
...
function validateProvenanceSearch(ledger, registry) {
  assertExactKeys(ledger,
    ...
    'provenanceSearch');
  assert(ledger.schemaVersion === 'ielts-reading-note-completion-provenance-search.v1' &&
...
function validateAuditVerdicts(audit, facts = null) {
  assertExactKeys(audit, ['schemaVersion', 'reviewedAt', 'status', 'passMeaning', 'rows'],
    'auditVerdicts');
  assert(audit.schemaVersion === 'ielts-reading-note-completion-audit-verdicts.v1' &&
  ...
  assert(byLane.get('Anti-bias and leakage').findings.includes('14/18') &&
...
function validateBaseline(baseline, assets, storedRisk, renderClosurePaths) {
  ...
  assert(baseline.schemaVersion === 'ielts-reading-note-completion-baseline.v1' &&
    baseline.scope.unit === 'F0.2b.10' && baseline.scope.format === 'note-completion' &&
  ...
  assert(structural.answerSurfaceProfile.questions === 18 &&
    ...
    structural.answerSurfaceProfile.normalizedAcceptedValues === 18 &&
    ...
    structural.answerSurfaceProfile.manuallyClassifiedNominalOrNounPhrasePrimaryAnswers === 15 &&
    structural.answerSurfaceProfile.manualClassificationCaveat.includes('Manual'),
  'baseline.answerSurfaceProfile contradice 18/18/18 o sus categorías gramaticales.');
  ...
  assert(structural.textOrder.setsWithStrictlyAscendingEvidenceOrder ===
    ...
    structural.textOrder.canonicalAnswersWithExactlyOneWholeTokenOccurrence === 15 &&
    structural.textOrder.canonicalAnswersWithMultipleWholeTokenOccurrences === 3 &&
    stableJson(structural.textOrder.naiveFirstMatchOrderViolationQuestionIds) ===
      stableJson(['note-night-libraries-05']) &&
  ...
  assert(structural.shortcutSignals.questionsWithUniqueExactPromptPassageBigram === 17 &&
    structural.shortcutSignals.alwaysTwoWordPrimaryAnswerHits === 5 &&
    structural.shortcutSignals.sameQuestionPositionAcrossSetsModalEvidenceParagraphHits === 18 &&
    structural.shortcutSignals.manuallyClassifiedNominalOrNounPhrasePrimaryAnswers === 15 &&
...
function validateUnitChangeManifest(manifest, renderClosurePaths) {
  ...
  assert(manifest.schemaVersion ===
    'ielts-reading-note-completion-unit-change-manifest.v1' &&
    manifest.unit === 'F0.2b.10' &&
  ...
  assert(manifest.nextUnitBoundary.unit === 'F0.2b.11' &&
    manifest.nextUnitBoundary.format === 'table-completion' &&
    manifest.nextUnitBoundary.status === 'not-started',
  'unitChangeManifest: frontera F0.2b.11 inválida.');
...
export function buildValidationArtifacts() {
  ...
  assert(findNoteCompletionAssignmentLeaks(closureTexts).length === 0,
  ...
  const runtime = {
    ...
    storedWordLimitParsedForEnforcement:
      engineText.includes('wordCount(value) > passage.maxWords') &&
      engineText.includes('{passage.maxWords} words'),
  ...
  const leakControls = {
    englishAnswer: findNoteCompletionAssignmentLeaks({
      instruction: 'Answer for note-mobile-libraries-06 is volunteer groups',
    }),
    englishCompletion: findNoteCompletionAssignmentLeaks({
      instruction: 'Complete note-mobile-libraries-01 with expensive',
    }),
    spanishCompletion: findNoteCompletionAssignmentLeaks({
      nested: { repair: 'note-seed-banks-01 se completa con harvest' },
    }),
    spanishAnswer: findNoteCompletionAssignmentLeaks({
      repair: 'La respuesta de note-night-libraries-02 es wireless internet',
    }),
    nestedField: findNoteCompletionAssignmentLeaks({
      questionId: 'note-night-libraries-01', review: { selectedAnswer: 'home' },
    }),
  };
  ...
  assertHistoricalOrAdvancedBoardBoundary(readFileSync(resolve(ROOT, LOOP_DOC_PATH), 'utf8'));
  ...
  const validation = {
    schemaVersion: 'ielts-reading-note-completion-rights-validation.v1',
    generatedAt: audit.reviewedAt,
    unit: 'F0.2b.10 — three formative Note Completion passages',
    status: 'pass',
    passMeaning:
    ...
    checks: {
      ...
      factualSourceAvailabilityRecordedNotVerified:
        sourceAvailability.sources.length === 17 &&
      ...
      threePacketsContainNoKeysAlternativesEditorialFeedbackHintsPiiOrAssignments:
        [promptOnlyPacket, blindPacket, factualPacket].every(packet =>
          ...
          findNoteCompletionAssignmentLeaks(packet).length === 0 &&
      ...
      multidimensionalShortcutAuditComplete:
        storedRisk.alwaysTwoWordAnswerPredictor.hits === 5 &&
        storedRisk.sameQuestionPositionAcrossSetsModalAnswerWordCount.hits === 14 &&
        storedRisk.sameQuestionPositionAcrossSetsModalEvidenceParagraph.hits === 18 &&
        storedRisk.canonicalAnswerTokenVisibleInNoteFrame.questionsWithOverlap === 0 &&
        storedRisk.promptPassageLexicalAnchors.questionsWithUniqueExactBigram === 17,
  ...
  const failed = Object.entries(validation.checks).filter(([, passed]) => !passed)
    .map(([name]) => name);
  assert(failed.length === 0, `Falló gate F0.2b.10: ${failed.join(', ')}`);
...
export function validateFinalReportArtifacts(validation) {
  ...
  assert(artifact.surface === 'report' && artifact.manifest?.surface === 'report' &&
    artifact.manifest.title === 'IELTS Reading Note Completion — audit gate' &&
  ...
  assert(sourceSha256(REPORT_HTML_PATH) === EXPECTED_REPORT_HTML_SHA256,
    'report.html no coincide byte a byte con la entrega Chromium verificada.');
  ...
  const reportBuildDirectory = mkdtempSync(join(tmpdir(), 'ielts-note-report-build-'));
  try {
  ...
  assert(html.includes('IELTS Reading Note Completion') &&
  ...
  assert(verification.schemaVersion ===
    'ielts-reading-note-completion-report-verification.v1' &&
