...
- **Note Completion:** progressive vertical implemented with 6 source-backed or conservatively bounded passages and 36 structured-note decisions. Guided and independent pools are held apart from four Progress Engine passages; three visible headings map the evidence zones, full-set feedback stays closed, guided errors reopen for repair and local drafts use a versioned storage key. Browser and release gates must pass before this line becomes a production certificate.
- **Next question-type vertical:** Table Completion. Apply this same content, accessibility, persistence, real-browser and release contract before scaling again.
