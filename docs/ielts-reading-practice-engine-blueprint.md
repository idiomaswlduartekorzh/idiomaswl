...
- **Sentence Completion:** progressive vertical implemented with 6 source-backed or conservatively bounded passages and 30 exact-span decisions. Guided and independent pools are held apart from four Progress Engine passages; each answer is checked against its displayed limit, literal passage wording and rebuilt sentence, while feedback diagnoses six answer-boundary and evidence errors. Browser and release gates must pass before this line becomes a production certificate.
- **Next question-type vertical:** Summary Completion. Apply this same content, accessibility, persistence, real-browser and release contract before scaling again.
