...
- **Summary Completion:** progressive vertical implemented with 6 source-backed or conservatively bounded passages and 36 exact-span decisions. Guided and independent pools are held apart from four Progress Engine passages; the learner maps the connected overview before solving gaps, full-set feedback stays closed and local drafts use a versioned storage key. The inherited `the roof` alternative is rejected because its frame would read `the the roof`. Browser and release gates must pass before this line becomes a production certificate.
- **Next question-type vertical:** Note Completion. Apply this same content, accessibility, persistence, real-browser and release contract before scaling again.
