...
- **Matching Information:** progressive vertical implemented with 6 source-backed passages and 30 exact-detail decisions. Guided radio cards, a held-back independent set, paragraph reuse, closed full-set feedback, deterministic drill order, local drafts and 6 progress levels are pinned in both the canonical source page and public rewrite destination. The 3 legacy passages retain their conservative candidate-source limitations; 3 existing source-backed passages are reused for transfer without creating new factual-verification claims. Browser and release gates must pass before this line is used as a production certificate.
- **Next question-type vertical:** Matching Features. Apply this same content, accessibility, persistence, real-browser and release contract before scaling again.
