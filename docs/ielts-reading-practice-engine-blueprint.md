...
- Mixed Practice must preserve the native response contract of every question family. It may combine tasks inside one passage, but it cannot replace a real answer with a quiz about the name of the format. Evidence-state labels, paragraph or feature choices, one-best options and exact passage spans retain their own scoring, word-limit, grammar, attribution and reuse rules.
- Guided Mixed Practice may reveal the task family after an explicit check and must reopen a wrong response for repair. Independent and Progress Engine sets keep task labels, keys, evidence and explanations closed until one complete submission. Guided, independent and engine passage pools remain disjoint, and every feedback block links back to the focused lesson for that family.
...
- **Short Answer:** complete progressive vertical with 6 source-backed or conservatively bounded passages and 36 ordered factual-detail decisions. Guided and independent pools are held apart from four Progress Engine passages; each response classifies its target before scanning, rejects nearby true details, uses the smallest exact span and obeys the displayed maximum. Full-set feedback stays closed, guided errors reopen for repair and local drafts use a versioned storage key. Static gates, production build, desktop/mobile interaction, missing-answer focus, repair behaviour, reload persistence and production release passed.
- **Historical handoff completed — Next product vertical:** Mixed Practice integration. This marker remains immutable so the audited Short-Answer-to-Mixed-Practice transition cannot disappear when the roadmap advances.
- **Mixed Practice integration:** complete progressive vertical with 6 passages and 36 authentic decisions across all 14 question families. Guided and independent pools are held apart from four Progress Engine passages; responses use their native label, option, paragraph/category or exact-span contract instead of identifying a format name. Wrong guided decisions reopen for repair, complete-set feedback stays closed, local drafts use a versioned storage key and feedback links to every focused lesson. Static gates, production build, desktop/mobile interaction, missing-answer focus, repair behaviour, reload persistence and production release are the required completion evidence for the entire Reading product.
