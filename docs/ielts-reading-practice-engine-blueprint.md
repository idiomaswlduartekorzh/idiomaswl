...
- **Matching Features:** progressive vertical implemented with 6 source-backed or conservatively bounded passages and 30 attribution decisions. Guided and independent pools are held apart from 4 Progress Engine passages; native radios remain editable before checking, full sets use labelled selects, features may repeat, feedback stays closed until submission and local drafts use a versioned storage key. Three existing scenarios preserve explicit candidate-source limitations; three reviewed passages are adapted without adding new factual-verification claims. Browser and release gates must pass before this line is used as a production certificate.
- **Next question-type vertical:** Matching Sentence Endings. Apply this same content, accessibility, persistence, real-browser and release contract before scaling again.
