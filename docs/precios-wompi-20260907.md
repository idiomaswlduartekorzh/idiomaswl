...
3. Supabase ya tiene clave de servidor en Production y Preview. Resend quedó vinculado mediante una clave con permiso exclusivo de envío, cargada como secreto en ambos entornos de Vercel. Falta completar la verificación DNS de `idiomaswl.com`, comprobar el correo de activación de Supabase y probar entrega, rebote y que una repetición del webhook no duplique correos. `COURSE_FROM_EMAIL`, `COURSE_OWNER_NOTIFICATION_EMAIL` y `COURSE_ORDER_ACCESS_SECRET` ya están configuradas.
