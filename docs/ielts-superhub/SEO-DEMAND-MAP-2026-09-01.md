...
| `question types`, `types practice`, `examples` | `/practica/ielts/listening/tipos-de-preguntas` (provisional) | **defer** | Requiere catálogo completo, ejemplo original y práctica funcional por tipo. Congelar un solo slug después de investigación regional; nunca publicar un alias competidor. |
| `score`, `band score`, `calculator`, `conversion` | futura guía de score | **defer** | Intención distinta. Debe citar la tabla oficial vigente, fecharla y evitar prometer una banda desde una práctica de 10 ítems. |
