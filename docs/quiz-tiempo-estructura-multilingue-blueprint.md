...
Corrección local de continuidad en Mac mini (4 de septiembre de 2026; todavía sin integrar):
los microtextos de imperativo con `mitnehmen`, `ausfüllen` y `abgeben` evalúan solo el fragmento
ausente; el relato y la reparación de noticias del presente conservan `aus` y `vor` al final
de su cláusula. La clave de almacenamiento alemana pasa a `wl-german-structure-quest-v5` para
no reutilizar respuestas guardadas con las claves v3 de main o v4 de la rama anterior.
El guardián inspecciona partículas mal colocadas en microtextos, relatos y textos reparados de
las formas finitas; una regresión reconstruye literalmente los cinco ejercicios corregidos.
Referencia lingüística: [IDS grammis, Satzklammer](https://grammis.ids-mannheim.de/sgt/2269),
sobre la separación del verbo finito y la partícula en oraciones principales. Esta comprobación
es acotada a los bancos existentes; no constituye una auditoría lingüística completa.
