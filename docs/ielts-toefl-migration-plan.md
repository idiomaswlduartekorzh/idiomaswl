...
- The IELTS Matching Headings route now follows the progressive Reading blueprint: six source-backed passages, thirty paragraph-function decisions, a clickable guided Heading Bank, a held-back five-paragraph independent set, eight locally persisted levels, non-reuse enforcement, two unused headings per full set and explicit distractor diagnosis. The canonical source page and its public rewrite destination are both guarded so the former Spanish-only lesson cannot silently replace the real practice surfaces.
