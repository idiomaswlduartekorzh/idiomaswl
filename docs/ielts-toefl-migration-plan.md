...
Current status: the IELTS Reading skills hub is live as the cluster index with official-format vs WeLearn-strategy guidance, recommended study order, FAQ schema and skill-to-question transfer links. All six subskills—Skimming, Scanning, Paraphrase Recognition, Inference, Word-limit control and Time management—now implement the complete practice blueprint: explicit decisions, held-back independent passages, six-level local Progress Engines, source boundaries, draft persistence and error review. After Time management passes release gates, scaling returns to question-type routes one by one, beginning with Multiple Choice. These pages include a direct answer, official-format vs WeLearn-strategy note, guided practice, independent practice, answers/explanations, common mistakes/checklists, FAQs, next lessons and internal links back to IELTS Reading and official task-type pages.
