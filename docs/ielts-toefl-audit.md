...
- The audio library remains available at `/podcasts`, but each of the eight exam-preparation episodes now has a canonical editorial URL under `/examenes/<exam>/podcast/<slug>`.
...
- The new files and all eight public episode routes are protected in `config/production-baseline.json`. Operational details live in `docs/podcast-exam-hubs.md`.
...
- 2026-08-03 Task 1 discoverability closure: all ten canonical lessons, including `body-1` and `body-2`, are represented in the XML sitemap. The direct timed-writing route under `tarea-completa/sesion` is deliberately `noindex, nofollow` and omitted from the sitemap because it is an individual study surface, not a competing information page. The content validator now protects both decisions.
