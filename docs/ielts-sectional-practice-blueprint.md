...
Reading presenta una banda estimada a partir de sus 40 respuestas y un desglose por pasaje. La corrección ocurre en `/api/practica/ielts/reading/score`: valida la versión editorial, los identificadores de respuesta y una cuota por conexión, y devuelve puntuación sin solucionario. Writing muestra los dos borradores y sus extensiones; Speaking permite grabar y reproducir durante la sesión. Ninguna de estas dos últimas habilidades presenta una banda automática ni afirma que la práctica se haya enviado al profesor. Las grabaciones de Speaking no se guardan en `localStorage` y desaparecen al cerrar la sesión.

El runner del mock completo recibe una proyección sin claves ni transcripciones durante el examen. Después de verificar la entrega, el servidor devuelve el mock de revisión al navegador para mostrar la corrección detallada. La prueba `ielts-runner-key-boundary` inspecciona las 20 proyecciones y verifica que Reading conserva la ruta 40/40 en el servidor. La revisión de seguridad debe comprobar además que el bundle de cliente no importe `exam-bridge/ielts`, que contiene los loaders del banco de mocks.
...
La ampliación editorial futura seguirá el principio de fuente única: auditar primero el set completo y después exponerlo en las bibliotecas. Listening mantiene además su PDF estudiantil. Si se necesita descarga de hojas para Reading, Writing o Speaking, se añadirá un adaptador específico antes de habilitarla; el modo actual no promete esas prestaciones.
