...
| teacher-ops | BLOCKED | `src/lib/icfes/teacher-ops.server.ts`, `supabase/migrations/20260909183000_icfes_teacher_review_delivery.sql`, `tests/icfes-teacher-api-v1.test.mjs` | El camino local cubre solicitud visible, membership server-side, payload sin PII, credencial de revisor, leases con heartbeat, QA/retry y resultado inmutable. Sigue bloqueado porque no existen worker desplegado, roster calibrado real, métricas observadas, migraciones aplicadas ni corrida Sandbox. |
