...
alter table public.xpress_orders
  ...
  add constraint xpress_orders_offer_quote_check check (
    ...
    or (
      offer_version in ('xpress-2026-09-09-v3','xpress-2026-09-12-v4')
      and (
...
alter table public.xpress_teacher_capacity_reservations
  ...
  check (addendum_version in ('icfes-teacher-addendum-2026-09-09-v1','icfes-teacher-addendum-2026-09-12-v2'));
...
begin
  if p_user is null or p_key is null or p_environment not in ('sandbox','production')
    ...
    or p_addendum not in ('icfes-teacher-addendum-2026-09-09-v1','icfes-teacher-addendum-2026-09-12-v2')
...
begin
  ...
  if not (
    ...
    or p_offer_version in ('xpress-2026-09-09-v3','xpress-2026-09-12-v4') and (
...
declare
  ...
  membership_end timestamptz; selected_membership_id uuid;
begin
  ...
  if approved_count>0 then
    ...
    if selected_order.order_kind='single' then
      ...
      insert into public.xpress_memberships(user_id,source_order_id,credited_provider_id,environment,exam_slug,offer_id,grant_kind,starts_at,ends_at)
...
begin
  ...
  update public.xpress_teacher_review_notifications set status=case when attempts>=5 then 'dead' else 'failed' end,lease_id=null,
