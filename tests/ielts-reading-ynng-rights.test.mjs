...
} from '../scripts/check-ielts-reading-ynng-rights.mjs';

const { validation, blindReview, factualSourceReview } = buildValidationArtifacts();

test('F0.2b.3 covers exactly three formative sets and 22 unique writer-view statements', () => {
...
test('official rule and all eight candidate factual sources have recorded availability', () => {
  ...
  assert.equal(validation.sourceAvailability.sources.length, 9);
...
test('all three assets remain deny-by-default quarantined under valid records', () => {
...
test('independent expert enforces writer attribution and absence contracts for all decisions', () => {
  ...
  assert.equal(validation.checks.viewpointAttributionContractEnforced, true);
  assert.equal(validation.checks.notGivenAbsenceContractEnforced, true);
  assert.equal(validation.expertReview.answerAgreement.total, 22);
  assert.equal(validation.expertReview.humanSignature, false);
});

test('average-student walkthrough covers every statement without assigning keys', () => {
...
test('fixed-label anti-shortcut profile covers degree, title, wording and sequence cues', () => {
  ...
  assert.equal(validation.antiBias.storedKeyProfile.degreeLanguageImpliesNo.eligible, 11);
  assert.equal(validation.antiBias.storedKeyProfile.degreeLanguageImpliesNo.hits, 6);
  ...
  assert.ok(Object.keys(validation.antiBias.storedKeyProfile.writerVerbByLabel).length >= 4);
...
test('content-hash and text-value leakage mutations fail closed', () => {
...
test('chronology, final panel and portable report closure are validated', () => {
...
test('board state closes exactly F0.2b.3 and preserves F0.2b.4 as the stop boundary', () => {
