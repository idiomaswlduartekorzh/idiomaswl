...
test('ready notes and may-repeat choices agree with the existing transport validator', () => {
  for (const spec of [notes(), matching()]) {
    const specs = spec.questionNumbers.map((number) => spec.type === 'matching'
      ? { number, kind: 'choice', allowedValues: spec.options.map(({ key }) => key) }
