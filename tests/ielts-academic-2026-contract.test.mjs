...
test('all 20 sets have an explicit editorial certification state', () => {
  ...
  assert.equal(IELTS_EDITORIAL_STATUS_2026[17].certification, 'certified-golden-content');
  ...
  assert.equal(IELTS_EDITORIAL_STATUS_2026[20].certification, 'certified-golden-content');
  assert.equal(IELTS_EDITORIAL_STATUS_2026[20].provenance, 'audited-original-welearn');
});
...
test('all 20 Listening papers preserve authored evidence while Sets 1–15 pin the timing-density reference', async () => {
  ...
  for (let setNumber = 1; setNumber <= 20; setNumber += 1) {
    ...
    assert.ok(listeningWords >= (setNumber <= 15 ? 2800 : 2200), `${mock.id} Listening density`);
    for (const [index, section] of listening.entries()) {
      ...
      const partWordRange = setNumber <= 15 ? [680, 760] : [540, 620];
