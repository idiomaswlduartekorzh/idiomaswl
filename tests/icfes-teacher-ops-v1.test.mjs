...
test('the unapplied SQL keeps teacher operations behind service-role RLS', async () => {
  const sql = `${await readFile(new URL('../supabase/migrations/20260909001000_icfes_teacher_review_queue.sql', import.meta.url), 'utf8')}\n${await readFile(new URL('../supabase/migrations/20260909002000_icfes_attempt_claim_evidence.sql', import.meta.url), 'utf8')}\n${await readFile(new URL('../supabase/migrations/20260909183000_icfes_teacher_review_delivery.sql', import.meta.url), 'utf8')}\n${await readFile(new URL('../supabase/migrations/20260912160504_icfes_teacher_12h_operational_loop.sql', import.meta.url), 'utf8')}`;
  ...
  assert.match(sql, /grant execute on function public\.claim_xpress_teacher_review_v3/);
  assert.match(sql, /grant execute on function public\.complete_xpress_teacher_review_admin_v3/);
