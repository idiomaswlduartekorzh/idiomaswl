...
test('ICFES keeps its attempt-bound COP 12,900 pass separate from generic exam credits', () => {
...
test('the ICFES UI states one credit, a capacity-dependent target and explicit acceptance', () => {
  const client = read('src/app/(site)/suscripcion/examenes/XpressMembershipClient.tsx');
  assert.match(client, /feedback pedagógico personalizado de WeLearn con asistencia de IA/);
  assert.match(client, /objetivo operativo de 12 horas sujeto a capacidad/);
  assert.match(client, /name="accepted_icfes_teacher_addendum"/);
  ...
  assert.match(client, /isIcfesTeacherOffer[\s\S]{0,360}feedback pedagógico personalizado/);
  assert.match(client, /plan automático incluye reporte y áreas de atención generadas por el sistema\. No incluye feedback personalizado/);
});
...
test('unavailable teacher tier is visibly disabled and accessibility-linked', () => {
  const client = read('src/app/(site)/suscripcion/examenes/XpressMembershipClient.tsx');
  assert.match(client, /includedByMembership \|\| membershipUnavailable \|\| teacherUnavailable/);
  ...
  assert.match(client, /recurringReady[\s\S]{0,260}!isIcfesMembershipUnavailable[\s\S]{0,100}attemptClaim !== 'claiming'/);
});
