...
test('audio-blocked and legacy Goethe A2 mocks cannot be resolved through the public registry', () => {
  for (let set = 1; set <= 5; set += 1) {
    const id = `a2-${set}`;
    assert.equal(isGoetheA2Held(id), true);
    assert.equal(isGoetheA2Published(id), false);
    assert.equal(getGoetheA2Release(id)?.state, set === 1 ? 'AUDIO_BLOCKED' : 'LEGACY_HOLD');
