...
const baseMigration = readFileSync('supabase/migrations/20260908170000_icfes_secure_attempts_and_pass.sql', 'utf8');
...
test('the current draft keeps export, deletion and purge fail-closed', () => {
  assert.match(baseMigration, /VALUES \('icfes-privacy-2026-09-draft', 'DRAFT_BLOCKED'\)/);
  assert.doesNotMatch(baseMigration, /VALUES \('icfes-privacy-2026-09-draft', 'APPROVED'\)/);
