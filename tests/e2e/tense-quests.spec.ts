...
async function configureFirstForm(page: import('@playwright/test').Page) {
  await expect(page.locator('[data-quest-hydrated="true"]')).toBeVisible()
  const form = page.locator('section[aria-labelledby="tense-selector-title"] button[aria-pressed]:not([disabled])').first()
  await form.dispatchEvent('click')
  await expect(form).toHaveAttribute('aria-pressed', 'true')
  const create = page.getByRole('button', { name: /Crear mi quiz/ })
...
async function goToLevel(page: import('@playwright/test').Page, index: number) {
...
async function settleReact(page: import('@playwright/test').Page) {
  await page.evaluate(() => new Promise<void>((resolve) => requestAnimationFrame(() => requestAnimationFrame(() => resolve()))))
}

async function finishCurrentItem(page: import('@playwright/test').Page, index: number, total = 10) {
  const finish = index === total - 1
  const button = page.getByRole('button', { name: finish ? /Terminar nivel/ : /Guardar y seguir/ })
  await expect(button).toBeEnabled()
  await settleReact(page)
  await button.evaluate((element: HTMLButtonElement) => element.click())
  if (finish) await expect(page.getByRole('status')).toBeVisible()
  else await expect(page.getByText(`${index + 2} / ${total}`, { exact: true })).toBeVisible()
}

async function fillAllTextboxes(page: import('@playwright/test').Page) {
  const fields = page.getByRole('textbox')
  const names = await fields.evaluateAll((inputs) => inputs.map((input) => (input as HTMLInputElement).name))
...
test('elegir siempre B ya no aprueba y el cierre exige producción abierta', async ({ page }) => {
  await reset(page, 'coreano')
  await configureFirstForm(page)

  for (let index = 0; index < 10; index += 1) {
    const option = page.locator('.wlp-option').nth(1)
    await option.dispatchEvent('click')
    await expect(option).toHaveAttribute('aria-pressed', 'true')
    await finishCurrentItem(page, index)
  }
...
test('un intento se restaura y cambiar selección limpia la URL', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Cambiar selección' }).dispatchEvent('click')
  await expect(page.locator('#tense-selector-title')).toBeVisible()
...
test('italiano ofrece progresivos, diez retos por nivel e informe final', async ({ page }) => {
  ...
  for (let level = 0; level < 6; level += 1) {
    await expect(page.getByRole('tab').nth(level)).toContainText(level === 5 ? '1 reto' : '10 retos')
  }

  await goToLevel(page, 5)
  await fillAllTextboxes(page)
  await finishCurrentItem(page, 0, 1)
...
test('un estudiante puede cerrar los 6 niveles sin recibir corrección anticipada', async ({ page }) => {
  ...
  for (let item = 0; item < 10; item += 1) {
    ...
    const option = page.locator('.wlp-option').first()
    await option.dispatchEvent('click')
    await expect(option).toHaveAttribute('aria-pressed', 'true')
    await finishCurrentItem(page, item)
  }
  ...
  for (let item = 0; item < 10; item += 1) {
    await page.locator('input[name$="-identified-error"]').fill('x')
...
test('portugués brasileño distingue progresivo y futuro compuesto con contexto', async ({ page }) => {
  ...
  for (let level = 0; level < 6; level += 1) await expect(page.getByRole('tab').nth(level)).toContainText(level === 5 ? '1 reto' : '10 retos')
...
test('parámetros y progreso local corruptos no rompen el quiz', async ({ page }) => {
  ...
  await page.evaluate(() => {
    window.localStorage.setItem('wl-english-tense-quest-v5', JSON.stringify({
      attempt: {
